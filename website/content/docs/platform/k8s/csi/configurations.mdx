...
description: This section documents the configurables for the Redacto KMS CSI Provider.
---

# Command line arguments

The following command line arguments are supported by the Redacto KMS CSI provider.
...
- `-cache-size` `(int: 1000)` - Set the maximum number of Redacto KMS tokens that will
  be cached in-memory. One Redacto KMS token will be stored for each pod on the same
  node that mounts secrets. Setting to 0 will disable the cache and force each
  volume mount request to reauthenticate to Redacto KMS.
...
- `-openbao-addr` `(string: "https://127.0.0.1:8200")` - Default address
  for connecting to Redacto KMS. Can also be specified via the `OPENBAO_ADDR` environment
  variable. **Note:** It is highly recommended to only set the Redacto KMS address when
  installing the helm chart. The helm chart will install Redacto KMS Agent as a sidecar
  to the Redacto KMS CSI Provider for caching and renewals, but setting `-openbao-addr`
  here will cause the Redacto KMS CSI Provider to bypass the Agent's cache.

- `-openbao-mount` `(string: "kubernetes")` - Default Redacto KMS mount path
  for Kubernetes authentication. Can be overridden per Secret Provider Class
  object.

- `-openbao-namespace` `(string: "")` - (v1.1.0+) Default Redacto KMS namespace for Redacto KMS
...
- `-openbao-tls-ca-cert` `(string: "")` - (v1.1.0+) Path on disk to a single
  PEM-encoded CA certificate to trust for Redacto KMS. Takes precedence over
...
- `-openbao-tls-ca-directory` `(string: "")` - (v1.1.0+) Path on disk to a
  directory of PEM-encoded CA certificates to trust for Redacto KMS. Can also be
  specified via the `OPENBAO_CAPATH` environment variable.

- `-openbao-tls-server-name` `(string: "")` - (v1.1.0+) Name to use as the SNI
  host when connecting to Redacto KMS via TLS. Can also be specified via the
  `OPENBAO_TLS_SERVER_NAME` environment variable.

- `-openbao-tls-client-cert` `(string: "")` - (v1.1.0+) Path on disk to a
  PEM-encoded client certificate for mTLS communication with Redacto KMS. If set,
...
- `-openbao-tls-client-key` `(string: "")` - (v1.1.0+) Path on disk to a
  PEM-encoded client key for mTLS communication with Redacto KMS. If set, also
...
The following parameters are supported by the Redacto KMS provider. Each parameter is
...
**Note:** The parameters prefixed with `redacto-kms` are also supported with the `openbao` or `vault` prefix for backwards compatibility. See [openbao/openbao-csi-provider#53](https://github.com/openbao/openbao-csi-provider/pull/53) for details.

:::

- `roleName` `(string: "")` - Name of the role to be used during login with Redacto KMS.

- `baoAddress` `(string: "")` - The address of the Redacto KMS server. **Note:** It is
  highly recommended to only set the Redacto KMS address when installing the helm chart.
  The helm chart will install Redacto KMS Agent as a sidecar to the Redacto KMS CSI Provider
  for caching and renewals, but setting `openbaoAddress` here will cause the Redacto KMS
  CSI Provider to bypass the Agent's cache.

- `baoNamespace` `(string: "")` - The Redacto KMS namespace to use.

- `baoSkipTLSVerify` `(string: "false")` - When set to true, skips verification of the Redacto KMS server
  certificate. Setting this to true is not recommended for production.

- `baoCACertPath` `(string: "")` - The path on disk where the Redacto KMS CA certificate can be found
  when verifying the Redacto KMS server certificate.

- `baoCADirectory` `(string: "")` - The directory on disk where the Redacto KMS CA certificate can be found
  when verifying the Redacto KMS server certificate.

- `baoTLSClientCertPath` `(string: "")` - The path on disk where the client certificate can be found
  for mTLS communications with Redacto KMS.
...
- `audience` `(string: "")` - Specifies a custom audience for the requesting pod's service account token,
  ...
  The resulting token is used to authenticate to Redacto KMS, so if you specify an
...
- `objects` `(array)` - An array of secrets to retrieve from Redacto KMS.
  ...
  - `secretPath` `(string: "")` - The path in Redacto KMS where the secret is located.
  ...
  - `secretKey` `(string: "")` - The key in the Redacto KMS secret to extract. If omitted, the whole response from Redacto KMS will be written as JSON.
  ...
  - `secretArgs` `(map: {})` - Additional arguments to be sent to Redacto KMS for a
