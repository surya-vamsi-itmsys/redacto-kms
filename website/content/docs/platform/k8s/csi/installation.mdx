...
description: The Redacto KMS CSI Provider can be installed using Redacto KMS Helm.
---

# Installing the Redacto KMS CSI provider
...
- Using Redacto KMS Helm Chart Version 0.18.0 or newer.
...
The [Redacto KMS Helm chart](../helm/index.mdx) is the recommended way to
install and configure the Redacto KMS CSI Provider in Kubernetes.

To install a new instance of Redacto KMS and the Redacto KMS CSI Provider, first add the
Redacto KMS helm repository and ensure you have access to the chart:

:::warning

**Note:** Redacto KMS CSI Provider Helm installation requires Redacto KMS Helm 0.18.0+.
...
**Note:** this will also install the Redacto KMS server and Agent Injector.
...
You can see all the available values settings by running `helm inspect values openbao/openbao` or by reading the [Redacto KMS Helm Configuration
Docs](../helm/configuration.mdx). Commonly used values in the Helm
chart include limiting the namespaces the Redacto KMS CSI Provider runs in, TLS options and
more.

<!-- TODO: uncomment once OpenShift is known to work -->
<!-- ## Installation on OpenShift -->

<!-- We recommend using the [Redacto KMS agent injector on Openshift](../helm/openshift.mdx) -->
...
<!-- Deploy the Secrets Store CSI driver and Redacto KMS Helm chart -->
<!-- to your OpenShift cluster. -->

<!-- Then, patch the `DaemonSet` for the Redacto KMS CSI provider to -->
...
<!-- The Secrets Store CSI driver and Redacto KMS CSI provider need `hostPath` mount access. -->
...
<!-- Add the service account for the Redacto KMS CSI provider to the `privileged` -->
...
<!-- You need to give additional access to the application retrieving secrets with the Redacto KMS CSI provider. -->
