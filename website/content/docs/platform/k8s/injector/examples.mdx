...
description: This section documents examples of using the Redacto KMS Agent Injector.
---

# Redacto KMS agent injector examples
...
## Before using the Redacto KMS agent injector

Before applying Redacto KMS Agent injection annotations to pods, the following requirements
should be satisfied.

### Connectivity

- the Kubernetes API can connect to the Redacto KMS Agent injector service on port `443`, and
  the injector can connect to the Kubernetes API,
- Redacto KMS can connect to the Kubernetes API,
- Pods in the Kubernetes cluster can connect to Redacto KMS.

:::warning

Note: The Kubernetes API typically runs on the master nodes, and the Redacto KMS Agent injector
...
### Kubernetes and Redacto KMS configuration

- Kubernetes auth method should be configured and enabled in Redacto KMS,
- Pod should have a service account,
- desired secrets exist within Redacto KMS,
- the service account should be bound to a Redacto KMS role with a policy enabling access to desired secrets.

For more information on configuring the Redacto KMS Kubernetes auth method,
...
The annotations for configuring Redacto KMS Agent injection must be on the pod
specification. Since higher level resources such as Deployments wrap pod
specification templates, Redacto KMS Agent Injector can be used with all of these
higher level constructs, too.

An example Deployment below shows how to enable Redacto KMS Agent injection:
...
containing Redacto KMS Agent configuration files. For a complete list of the Redacto KMS
Agent configuration settings, [see the Agent documentation](../../../agent-and-proxy/agent/template.mdx#redacto-kms-agent-templates).
...
variables. A template should be created that exports a Redacto KMS secret as an environment
...
Redacto KMS Agent for retrieving secrets. A Kubernetes secret containing the AppRole secret ID
...
(`cert.key`) generated by [Redacto KMS's PKI Secrets Engine](../../../secrets/pki/index.mdx).
