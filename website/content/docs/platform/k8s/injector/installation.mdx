...
description: The Redacto KMS Agent Sidecar Injector can be installed using Redacto KMS Helm.
---
...
The Redacto KMS Agent Injector requires Redacto KMS 2.0.0 or greater.

:::

To install a new instance of Redacto KMS and the Redacto KMS Agent Injector, first add the
...
The Redacto KMS Agent Injector also supports two TLS management options:
...
By default, the Redacto KMS Agent Injector will bootstrap TLS by generating a certificate
authority and creating a certificate/key to be used by the controller. If using
Redacto KMS Helm, the chart will automatically create the necessary DNS entries for the
...
For more information on configuring manual TLS, see the [Redacto KMS Helm cert values](../helm/configuration.mdx#certs).
...
The Redacto KMS Agent Injector can be run with multiple replicas if using [Manual
...
[Auto TLS](#auto-tls). The number of replicas is controlled in the Redacto KMS Helm
...
By default, the Redacto KMS Agent Injector will process all namespaces in Kubernetes except
...
For more information on configuring namespace selection, see the [Redacto KMS Helm namespaceSelector value](../helm/configuration.mdx#namespaceselector).
