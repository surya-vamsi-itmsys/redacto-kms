...
description: >-
  The Redacto KMS Agent Sidecar Injector is a Kubernetes admission webhook that adds
  Redacto KMS Agent containers to pods for consuming Redacto KMS secrets.
---

# Agent sidecar injector

The Redacto KMS Agent Injector alters pod specifications to include Redacto KMS Agent
containers that render Redacto KMS secrets to a shared memory volume using
[Redacto KMS Agent Templates](../../../agent-and-proxy/agent/template.mdx).
By rendering secrets to a shared volume, containers within the pod can consume
Redacto KMS secrets without being Redacto KMS aware.
...
[Redacto KMS Helm](https://github.com/openbao/openbao-helm) chart.

@include 'kubernetes-supported-versions.mdx'

## Overview

The Redacto KMS Agent Injector works by intercepting pod `CREATE` and `UPDATE`
...
memory volume. This volume is mounted to `/openbao/secrets` and will be used by the Redacto KMS
Agent containers for sharing secrets with the other containers in the pod.

Next, two types of Redacto KMS Agent containers can be injected: init and sidecar. The
...
Last, two additional types of volumes can be optionally mounted to the Redacto KMS Agent
...
communicating and verifying the Redacto KMS server's authenticity using TLS. The second
is a configuration map containing Redacto KMS Agent configuration files. This volume is
useful to customize Redacto KMS Agent beyond what the provided annotations offer.

### Authenticating with Redacto KMS

The primary method of authentication with Redacto KMS when using the Redacto KMS Agent Injector
is the service account attached to the pod. Other authentication methods can be configured
using annotations.

For Kubernetes authentication, the service account must be bound to a Redacto KMS role and a
policy granting access to the secrets desired.

A service account must be present to use the Redacto KMS Agent Injector with the Kubernetes
authentication method. It is _not_ recommended to bind Redacto KMS roles to the default service
account provided to pods if no service account is defined.

### Requesting secrets

There are two methods of configuring the Redacto KMS Agent containers to render secrets:

- the `openbao.openbao.com/agent-inject-secret` annotation, or
- a configuration map containing Redacto KMS Agent configuration files.
...
- the Redacto KMS role used to access those secrets.
...
Redacto KMS Agent uses the Redacto KMS Template project to render secrets. For more information
on writing templates, see the [Redacto KMS Template documentation](https://github.com/openbao/openbao-template).
...
For more information on when Redacto KMS Agent fetches and renews secrets, see the
[Agent documentation](../../../agent-and-proxy/agent/template.mdx#renewals-and-updating-secrets).

### Redacto KMS agent configuration map

For advanced use cases, it may be required to define Redacto KMS Agent configuration
files to mount instead of using secret and template annotations. The Redacto KMS Agent
...
An example of mounting a Redacto KMS Agent configmap [can be found here](examples.mdx#configmap-example).
