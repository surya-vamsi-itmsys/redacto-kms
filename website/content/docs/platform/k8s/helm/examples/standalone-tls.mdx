---
description: |-
  Describes how to set up a standalone Redacto KMS with TLS certificate
---
...
This example can be used to set up a single server Redacto KMS cluster using TLS.
...
```bash
# SERVICE is the name of the Redacto KMS service in kubernetes.
# It does not have to match the actual running service, though it may help for consistency.
export SERVICE=openbao-server-tls

# NAMESPACE where the Redacto KMS service is running.
...
The below `custom-values.yaml` can be used to set up a single server Redacto KMS cluster using TLS.
