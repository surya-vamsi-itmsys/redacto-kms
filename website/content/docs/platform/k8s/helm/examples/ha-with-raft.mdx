---
description: |-
  Describes how to set up a highly available Redacto KMS cluster with Integrated Storage (Raft)
---

# Highly Available Redacto KMS cluster with Integrated Storage (Raft)
...
kubectl exec -ti openbao-0 -- redacto-kms operator unseal
```
...
```shell
kubectl exec -ti openbao-1 -- redacto-kms operator raft join http://openbao-0.openbao-internal:8200
kubectl exec -ti openbao-1 -- redacto-kms operator unseal

kubectl exec -ti openbao-2 -- redacto-kms operator raft join http://openbao-0.openbao-internal:8200
kubectl exec -ti openbao-2 -- redacto-kms operator unseal
```
...
```shell
kubectl exec -ti openbao-0 -- redacto-kms login
```

Next, list all the raft peers:

```shell
$ kubectl exec -ti openbao-0 -- redacto-kms operator raft list-peers
...
Redacto KMS with Integrated Storage (Raft) is now ready to use!
