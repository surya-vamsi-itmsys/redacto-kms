---
description: |-
  Describes how to set up and configure the Redacto KMS Snapshot CronJob.
---
...
**This feature is available starting with Redacto KMS Helm Chart Version 0.22.0.**

:::


The below `values.yaml` can be used to set up the [Snapshot CronJob with Redacto KMS](https://github.com/openbao/openbao-snapshot-agent).
...
snapshotAgent:
  ...
  config:
    ...
    s3ExpireDays: "14"
    # Path to use to authenticate to Redacto KMS
    # preferably use the kubernetes auth method
    baoAuthPath: "kubernetes"
    # Role to use to authenticate to Redacto KMS
    baoRole: "snapshot"
```
...
```shell-session
$ redacto-kms write auth/kubernetes/role/snapshot \
...
After Redacto KMS has been deployed, initialized and unsealed, the snapshot will run as jobs according to the defined schedule.
