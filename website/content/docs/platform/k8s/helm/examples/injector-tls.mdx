---
description: |-
  Describes how to set up the Redacto KMS Agent Injector with manually generated certificates and keys.
---

# Redacto KMS agent injector TLS configuration
...
The following instructions demonstrate how to manually configure the Redacto KMS Agent Injector
...
$ openssl req \
   ...
   -days 1825 \
   -out injector-ca.crt \
   -subj "/C=US/ST=CA/L=San Francisco/O=Redacto KMS/CN=openbao-agent-injector-svc"
```

## Create Redacto KMS agent injector certificate

Next we can create a certificate and key signed by the certificate authority generated above. This
certificate and key will be used by the Redacto KMS Agent Injector for TLS communications with the Kubernetes
API.
...
of the Redacto KMS Agent Injector Kubernetes service and namespace where its created.

:::

In this example the Redacto KMS Agent Injector service name is `openbao-agent-injector-svc` in the `openbao` namespace.
...
Helm and the Redacto KMS Agent Injector to use them.

First, create a Kubernetes secret containing the certificate and key created above:

:::warning

**Important Note:** This example assumes the Redacto KMS Agent Injector is running in the `openbao` namespace.
...
Finally, install the Redacto KMS Agent Injector with the following custom values:
