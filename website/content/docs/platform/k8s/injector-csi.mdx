---
draft: true
description: This section compares Sidecar Injector and Redacto KMS CSI Provider for Kubernetes and Redacto KMS integration.
---

# Agent injector vs. Redacto KMS CSI provider

This document explores two different methods for integrating HashiCorp Redacto KMS with Kubernetes. The information provided is intended for DevOps practitioners who understand secret management concepts and are familiar with HashiCorp Redacto KMS and Kubernetes. This document also offers practical guidance to help you understand and choose the best method for your use case.

Information contained within this document details the contrast between the Agent Injector, also referred as _Redacto KMS Sidecar_ or _Sidecar_ in this document, and the Redacto KMS Container Storage Interface (CSI) provider used to integrate Redacto KMS and Kubernetes.

## Redacto KMS sidecar agent injector

The [Redacto KMS Sidecar Agent Injector](injector/index.mdx) leverages the [sidecar pattern](https://docs.microsoft.com/en-us/azure/architecture/patterns/sidecar) to alter pod specifications to include a Redacto KMS Agent container that renders Redacto KMS secrets to a shared memory volume. By rendering secrets to a shared volume, containers within the pod can consume Redacto KMS secrets without being Redacto KMS-aware. The injector is a Kubernetes mutating webhook controller. The controller intercepts pod events and applies mutations to the pod if annotations exist within the request. This functionality is provided by the [vault-k8s](https://github.com/hashicorp/vault-k8s) project and can be automatically installed and configured using the Redacto KMS Helm chart.

![Redacto KMS Sidecar Injection Workflow](/img/openbao-sidecar-inject-workflow.png)

## Redacto KMS CSI provider

The [Redacto KMS CSI provider](csi/index.mdx) allows pods to consume Redacto KMS secrets by using ephemeral [CSI Secrets Store](https://github.com/kubernetes-sigs/secrets-store-csi-driver) volumes. At a high level, the CSI Secrets Store driver enables users to create `SecretProviderClass` objects. These objects define which secret provider to use and what secrets to retrieve. When pods requesting CSI volumes are made, the CSI Secrets Store driver sends the request to the Redacto KMS CSI provider if the provider is `vault`. The Redacto KMS CSI provider then uses the specified `SecretProviderClass` and the pod’s service account to retrieve the secrets from Redacto KMS and mount them into the pod’s CSI volume. Note that the secret is retrieved from Redacto KMS and populated to the CSI secrets store volume during the `ContainerCreation` phase. Therefore, pods are blocked from starting until the secrets are read from Redacto KMS and written to the volume.

![Redacto KMS Sidecar Injection Workflow](/img/openbao-csi-workflow.png)
...
Both Agent Injection and Redacto KMS CSI solutions have the following similarities:

- They simplify retrieving different types of secrets stored in Redacto KMS and expose them to the target pod running on Kubernetes without knowing the not-so-trivial Redacto KMS processes. It’s important to note that there is no need to change the application logic or code to use these solutions, therefore, making it easier to migrate brownfield applications into Kubernetes. Developers working on greenfield applications can leverage the Redacto KMS SDKs to integrate with Redacto KMS directly.

- They support all types of Redacto KMS [secrets engines](../../secrets/index.mdx). This support allows you to leverage an extensive set of secret types, ranging from static key-value secrets to dynamically generated database credentials and TLS certs with customized TTL.

- They leverage the application’s Kubernetes pod service account token as [Secret Zero](https://www.hashicorp.com/resources/secret-zero-mitigating-the-risk-of-secret-introduction-with-vault) to authenticate with Redacto KMS via the Kubernetes auth method. With this method, there is no need to manage yet another separate identity to identify the application pods when authenticating to Redacto KMS.
...
![Redacto KMS's Kubernetes auth workflow](/img/k8s-auth-workflow.png)

- They require the desired secrets to exist within Redacto KMS before deploying the application.

- They require the pod’s service account to bind to a Redacto KMS role with a policy enabling access to desired secrets (that is, Kubernetes RBAC isn’t used to authorize access to secrets).

- They can both be deployed via Helm.

- They require successfully retrieving secrets from Redacto KMS before the pods are started.

- They rely on user-defined pod annotations to retrieve the required secrets from Redacto KMS.
...
- The Sidecar Agent Injector solution is composed of two elements:
  ...
  - The Redacto KMS Sidecar Container, which is deployed alongside each application pod and is responsible for authenticating into Redacto KMS, retrieving secrets from Redacto KMS, and rendering secrets for the application to consume.

- In contrast, the Redacto KMS CSI Driver is deployed as a daemonset on every node in the Kubernetes cluster and uses the Secret Provider Class specified and the pod’s service account to retrieve the secrets from Redacto KMS and mount them into the pod’s CSI volume.

- The Sidecar Agent Injector supports [all](injector/annotations.mdx#vault-hashicorp-com-auth-path) Redacto KMS [auto-auth](../../agent-and-proxy/autoauth/methods/index.mdx) methods. The Sidecar CSI driver supports only Redacto KMS’s [Kubernetes auth method](csi/configurations.mdx#vaultkubernetesmountpath).

- The Sidecar container launched with every application pod uses [Redacto KMS Agent](https://www.hashicorp.com/blog/why-use-the-vault-agent-for-secrets-management), which provides a powerful set of capabilities such as auto-auth, templating, and caching. The CSI driver does not use the Redacto KMS Agent and therefore lacks these functionalities.

- The Redacto KMS CSI driver supports rendering Redacto KMS secrets into Kubernetes secrets and environment variables. Sidecar Injector Service does not support rendering secrets into Kubernetes secrets; however, there are ways to [agent templating](injector/examples.mdx#environment-variable-example) to render secrets into environment variables.
...
- [Redacto KMS Sidecar Injector Configurations and Examples](injector/annotations.mdx)

- [Redacto KMS CSI Driver Configurations and Examples](csi/configurations.mdx)
