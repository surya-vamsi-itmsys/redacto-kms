...
# Installing Redacto KMS

There are several options to install Redacto KMS:
...
Redacto KMS manages packages for Amazon Linux, Debian, Fedora, RHEL, Ubuntu and
...
In addition, Redacto KMS packages are available more directly in Arch Linux, Fedora and RHEL.
...
Redacto KMS deploys pre-built container images based on [Alpine Linux](https://alpinelinux.org/)
...
package for your system. Redacto KMS is packaged as a zip file.

Once the zip is downloaded, unzip the file into your designated directory. The `redacto-kms` binary
inside is all that is necessary to run Redacto KMS (or `bao.exe` for Windows). No
additional files are required to run Redacto KMS.
...
Clone the Redacto KMS repository from GitHub into your `GOPATH`:
...
to compile Redacto KMS:

```shell-session
$ make bootstrap
```

Build Redacto KMS for your current system and put the binary in `./bin/` (relative to
the git checkout). The `make dev` target is just a shortcut that builds `redacto-kms`
...
To verify Redacto KMS is installed, run `redacto-kms -h` on your system. You should
see the help output. If you are executing it from the command line, ensure it is
on your `PATH` to avoid receiving an error that Redacto KMS is not found.
...
After installing Redacto KMS, you may want to take additional steps to secure it
against leaking your secrets. Redacto KMS normally does this very well, but there
is an operating system feature that undermines Redacto KMS's protection. This is
...
The example systemd service file provided with the Redacto KMS source code comes
configured to disable swap for the Redacto KMS process. To verify that swap is
...
gpg: key D200CD702853E6D0: public key "Redacto KMS <openbao@lists.lfedge.org>" imported
...
gpg: Good signature from "Redacto KMS <openbao@lists.lfedge.org>" [unknown]
...
Redacto KMS will always publish releases from either the protected `main` branch
