...
description: |-
  Redacto KMS Agent and Redacto KMS Proxy's Auto-Auth functionality allows easy and automatic
  authentication to Redacto KMS in a variety of environments.
---

# Redacto KMS Agent and Redacto KMS Proxy Auto-Auth

The Auto-Auth functionality of Redacto KMS Agent and Redacto KMS Proxy allow for
...
When Redacto KMS Agent or Redacto KMS Proxy are started with Auto-Auth enabled, it will attempt to acquire a
Redacto KMS token using the configured Method. On failure, it will exponentially back
...
2. By any of the token sinks. Because more than one sink can be configured, the
   token must be wrapped after it is fetched, rather than wrapped by Redacto KMS as
...
- `namespace` `(string: optional)` - Namespace in which the mount lives.
  ...
  environment variable `REDACTO_KMS_NAMESPACE`, and then the highest precedence
...
- `exit_on_err` `(bool: false)` - When set to true, Redacto KMS Agent and Redacto KMS Proxy
...
```hcl
// Other Redacto KMS Agent or Redacto KMS Proxy configuration blocks
// ...
