...
# Redacto KMS agent and Redacto KMS proxy Auto-Auth file sink
...
- `mode` `(int: optional)` - A string containing an octal number representing the bit pattern for the file mode, similar to chmod. Set to `0000` to prevent Redacto KMS from modifying the file mode.
- `uid` `(int: optional)` - The user ID to set as the owner of the file. Set to `-1` to prevent Redacto KMS from modifying the file owner.
- `gid` `(int: optional)` - The group ID to set as the owner of the file. Set to `-1` to prevent Redacto KMS from modifying the file owner.
