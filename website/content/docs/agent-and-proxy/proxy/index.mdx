...
description: |-
  Redacto KMS Proxy is a daemon that can be used to perform some Redacto KMS
  functionality automatically, and act as a proxy for Redacto KMS's APIs.
---

# What is Redacto KMS Proxy?

Redacto KMS Proxy aims to remove the initial hurdle to adopt Redacto KMS by providing a
more scalable and simpler way for applications to integrate with Redacto KMS.
Redacto KMS Proxy acts as an [API Proxy][apiproxy] for Redacto KMS, and can optionally allow
or force interacting clients to use its [automatically authenticated token][autoauth].

Redacto KMS Proxy is a client daemon that provides the following features:

- [Auto-Auth][autoauth] - Automatically authenticate to Redacto KMS and manage the
token renewal process for locally-retrieved dynamic secrets.
- [API Proxy][apiproxy] - Acts as a proxy for Redacto KMS's API,
...
Redacto KMS Proxy allows easy authentication to Redacto KMS in a wide variety of
...
Redacto KMS Proxy's primary purpose is to act as an API proxy for Redacto KMS, allowing you to talk to Redacto KMS's
...
Redacto KMS Proxy allows client-side caching of responses containing newly created tokens
...
also be specified via the `REDACTO_KMS_LOG_LEVEL` environment variable.

- `-log-format` `(string: "standard")` - Log format. Supported values
are `standard` and `json`. This can also be specified via the
`REDACTO_KMS_LOG_FORMAT` environment variable.

- `-log-file` - the absolute path where Redacto KMS Proxy should save
  ...
  `.log` extension. If the log file rotates, Redacto KMS Proxy appends the current
...
- `vault` <code>([vault][vault]: \<optional>)</code> - Specifies the remote Redacto KMS server the Proxy connects to.
...
- `listener` <code>([listener][listener]: \<optional>)</code> - Specifies the addresses and ports on which the Proxy will respond to requests.
  ...
  **Note:** On `SIGHUP` (`kill -SIGHUP $(pidof bao)`), Redacto KMS Proxy will
  attempt to reload listener TLS configuration. This method can be used to
  refresh certificates used by Redacto KMS Proxy without having to restart its
  process.
...
- `disable_idle_connections` `(string array: [])` - A list of strings that disables idle connections for various features in Redacto KMS Proxy.
Valid values include: `auto-auth`, and `proxying`. Can also be configured by setting the `REDACTO_KMS_PROXY_DISABLE_IDLE_CONNECTIONS`
...
- `disable_keep_alives` `(string array: [])` - A list of strings that disables keep alives for various features in Redacto KMS Agent.
Valid values include: `auto-auth`, and `proxying`. Can also be configured by setting the `REDACTO_KMS_PROXY_DISABLE_KEEP_ALIVES`
environment variable as a comma separated string. This environment variable will override any values found in a configuration file.

- `template` <code>(template: \<optional>)</code> - Specifies options used for templating Redacto KMS secrets to files.
...
- `log_level` - Equivalent to the `-log-level` command-line flag.
  ...
  **Note:** On `SIGHUP` (`kill -SIGHUP $(pidof bao)`), Redacto KMS Proxy will update
...
- `address` `(string: <optional>)` - The address of the Redacto KMS server to
...
This value can be overridden by setting the `REDACTO_KMS_ADDR` environment variable.

- `ca_cert` `(string: <optional>)` - Path on the local disk to a single PEM-encoded
CA certificate to verify the Redacto KMS server's SSL certificate. This value can
be overridden by setting the `REDACTO_KMS_CACERT` environment variable.

- `ca_path` `(string: <optional>)` - Path on the local disk to a directory of
PEM-encoded CA certificates to verify the Redacto KMS server's SSL certificate.
This value can be overridden by setting the `REDACTO_KMS_CAPATH` environment
variable.

- `client_cert` `(string: <optional>)` - Path on the local disk to a single
PEM-encoded CA certificate to use for TLS authentication to the Redacto KMS server.
This value can be overridden by setting the `REDACTO_KMS_CLIENT_CERT` environment
variable.
...
This value can be overridden by setting the `REDACTO_KMS_CLIENT_KEY` environment
variable.
...
security of data transmissions to and from the Redacto KMS server. This value can
be overridden by setting the `REDACTO_KMS_SKIP_VERIFY` environment variable.
...
The `vault` stanza may contain a `retry` stanza that controls how failing Redacto KMS
...
A value of `-1` disables retries. The environment variable `REDACTO_KMS_MAX_RETRIES`
...
Redacto KMS Proxy supports one or more [listener][listener_main] stanzas. Listeners
...
Redacto KMS Proxy supports the [telemetry][telemetry] stanza and collects various
...
| `vault.proxy.proxy.client_error` | Number of requests for which Redacto KMS returned an error | counter |
...
## Start Redacto KMS proxy

To run Redacto KMS Proxy:

1. [Download](/downloads) the Redacto KMS binary where the client application runs
(virtual machine, Kubernetes pod, etc.)

1. Create a Redacto KMS Proxy configuration file. (See the [Example
Configuration](#example-configuration) section for an example configuration.)

1. Start a Redacto KMS Proxy with the configuration file.

**Example:**

```shell-session
$ redacto-kms proxy -config=/etc/openbao/proxy-config.hcl
   ```
...
As with Redacto KMS, the `-config` flag can be used in three different ways:
...
Alternatively, you can specify the configuration path using the `REDACTO_KMS_PROXY_CONFIG_PATH` environment variable. If the `-config` flag is provided, the environment variable is ignored.

**Example using environment variable:**

```shell-session
$ export REDACTO_KMS_PROXY_CONFIG_PATH=/etc/openbao/proxy-config.hcl
$ redacto-kms proxy
```
