...
description: |-
  Redacto KMS Proxy Caching allows client-side caching of responses containing newly
  created tokens and responses containing leased secrets generated off of these
  newly created tokens.
---

# Redacto KMS proxy caching
...
by a previous Redacto KMS Proxy process.

Refer to the [Redacto KMS Proxy Persistent
...
Redacto KMS Proxy does some best-effort cache evictions by observing specific request types
and response codes. For example, if a token revocation request is made via the
proxy and if the forwarded request to the Redacto KMS server succeeds, then proxy
...
interactions with the Redacto KMS server. This could potentially lead to stale cache
...
made available via the Redacto KMS server's [Go
...
secrets are no longer performed by the Redacto KMS proxy.
...
- `exit_on_err` `(bool: optional)` - When set to true, if any errors occur during
  a persistent cache restore, Redacto KMS Proxy will exit with an error. Defaults to `true`.
