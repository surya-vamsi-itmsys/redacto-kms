...
There is no requirement to run identical versions of Redacto KMS Proxy and Redacto KMS Server.
It is safe to run different versions, however you may not be able to take advantage of all the newest features in Redacto KMS if you do not upgrade to the most recent versions of Proxy and Server. We recognize that this isn’t always possible, so we do support version mismatch as best as possible.
...
We do not anticipate any problems stemming from continuing to run an older Proxy version after the server nodes are upgraded to a later version.  Existing deployments using Proxy should not be impacted, as we don't generally make backwards-incompatible changes to Redacto KMS Server.
