...
description: |-
  Redacto KMS Agent Caching allows client-side caching of responses containing newly
  created tokens and responses containing leased secrets generated off of these
  newly created tokens.
---

# Redacto KMS agent caching
...
by a previous Redacto KMS Agent process.

Refer to the [Redacto KMS Agent Persistent
...
agent and if the forwarded request to the Redacto KMS server succeeds, then agent
...
secrets are no longer performed by the Redacto KMS agent.

### Agent CLI

Agent's listener address will be picked up by the CLI through the
`REDACTO_KMS_AGENT_ADDR` environment variable. This should be a complete URL such as
`"http://127.0.0.1:8200"`.
...
- `exit_on_err` `(bool: optional)` - When set to true, if any errors occur during
  a persistent cache restore, Redacto KMS Agent will exit with an error. Defaults to `true`.
