...
There is no requirement to run identical versions of Redacto KMS Agent and Redacto KMS Server.
It is safe to run different versions, however you may not be able to take advantage of all the newest features in Redacto KMS if you do not upgrade to the most recent versions of Agent and Server. We recognize that this isn’t always possible, so we do support version mismatch as best as possible.
...
We do not anticipate any problems stemming from continuing to run an older Agent version after the server nodes are upgraded to a later version.  Existing deployments using Agent should not be impacted, as we don't generally make backwards-incompatible changes to Redacto KMS Server.
...
Templating:
- the templating language features that interact with the Redacto KMS server use stable Redacto KMS APIs to retrieve and renew secrets
- even if new secret engine types are introduced in newer Redacto KMS releases, these should not require an Agent upgrade to access via templates
...
Templating:
- we don't anticipate a scenario where changes to Agent's templating itself gives rise to an incompatibility with older Redacto KMS Servers, though of course with any Agent version it's possible to write templates that issue requests which make use of functionality not yet present in the upstream Redacto KMS server, e.g. \{\{ with secret "secret/my-secret?some-new-option" }}
