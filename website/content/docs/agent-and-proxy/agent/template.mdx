...
description: >-
  Redacto KMS Agent's Template functionality allows Redacto KMS secrets to be rendered to
  files using Consul Template markup.
---

# Redacto KMS agent templates

Redacto KMS Agent's Template functionality allows Redacto KMS secrets to be rendered to files
...
The `template` stanza configures the Redacto KMS agent for rendering secrets to files
...
Redacto KMS token using the configured auto-auth Method. On failure, it will back off
...
retrieved from Redacto KMS and rendered locally.
...
In order to fetch secrets from Redacto KMS, whether those are static secrets, dynamic
credentials, or certificates, Redacto KMS Agent templates require the use of the
`secret`
...
The following links contain additional resources for the templating language used by Redacto KMS Agent templating.
...
The following is an example of a template that retrieves a generic secret from Redacto KMS's
KV store:
```
...
Redacto KMS's PKI secrets engine. The fetching of the certificate or key from a PKI role
...
- `exit_on_retry_failure` `(bool: false)` - This option configures Redacto KMS Agent
...
- `static_secret_render_interval` `(string or integer: 5m)` <a name="static_secret_render_interval"></a> - If specified, configures
  how often Redacto KMS Agent Template should render non-leased secrets such as KV v2.
  This setting will not change how often Redacto KMS Agent Templating renders leased
...
Whether Redacto KMS Agent will exit when the templating engine errors depends on the
...
- `destination`Δ `(string: required)` - Path on disk where the rendered secrets should
  be created. If the parent directories do not exist, Redacto KMS
  Agent will attempt to create them, unless `create_dest_dirs` is false.
- `create_dest_dirs`Δ `(bool: true)` - This option tells Redacto KMS Agent to create
...
- `command`Δ `(string: "")` - This is the optional command to run when the
  ...
  exit code. Redacto KMS Agent is not a replacement for a process monitor or init system.
...
- `error_on_missing_key` `(bool: false)` <a name="error_on_missing_key"></a> - Exit with an error when accessing
  ...
  to "true". Also see [`exit_on_retry_failure` in global Redacto KMS Agent Template Config](template.mdx#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
...
- `perms`Δ `(string: "")` - This is the permission to render the file. If
  this option is left unspecified, Redacto KMS Agent will attempt to match the permissions
...
If you only want to use the Redacto KMS agent to render one or more templates and do
...
The Redacto KMS Agent templating automatically renews and fetches secrets/tokens.
Unlike [Redacto KMS Agent caching](caching/index.mdx), the behavior of how Redacto KMS Agent
templating does this depends on the type of secret or token. The following is a
high level overview of different behaviors.

### Renewable secrets

If a secret or token is renewable, Redacto KMS Agent will renew the secret after 2/3
of the secret's lease duration has elapsed.

### Non-Renewable secrets

If a secret or token isn't renewable or leased, Redacto KMS Agent will fetch the secret every 5 minutes.
...
If a secret or token is non-renewable but leased, Redacto KMS Agent will fetch the secret when 85% of the
...
Redacto KMS Agent template will fetch the new secret as it changes in Redacto KMS. It does
...
function, Redacto KMS Agent template will have the following fetching and re-rendering
...
The following demonstrates Redacto KMS Agent Templates configuration blocks.
