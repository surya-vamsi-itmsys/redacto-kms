...
description: >-
  Redacto KMS Agent's Process Supervisor Mode allows Redacto KMS secrets to be injected
  into a process via environment variables using Consul Template markup.
---

# Redacto KMS agent's process supervisor mode

Redacto KMS Agent's Process Supervisor Mode allows Redacto KMS secrets to be injected into
...
<!--   evaluating the [Redacto KMS Agent Sidecar Injector](../../platform/k8s/injector/index.mdx). -->

<!-- ::: -->

:::danger

Redacto KMS Agent's Process Supervisor Mode is in public beta. Please provide your
...
Redacto KMS Agent will inject secrets referenced in the `env_template` configuration
blocks as environment variables into the child process specified in the `exec` block.

When you start Redacto KMS Agent in process supervisor mode, it will wait until each
...
In many ways, Redacto KMS Agent will mirror the child process. Standard intput and
output streams (`stdin` / `stdout` / `stderr`) are all forwarded to the child
process. Additionally, Redacto KMS Agent will exit when the child process exits on
...
- `error_on_missing_key` `(bool: false)` - Exit with an error when accessing
  ...
  to "true". Also see
  [`exit_on_retry_failure` in global Redacto KMS Agent Template Config](template.mdx#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
...
[`redacto-kms agent generate-config`](generate-config/index.mdx),
a configuration helper tool. Given this configuration, Redacto KMS Agent will run
the child process (`./my-app arg1 arg2`) with two additional environment
variables (`FOO_USER` and `FOO_PASSWORD`) populated with secrets from Redacto KMS.
...
auto_auth {
  ...
  method {
    ...
    config {
      token_file_path = "/Users/avean/.redacto-kms-token"
    }
  }
}
