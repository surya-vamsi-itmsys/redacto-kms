...
description: >-
  Redacto KMS server configuration reference for declarative plugin parameters.
---
...
Redacto KMS supports the distribution of plugins in Open Container Initiative (OCI)
artifacts. Operators can define the desired plugins for their Redacto KMS cluster
directly in the server configuration files. On startup, Redacto KMS will check for
...
managing the plugin binaries:
  ...
  * **Automated Discovery and Installation:** Operators can define the used
    plugins in code and Redacto KMS will automatically download and extract them
    during startup.
...
`type` must be one of the pre-defined plugin types available in Redacto KMS, see
...
- `sha256sum` `(string: optional)` - Expected SHA-256 digest of the plugin
  ...
  image digest, and **optional** otherwise. When set, Redacto KMS will revalidate
...
- `binary_name` `(string: optional)` - Name of the plugin binary file
  within the OCI image. Only applies when `image` is set. If unset, Redacto KMS
...
- `true` - server startup and `SIGHUP` triggers downloading of plugins; can still be manually downloaded with `redacto-kms plugin init`.
- `false` (default) - only `redacto-kms plugin init` will manually download plugins.
...
- `true` (default) - server startup and `SIGHUP` triggers registering of plugins; can still be manually registered with `redacto-kms plugin register`.
- `false` - only `redacto-kms plugin register` will manually register plugins.
...
When downloading plugin images from a private registry, Redacto KMS will use
...
When deploying Redacto KMS to Kubernetes you can mount an Image Pull Secret into the
...
Redacto KMS requires the plugin binary to be present in the image's root directory.
