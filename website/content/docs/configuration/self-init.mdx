...
description: |-
  The initialize stanza allows request-based one-time initialization of Redacto KMS
...
The `initialize` stanza specifies various configurations for Redacto KMS to
...
starting Redacto KMS with the `TRACE` log level, which shows all request/response
...
- `path` `(value[string]: <required>)` - path to perform the given operation
  on.
- `token` `(value[string]: <optional>)` - the Redacto KMS API token to use for
