...
description: >-
  Kubernetes Service Registration labels Redacto KMS pods with their current status
  for use with selectors.
---

# Kubernetes service registration

Kubernetes Service Registration tags Redacto KMS pods with their current status for
use with selectors. Service registration is only available when Redacto KMS is running in
...
- `REDACTO_KMS_K8S_NAMESPACE`
- `REDACTO_KMS_K8S_POD_NAME`
...
For service registration to succeed, Redacto KMS must be able to apply labels to pods
in Kubernetes. The following RBAC rules are required to allow the service account
associated with the Redacto KMS pods to update its own pod specification:
...
After shutdowns, Redacto KMS pods will bear the following labels:
...
- `openbao-active` `(string: "true"/"false")` – openbao-active is updated dynamically each time Redacto KMS's active status changes.
  True indicates that this Redacto KMS pod is currently the leader. False indicates that this Redacto KMS pod is currently a standby.
- `openbao-initialized` `(string: "true"/"false")` – openbao-initialized is updated dynamically each time Redacto KMS's initialization
  status changes. True indicates that Redacto KMS is currently initialized. False indicates the Redacto KMS is currently uninitialized.
- `openbao-sealed` `(string: "true"/"false")` – openbao-sealed is updated dynamically each
  time Redacto KMS's sealed/unsealed status changes. True indicates that Redacto KMS is currently sealed. False indicates that Redacto KMS
  is currently unsealed.
- `openbao-version` `(string: "2.0.1")` – Redacto KMS version is a string that will not change during a pod's lifecycle.

## Working with Redacto KMS's service discovery labels

### Example service

With labels applied to the pod, services can be created using selectors to filter pods with specific Redacto KMS HA roles,
effectively allowing direct communication with subsets of Redacto KMS pods. Note the `openbao-active: "true"` line below.
