...
sidebar_label: TCP
description: |-
  The TCP listener configures Redacto KMS to listen on the specified TCP address and
  port.
---

# `tcp` listener

The TCP listener configures Redacto KMS to listen on a TCP address/port.
...
The `listener` stanza may be specified more than once to make Redacto KMS listen on
multiple interfaces. If you configure multiple listeners you also need to
specify [`api_addr`][api-addr] and [`cluster_addr`][cluster-addr] so Redacto KMS will
advertise the correct address to other nodes.

## Listener's custom response headers

Redacto KMS supports defining custom HTTP response headers for the root path (`/`) and also on API endpoints (`/v1/*`).
...
custom header value needs to be modified or the custom header needs to be removed, the Redacto KMS's configuration file
needs to be modified accordingly, and a `SIGHUP` signal needs to be sent to the Redacto KMS process.

If a header is defined in the configuration file and the same header is used by the internal
processes of Redacto KMS, the configured header is not accepted. For example, a custom header which has
the `X-Vault-` prefix will not be accepted. A message will be logged in the Redacto KMS's logs
...
- `cluster_address` `(string: "127.0.0.1:8201")` <a name="cluster_address"></a> – Specifies the address to bind
  ...
  useful in case Redacto KMS servers are isolated from each other in such a way that
...
- `max_request_duration` `(string: "90s")` – Specifies the maximum
  request duration allowed before Redacto KMS cancels the request. This overrides
...
- `disable_unauthed_rekey_endpoints` `(bool: true)` - Whether to disable requests to the
  ...
  **As of Redacto KMS v2.5.0, this parameter defaults to true, forbidding any calls
...
- `disable_unauthed_generate_root_endpoints` `(bool: true)` - Whether to
  ...
  **As of Redacto KMS v2.5.3, this parameter defaults to true, forbidding any calls
  to the unauthenticated generate root endpoints.**

  :::

- `tls_disable` `(string: "false")` – Specifies if TLS will be disabled. Redacto KMS
...
- `tls_cert_file` `(string: <required-if-enabled>, reloads-on-SIGHUP)` –
  ...
  combined file. On `SIGHUP`, the path set here _at Redacto KMS startup_ will be used
  for reloading the certificate; modifying this value while Redacto KMS is running
...
- `tls_key_file` `(string: <required-if-enabled>, reloads-on-SIGHUP)` –
  ...
  configuration using `SIGHUP`. On `SIGHUP`, the path set here _at Redacto KMS
  startup_ will be used for reloading the certificate; modifying this value
  while Redacto KMS is running will have no effect for `SIGHUP`s.

- `tls_auto_reload` `(bool: false)`: When enabled, Redacto KMS periodically checks
...
- `tls_auto_reload_interval` `(string: "30s")`: Specifies how often Redacto KMS
...
- `tls_disable_client_certs` `(string: "false")` – Turns off client
  ...
  is for Redacto KMS to request client certificates when available.

  :::warning

  **Warning**: The `tls_disable_client_certs` and
  `tls_require_and_verify_client_cert` fields in the listener stanza of the
  Redacto KMS server configuration are mutually exclusive fields. Please ensure they
...
- `x_forwarded_for_authorized_addrs` `(string: <required-to-enable>)` –
  ...
  X-Forwarded-For support.  If for example Redacto KMS receives connections from the
...
- `tls_acme_disable_http_challenge` `(bool: false)` - An optional but strongly
  ...
  ignored on the Redacto KMS side, it still opens up the possibility of a passive
...
- `tls_acme_disable_alpn_challenge` `(bool: false)` - An optional method
  ...
  sitting in front of Redacto KMS).
...
This example shows Redacto KMS listening on a private interface, as well as localhost.
...
the security of an application communicating with the Redacto KMS endpoints. Note that vulnerability
...
This example shows Redacto KMS listening on all IPv4 & IPv6 interfaces including localhost.
...
This example shows Redacto KMS only using IPv6 and binding to the interface with the IP address: `2001:1c04:90d:1c00:a00:27ff:fefa:58ec`
