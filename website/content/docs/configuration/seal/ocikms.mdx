...
description: |-
  The OCI KMS seal configures Redacto KMS to use OCI KMS as the seal wrapping
  mechanism.
---
...
The OCI KMS seal configures Redacto KMS to use OCI KMS as the seal wrapping mechanism.
The OCI KMS seal is activated by one of the following:

- The presence of a `seal "ocikms"` block in Redacto KMS's configuration file
- The presence of the environment variable `REDACTO_KMS_SEAL_TYPE` set to `ocikms`. If
...
This example shows configuring the OCI KMS seal through the Redacto KMS configuration file
...
- `auth_type_api_key` `(boolean: false)`: Specifies if using API key to authenticate to OCI KMS service.
  If it is `false`, Redacto KMS authenticates using the instance principal from the compute instance. See Authentication section for details. Default is `false`.
...
For the [OCI KMS key rotation feature][oci-kms-rotation], OCI KMS will create a new version of key internally. This process is independent from Redacto KMS, and Redacto KMS still uses the same `key_id` without any interruption.
