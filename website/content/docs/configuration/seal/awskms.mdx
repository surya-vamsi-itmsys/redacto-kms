...
description: |-
  The AWS KMS seal configures Redacto KMS to use AWS KMS as the seal wrapping
  mechanism.
---
...
The AWS KMS seal configures Redacto KMS to use AWS KMS as the seal wrapping mechanism.
The AWS KMS seal is activated by one of the following:

- The presence of a `seal "awskms"` block in Redacto KMS's configuration file
- The presence of the environment variable `REDACTO_KMS_SEAL_TYPE` set to `awskms`. If
...
This example shows configuring AWS KMS seal through the Redacto KMS configuration file
...
- `endpoint` `(string: "")`: The KMS API endpoint to be used to make AWS KMS
  ...
  If not set, Redacto KMS will use the default API endpoint for your region.
...
Redacto KMS needs the following permissions on the KMS key:

- `kms:Encrypt`
- `kms:Decrypt`
- `kms:DescribeKey`

These can be granted via IAM permissions on the principal that Redacto KMS uses, on
...
Redacto KMS Seal specific values:
