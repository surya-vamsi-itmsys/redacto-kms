...
description: |-
  The static key seal configures Redacto KMS to use static keys provided alongside
...
Redacto KMS's static key unseal support is activated by the following:

- The presence of a `seal "static"` block in Redacto KMS's configuration file.
...
exists in the operating environment and it is desirable to chain Redacto KMS to
...
This example shows configuring static seal through the Redacto KMS configuration
...
These parameters apply to the `static` stanza in the Redacto KMS configuration
file:
...
- `disabled` `(string: "")`: Set this to `true` if Redacto KMS is migrating from
