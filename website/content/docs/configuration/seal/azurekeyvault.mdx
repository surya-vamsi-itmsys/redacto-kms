...
description: >-
  The Azure Key Vault seal configures Redacto KMS to use Azure Key Vault as the seal
  wrapping
...
- The presence of a `seal "azurekeyvault"` block in Redacto KMS's configuration file.
- The presence of the environment variable `REDACTO_KMS_SEAL_TYPE` set to `azurekeyvault`.
...
This example shows configuring Azure Key Vault seal through the Redacto KMS
...
**Note:** If Redacto KMS is hosted on Azure, Redacto KMS can use Managed Service
...
on the VMs hosting Redacto KMS, and it is the preferred configuration since MSI
...
using Azure Automation Account and Redacto KMS will recognize newly rotated keys.
