...
description: |-
  The KMIP seal configures Redacto KMS to use the KMIP protocol as the
...
The KMIP seal configures Redacto KMS to use a KMS or an HSM through the KMIP protocol as the autounseal
mechanism. Redacto KMS's KMIP support is activated by one of the following:

- The presence of a `seal "kmip"` block in Redacto KMS's configuration file
- The presence of the environment variable `REDACTO_KMS_SEAL_TYPE` set to `kmip`. If enabling via environment
  variable, all other required values (i.e. `REDACTO_KMS_KMIP_ENDPOINT`) must be also
  supplied.

:::info

Redacto KMS requires key material to be created externally
before initializing the instance. Redacto KMS will not attempt to create its own
key material.
...
This example shows configuring KMIP seal through the Redacto KMS configuration
...
- `kms_key_id` `(string: <required>)`: The wrapping key ID. Must be a symmetric AES key or a private RSA key).
  May also be specified by the `REDACTO_KMS_KMIP_WRAPPER_KEY_ID` environment variable.

- `endpoint` `(string: <required>)`: The KMIP endpoitn address and port. For example `my-kms.example.org:5696`.
  May also be specified by the `REDACTO_KMS_KMIP_ENDPOINT` environment variable.

- `client_cert` `(string: <required>)`: The client authentication TLS certificate.
  May also be specified by the `REDACTO_KMS_KMIP_CLIENT_CERT` environment variable.

- `client_key` `(string: <required>)`: The client TLS private key.
  May also be specified by the `REDACTO_KMS_KMIP_CLIENT_KEY` environment variable.

- `ca_cert` `(string: "")`: The server CA certificate.
  May also be specified by the `REDACTO_KMS_KMIP_CA_CERT` environment variable.

- `server_name` `(string: "")`: Optional server name,
  used to verify the hostname on the returned certificates.
  May also be specified by the `REDACTO_KMS_KMIP_SERVER_NAME`
  environment variable.

- `timeout` `(integer: 10)`: KMIP operation timeout in seconds.
  May also be specified by the `REDACTO_KMS_KMIP_TIMEOUT` environment variable.

- `encrypt_alg` `(string: "AES_GCM")`: The encryption/decryption algorithm to use.
  May also be specified by the `REDACTO_KMS_KMIP_ENCRYPT_ALG` environment variable.
...
- `tls12_ciphers` `(string: "")`: Comma separated list of TLS 1.2 ciphers to allow.
  ...
  May also be specified by the `REDACTO_KMS_KMIP_TLS12_CIPHERS` environment variable.
...
```text
REDACTO_KMS_SEAL_TYPE
REDACTO_KMS_KMIP_WRAPPER_KEY_ID
REDACTO_KMS_KMIP_ENDPOINT
REDACTO_KMS_KMIP_CA_CERT
REDACTO_KMS_KMIP_CLIENT_CERT
REDACTO_KMS_KMIP_CLIENT_KEY
REDACTO_KMS_KMIP_SERVER_NAME
REDACTO_KMS_KMIP_TIMEOUT
REDACTO_KMS_KMIP_ENCRYPT_ALG
REDACTO_KMS_KMIP_TLS12_CIPHERS
```

## Key rotation

This seal supports rotating keys by using different key IDs to track key versions. To rotate
the key value, create a new key or perform a rotation in the KMIP server and update Redacto KMS's
configuration with the new key ID.
The key type and encryption algorithm can also be changed as Redacto KMS keeps track of the algorithm.
Restart your Redacto KMS instance to pick up the new keyID and all new encryption operations will use
