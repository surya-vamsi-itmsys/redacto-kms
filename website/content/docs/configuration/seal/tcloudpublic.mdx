...
description: |-
  The T Cloud Public KMS seal configures Redacto KMS to use T Cloud Public KMS as
...
The T Cloud Public KMS seal configures Redacto KMS to use T Cloud Public KMS as the
seal wrapping mechanism. The T Cloud Public KMS seal is activated by one of the
following:

- The presence of a `seal "tcloudpublic"` block in Redacto KMS's configuration
  file.
- The presence of the environment variable `REDACTO_KMS_SEAL_TYPE` set to
...
Redacto KMS configuration file:
...
Redacto KMS needs the following permission on the KMS key:

- `kms:cmk:crypto`

This can be granted via IAM permissions at the project level or with Enterprise
Project Service on the principal that Redacto KMS uses for T Cloud Public KMS.
