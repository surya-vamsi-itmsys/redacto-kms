...
description: Redacto KMS server configuration reference.
---
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Redacto KMS configuration

Outside of development mode, Redacto KMS servers are configured using a file.
...
After the configuration is written, use the `-config` flag with `redacto-kms server`
...
- `storage` <code>([StorageBackend][storage-backend]: &lt;required&gt;)</code> <a name="storage"></a> –
  Configures the storage backend where Redacto KMS data is stored. Please see the
  [storage backends documentation][storage-backend] for the full list of
  available storage backends. Running Redacto KMS in HA mode would require
...
- `ha_storage` <code>([StorageBackend][storage-backend]: nil)</code> <a name="ha_storage"></a> – Configures
  the storage backend where Redacto KMS HA coordination will take place. This must be
...
- `listener` <code>([Listener][listener]: &lt;required&gt;)</code> – Configures how
  Redacto KMS is listening for API requests.
...
- `cluster_name` `(string: <generated>)` – Specifies the identifier for the
  Redacto KMS cluster. If omitted, Redacto KMS will generate a value.
...
- `disable_cache` `(bool: false)` – Disables all caches within Redacto KMS, including
...
- `plugin_directory` `(string: "")` – A directory from which plugins are
  allowed to be loaded. Redacto KMS must have permission to read files in this
...
- `plugin_file_uid` `(integer: 0)` – Uid of the plugin directories and plugin binaries if they
  are owned by an user other than the user running Redacto KMS. This only needs to be set if the
  file permissions check is enabled via the environment variable `REDACTO_KMS_ENABLE_FILE_PERMISSIONS_CHECK`.
...
- `plugin_file_permissions` `(string: "")` – Octal permission string of the plugin
  ...
  `REDACTO_KMS_ENABLE_FILE_PERMISSIONS_CHECK`.
...
- `default_max_request_duration` `(string: "90s")` <a name="default_max_request_duration"></a> – Specifies the default
  maximum request duration allowed before Redacto KMS cancels the request. This can
...
- `introspection_endpoint` `(bool: false)` - Enables the `sys/internal/inspect` endpoint
  which allows users with a root token or sudo privileges to inspect certain subsystems inside Redacto KMS.
...
- `ui` `(bool: false)` <a name="ui"></a> – Enables the built-in web UI, which is available on all
  ...
  Redacto KMS API address will automatically redirect there. This can also be provided
  via the environment variable `REDACTO_KMS_UI`. For more information, please see the
  [ui configuration documentation](ui.mdx).

- `pid_file` `(string: "")` - Path to the file in which the Redacto KMS server's
  Process ID (PID) should be stored.

- `enable_response_header_hostname` `(bool: false)` - Enables the addition of an HTTP header
  in all of Redacto KMS's HTTP responses: `X-Vault-Hostname`. This will contain the
  host name of the Redacto KMS node that serviced the HTTP request. This information
...
- `enable_response_header_raft_node_id` `(bool: false)` - Enables the addition of an HTTP header
  in all of Redacto KMS's HTTP responses: `X-Vault-Raft-Node-ID`. If Redacto KMS is participating
  in a Raft cluster (i.e. using integrated Storage), this header will contain the
  Raft node ID of the Redacto KMS node that serviced the HTTP request. If Redacto KMS is not
...
- `log_level` `(string: "info")` - Log verbosity level.
  ...
  This can also be specified via the `REDACTO_KMS_LOG_LEVEL` environment variable.

  :::warning

  Note: On SIGHUP (`sudo kill -s HUP` _pid of bao_), if a valid value is
  specified, Redacto KMS will update the existing log level, overriding (even if
  ...
  Note: Not all parts of Redacto KMS's logging can have its log level be changed
...
- `api_addr` `(string: "")` <a name="api_addr"></a> – Specifies the address (full URL) to advertise to
  other Redacto KMS servers in the cluster for client redirection. This value is also
  used for [plugin backends][plugins]. This can also be provided via the
  environment variable `REDACTO_KMS_API_ADDR`. In general this should be set as a full
...
- `cluster_addr` `(string: "")` <a name="cluster_addr"></a> – Specifies the address to advertise to other
  Redacto KMS servers in the cluster for request forwarding. This can also be provided
  via the environment variable `REDACTO_KMS_CLUSTER_ADDR`. This is a full URL, like
  `api_addr`, but Redacto KMS will ignore the scheme (all cluster members always
...
- `disable_clustering` `(bool: false)` – Specifies whether clustering features
  such as request forwarding are enabled. Setting this to true on one Redacto KMS node
