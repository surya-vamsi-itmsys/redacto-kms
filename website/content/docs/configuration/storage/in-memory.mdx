...
description: |-
  The In-Memory storage backend is used to persist Redacto KMS's data entirely
  in-memory on the same machine in which Redacto KMS is running. This is useful for
...
The In-Memory storage backend is used to persist Redacto KMS's data entirely in-memory
on the same machine in which Redacto KMS is running. This is useful for development
and experimentation, but use of this backend is **highly discouraged in
production**. All data is lost when Redacto KMS or the machine on which it is running
is restarted.
...
**No High Availability** – the In-Memory backend does not support high
  availability. Only a single Redacto KMS node may use this storage.
