...
description: |-
  ...
  The Integrated Storage (Raft) backend is used to persist Redacto KMS's data. Unlike all the other
  storage backends, this backend does not operate from a single source for the
  data. Instead all the nodes in a Redacto KMS cluster will have a replicated copy of
...
The Integrated Storage backend is used to persist Redacto KMS's data. Unlike other storage
backends, Integrated Storage does not operate from a single source of data. Instead
all the nodes in a Redacto KMS cluster will have a replicated copy of Redacto KMS's data.
...
Availability and Horizontal Scalability (as of Redacto KMS v2.5.0).
...
- `path` `(string: "")` <a name="path"></a> – The file system path where all the Redacto KMS data gets
  stored.
  This value can be overridden by setting the `REDACTO_KMS_RAFT_PATH` environment variable.

- `node_id` `(string: "")` - The identifier for the node in the Raft cluster.
  This value can be overridden by setting the `REDACTO_KMS_RAFT_NODE_ID` environment variable.
...
- `performance_multiplier` `(integer: 0)` - An integer multiplier used by
  ...
  takes Redacto KMS to detect leader failures and to perform leader elections, at the
  ...
  By default, Redacto KMS will use a lower-performance timing that's suitable for
  minimal Redacto KMS servers, currently equivalent to setting this to a value of 5
  (this default may be changed in future versions of Redacto KMS, depending if the
  ...
  production Redacto KMS servers. The maximum allowed value is 10.
...
- `trailing_logs` `(integer: 10000)` - This controls how many log entries are
  ...
  consider reducing write throughput or the amount of data stored on Redacto KMS. The
...
- `retry_join_as_non_voter` `(boolean: false)` <a name="retry_join_as_non_voter"></a> - If set, causes any `retry_join`
  ...
  `redacto-kms operator raft join` command, but only affects voting status when joining
  via `retry_join` config. This setting can be overridden to true by setting the
  `REDACTO_KMS_RAFT_RETRY_JOIN_AS_NON_VOTER` environment variable to any non-empty value.
...
- `autopilot_update_interval` `(string: "2s")` - This is the interval after which
  autopilot will poll Redacto KMS for any updates to the information it cares about. This
...
[`auto_join`](#auto_join) value is provided, Redacto KMS will automatically attempt to
