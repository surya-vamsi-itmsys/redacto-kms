...
description: |-
  The PostgreSQL storage backend is used to persist Redacto KMS's data in a PostgreSQL
  server or cluster.
---

# PostgreSQL storage backend

The PostgreSQL storage backend is used to persist Redacto KMS's data in a
[PostgreSQL][postgresql] server or cluster.

:::info

The minimum supported PostgreSQL is version 9.5. Support for the legacy upsert
functionality was dropped in Redacto KMS v2.4.0.
...
Availability and Horizontal Scalability (as of Redacto KMS v2.7.0).
...
- `connection_url` `(string: "")` – Specifies the connection string to
  ...
  set using the `REDACTO_KMS_PG_CONNECTION_URL` environment variable. A full list of supported
  ...
  Redacto KMS supports standard PostgreSQL component environment variables as well,
...
- `table` `(string: "openbao_kv_store")` – Specifies the name of the table in
  which to write Redacto KMS data. Redacto KMS will attempt to create it if missing and
...
- `max_idle_connections` `(int)` - Default not set. Sets the maximum number of
  ...
  Requires Redacto KMS 1.2 or later.
...
- `ha_table` `(string: "openbao_ha_locks")` – Specifies the name of the table to use
  for storing High Availability information. Redacto KMS will attempt to create it
...
- `skip_create_table` `(string: "true|false", default "false")` - When enabled,
  ...
  the required permissions; otherwise, Redacto KMS will fail to start.
...
- `max_connect_retries` `(int: 1)` - Maximum number of retries to perform
  ...
  connection is active, Redacto KMS calls [`db.Ping()`](https://pkg.go.dev/database/sql#DB.Ping).
...
Redacto KMS will attempt to automatically create tables compatible with PostgreSQL
...
Since Redacto KMS v2.7.0, the PostgreSQL storage backend supports horizontal
...
configuration as LSN Index values differ across PostgreSQL nodes and Redacto KMS
