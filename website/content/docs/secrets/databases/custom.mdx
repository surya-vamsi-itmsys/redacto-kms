...
description: |-
  ...
  plugin interface without needing to modify Redacto KMS's core code. This allows you
  ...
  plugins while keeping Redacto KMS itself statically linked.
---
...
Redacto KMS will continue to recognize the now deprecated version 4 of this
...
**Advanced topic!** Plugin development is a highly advanced topic in Redacto KMS,
...
while keeping Redacto KMS itself statically linked.
...
`ServeMultiplex` function call from Redacto KMS's `dbplugin` package.
...
map. The response object should contain the configuration map that should be saved within Redacto KMS.
...
The plugin runs as a separate binary outside of Redacto KMS, so the plugin itself
...
You should now be able to register your plugin into the Redacto KMS catalog. To do
this your token will need sudo permissions.

```shell-session
$ redacto-kms write sys/plugins/catalog/database/mydatabase-database-plugin \
    sha256="..." \
    command="mydatabase"
...
```shell-session
$ redacto-kms write database/config/mydatabase \
...
1. [Password policies](../../concepts/password-policies.mdx) required that
   Redacto KMS be responsible for generating passwords. In the prior version, the
...
2. Passwords needed to be generated by database plugins. This meant that plugin authors
   ...
   function available within the Redacto KMS SDK, however there was nothing preventing an
...
Redacto KMS supports both version 4 and version 5 database plugins. The support for
...
the case with version 5. Redacto KMS is responsible for generating passwords and passing them to
...
4. Update `CreateUser` to `NewUser`. The `NewUserRequest` object contains the username and
   ...
   instead, Redacto KMS will not know about it and will give the caller the wrong password.
