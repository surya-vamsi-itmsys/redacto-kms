...
request them from Redacto KMS, and use Redacto KMS's [leasing mechanism](../../concepts/lease.mdx)
...
Redacto KMS makes use of its own internal revocation system to ensure that users
become invalid within a reasonable time of the lease expiring.

### Static roles

With dynamic secrets, Redacto KMS generates a unique username and password pair for
each unique credential request. Redacto KMS also supports **static roles** for
some database secrets engines. Static roles are a 1-to-1 mapping of Redacto KMS roles
to usernames in a database. With static roles, Redacto KMS stores, and automatically
rotates, passwords for the associated database user based on a configurable
period of time.

When a client requests credentials for the static role, Redacto KMS
returns the current password for whichever database user is mapped to the
requested role. With static roles, anyone with the proper Redacto KMS policies can
access the associated user account in the database.

:::warning[Do not use static roles for root database credentials]
   Do not manage the same root database credentials that you provide to Redacto KMS in
   <tt>config/</tt> with static roles.

   Redacto KMS does not distinguish between standard credentials and root credentials
...
1. Configure Redacto KMS with the proper plugin and connection information:

   ```shell-session
   $ redacto-kms write database/config/my-database \
       plugin_name="..." \
       connection_url="..." \
       allowed_roles="..." \
       username="..." \
...
:::warning
 ...
 It is highly recommended a user within the database is created
   specifically for Redacto KMS to use. This user will be used to manipulate
   dynamic and static users within the database. This user is called the
   "root" user within the documentation.

:::

   Redacto KMS will use the user specified here to create/update/revoke database
...
1. After configuring the root user, it is highly recommended you rotate that user's
   ...
   ```shell-session
   $ redacto-kms write -force database/rotate-root/my-database
   ```
   ...
   user is created specifically for Redacto KMS to use to manage database users.

   :::

1. Configure a role that maps a name in Redacto KMS to a set of creation statements to
   create the database credential:

   ```shell-session
   $ redacto-kms write database/roles/my-role \
       db_name=my-database \
       creation_statements="..." \
       default_ttl="1h" \
       max_ttl="24h"
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```shell-session
    $ redacto-kms read database/creds/my-role
    Key                Value
    ---                -----
...
of dynamic and static roles configure the credential that Redacto KMS will generate and
...
`redacto-kms write database/config/my-database` you can specify a password policy for all
...
secrets engines to prevent Redacto KMS from escaping special characters in the
...
```shell-session
$ redacto-kms write database/config/my-mysql-database \
