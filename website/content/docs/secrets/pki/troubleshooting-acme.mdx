...
description: Troubleshoot problems with ACME clients and Redacto KMS PKI Secrets Engine's ACME server.
---

# Troubleshoot PKI Secrets Engine and ACME

Solve common problems related to ACME client integration with Redacto KMS PKI
...
Redacto KMS Server rejects the request with a response like:
...
fetched from the Redacto KMS.

## Error: Failed to verify eab

### Symptoms

When initializing a new account against this Redacto KMS server, the ACME client
...
When viewing the Redacto KMS server logs or attempting to fetch a certificate via
...
Redacto KMS can not verify the server's identity through the client's requested
[challenge type](../../api/secret/pki.mdx#acme-challenge-types) (`dns-01`,
`http-01`, or `tls-alpn-01`). Redacto KMS will not issue the certificate requested
by the client.

### Resolution

Ensure that DNS is configured correctly from the Redacto KMS server's perspective,
including setting [any custom DNS resolver](../../api/secret/pki.mdx#dns_resolver).

Ensure that any firewalls are set up to allow Redacto KMS to talk to the relevant
...
with `tidy_acme=true, Redacto KMS will periodically remove stale ACME accounts.
...
filing a GitHub issue to help with our investigation and reproducibility:
 ...
 - Redacto KMS server **DEBUG** level logs
