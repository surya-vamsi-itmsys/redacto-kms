...
description: The PKI secrets engine for Redacto KMS generates TLS certificates.
---
...
Redacto KMS storage is secure, but not as secure as a piece of paper in a bank vault.
It is, after all, networked software. If your root CA is hosted outside of
Redacto KMS, don't put it in Redacto KMS as well; instead, issue a shorter-lived
intermediate CA certificate and put this into Redacto KMS. This aligns with industry
best practices.
...
If you plan on using intermediate CAs with Redacto KMS, it is suggested that you let
Redacto KMS create CSRs and do not export the private key, then sign those with your
...
To keep old CAs active, there's two approaches to achieving rotation:
 ...
 1. Use multiple secrets engines. This allows a fresh start, preserving the
    old issuer and CRL. Redacto KMS ACL policy can be updated to deny new issuance
...
stored in Redacto KMS will often out-perform a HSM-backed intermediate. For
...
by the HSM and one backed by Redacto KMS -- can satisfy both use cases. Operators
...
issuer is no longer part of this PKI mount, Redacto KMS places them on the default
...
types can have meaningful impacts on performance of not only Redacto KMS, but
...
can be used to measure the performance of a Redacto KMS PKI instance. In general,
some considerations to be aware of:
 ...
 - Key signing requests (via `/pki/sign`) will be faster than (`/pki/issue`),
   especially for RSA keys: this removes the necessity for Redacto KMS to generate
 ...
 - Storage is an important factor: with [BYOC Revocation](../../api/secret/pki.mdx#revoke-certificate),
   ...
   certificate compounds the problem.
    ...
    - Storing too many certificates results in longer `LIST /pki/certs` time,
      ...
      logs to track certificates outside of Redacto KMS.

As a general comparison on unspecified hardware, using `benchmark-vault` for
`30s` on a local, single node, raft-backed Redacto KMS instance:

  - Redacto KMS can issue 300k certificates using EC P-256 for CA & leaf keys and
    without storage.

    - But switching to storing these leaves drops that number to 65k, and only
      20k with leases.

 - Using large, expensive RSA-4096 bit keys, Redacto KMS can only issue 160 leaves,
   regardless of whether or not storage or leases were used. The 95% key
   generation time is above 10s.

    - In comparison, using P-521 keys, Redacto KMS can issue closer to 30k leaves
...
root CA](#be-careful-with-root-cas), while letting Redacto KMS manage the
...
Redacto KMS supports the use of both the [`allowed_domains` parameter on
...
intermediate issuers will exist within the Redacto KMS PKI mount. In order to
...
certificates on behalf of consumers. Redacto KMS's PKI Engine only includes server
support for ACME; no client functionality has been included.

:::warning

Note: Redacto KMS's PKI ACME server caps the certificate's validity at 90 days
...
unauthenticated from a Redacto KMS perspective, the use of roles with ACME
...
To solve this issue, there are two possible approaches:
 ...
 1. Use a restrictive [`allowed_roles`, `allowed_issuers`, and
    ...
    to a Redacto KMS token other mechanism for acquiring a Redacto KMS EAB ACME token.
 2. Use a more permissive [configuration with
    ...
    to a Redacto KMS token which can be suitably ACL'd to particular sets of
    approved ACME directories.

The choice of approach depends on the policies of the organization wishing
to use ACME.

Another consequence of the Redacto KMS unauthenticated nature of ACME requests
...
accessible Redacto KMS instance. By default, without enforcing a restrictive
...
As such, we strongly recommend publicly facing Redacto KMS instances enforce that
...
Redacto KMS instances can enforce this by [setting the
`REDACTO_KMS_DISABLE_PUBLIC_ACME=true` environment
...
may not be tied to a valid Redacto KMS token when EAB is not used), many error
messages end up in the Redacto KMS server logs out of security necessity. When
...
errors), and then correlate with Redacto KMS server logs to identify the failure
reason.

### ACME security considerations

ACME allows any client to use Redacto KMS to make some sort of external call;
...
possible remote server implementations. Redacto KMS's ACME server makes three
types of requests:
 ...
 3. HTTP requests to `http://<domain>/.well-known/acme-challenge/<token>`,
    ...
    this could result in Redacto KMS being used to make (invalid) requests.
    Ideally, any such server implementations should be updated to ignore
    such ACME validation requests or to block access originating from Redacto KMS
    to this service.

In all cases, no information about the response presented by the remote
server is returned to the ACME client.

When running Redacto KMS on multiple networks, note that Redacto KMS's ACME server
places no restrictions on requesting client/destination identifier
validations paths; a client could use a HTTP challenge to force Redacto KMS to
reach out to a server on a network it could otherwise not access.

## Keep certificate lifetimes short, for CRL's sake

This secrets engine aligns with Redacto KMS's philosophy of short-lived secrets. As
...
If a certificate must truly be revoked, the normal Redacto KMS revocation function can
...
is an expensive operation! Due to the structure of the CRL standard, Redacto KMS must
...
clients don't have to figure out what to do with a lack of response. Run Redacto KMS
...
Note: When using multiple issuers in the same mount point,
   ...
   material). This means that Redacto KMS may need to regenerate multiple CRLs.
...
Note: Redacto KMS supports two complementary revocation mechanisms: Delta CRLs,
...
modification of the issuance behavior: should Redacto KMS `err`, preventing issuance
...
can negatively impact even large Redacto KMS clusters--even with short TTLs!

However, once these certificates are expired, a [tidy operation](../../api/secret/pki.mdx#tidy)
will clean up CRLs and Redacto KMS cluster storage.
...
OCSP responses are signed by the issuing CA within Redacto KMS). This means both are
...
different clusters. This has been the default behavior of Redacto KMS, but with
...
`no_store=true`, so Redacto KMS is strictly used as an issuing authority and isn't
...
Redacto KMS has no mechanism to sign OCSP requests at this time.

## Automate leaf certificate renewal

To manage certificates for services at scale, it is best to automate the
certificate renewal as much as possible. Redacto KMS Agent [has support for
...
The Redacto KMS PKI Secrets Engine supports many options to limit issuance via
...
be used sparingly, if at all).
 ...
 - When using short TTLs (< 30 days) or with high issuance volume, it is
   ...
   This prevents revocation but allows higher throughput as Redacto KMS no longer
...
Beyond Redacto KMS's default telemetry around request processing, PKI exposes count and
...
Because Redacto KMS HMACs audit string keys by default, it is necessary to tune
PKI secrets mounts to get an accurate view of issuance that is occurring under
this mount.

:::warning

Note: Depending on usage of Redacto KMS, CRLs (and rarely, CA chains) can grow to
   ...
   Redacto KMS can deny requests (with an opaque `500 Internal Error` message)
...
nature:
 ...
 - `private_key` - this response parameter contains the private keys
   generated by Redacto KMS during issuance, and
...
We suggest the following personas:
 ...
 - *Unauthed*; any arbitrary user or service that lacks a Redacto KMS token.
...
As of Redacto KMS v2.7, most places where CSRs, CRLs, or certificates are signed
with PSS should be compatible.

## OCSP

Go lacks support for creating OCSP responses with the PSS signature algorithm
or when the key type is ML-DSA. Redacto KMS will automatically downgrade issuers
