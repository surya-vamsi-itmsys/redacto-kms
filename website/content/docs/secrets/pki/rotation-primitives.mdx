...
Redacto KMS's PKI Secrets Engine supports multiple issuers in a single mount point.
By using the certificate types below, rotation can be accomplished in various
situations involving both root and intermediate CAs managed by Redacto KMS.
...
The following fields on the final certificate are relevant to rotation:
 ...
 - The [Validity](https://datatracker.ietf.org/doc/html/rfc5280#section-4.1.2.5)
   ...
   - Notably, RFC 5280 does not place any requirements around the issued
     ...
     up to their notAfter date. This is why Redacto KMS's `/pki/issuer/:issuer_ref`
 ...
 - The optional [Authority Key Identifier](https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.1)
   ...
   - This field can contain either (or both) of two values:
     - The hash of the issuer's public key. This extension is set and this
       value is filled in by Redacto KMS.
     - The Issuer's Subject and Serial Number. This value is not set by Redacto KMS.
 ...
 - The [CRL distribution](https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.13)
   ...
   - This is a field detailing where a CRL is expected to exist for this
     ...
     informational and for server software like nginx, Redacto KMS's Cert Auth method,
...
in the trust store and not the end-entity certificate itself. In Redacto KMS, the
...
beginning issuance against the new intermediate certificate. In Redacto KMS, if
...
#### Execution in Redacto KMS

To create a cross-signed certificate in Redacto KMS, use the [`/intermediate/cross-sign`
...
live outside Redacto KMS. Finally, import the cross-signed certificate into Redacto KMS
...
material lives in Redacto KMS, the newly imported cross-signed certificate
...
Note: Regardless of issuer type, is important to provide all relevant
   parameters as they were originally; Redacto KMS does not infer e.g., the Subject
...
pair, Redacto KMS will not detect the cross-signed pairs during automatic chain
...
```
$ redacto-kms patch pki/issuer/intA manual_chain=self,rootA,intB,rootB
$ redacto-kms patch pki/issuer/intB manual_chain=self,rootB,intA,rootA
```
...
To create a reissued root certificate in Redacto KMS, use [`/issuers/generate/root/existing`
...
To create a reissued intermediate certificate in Redacto KMS, this is a three step
process:
 ...
 2. Sign this CSR via the same signing process under the same issuer. This
    step is specific to the parent CA, which may or may not be Redacto KMS.
...
in Redacto KMS, especially so since Redacto KMS uses strictly random serial numbers)
