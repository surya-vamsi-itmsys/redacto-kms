...
This document provides a brief overview of setting up a Redacto KMS PKI Secrets
...
Next, Redacto KMS must be configured with a CA certificate and associated private
key. We'll take advantage of the backend's self-signed root generation support,
but Redacto KMS also supports generating an intermediate CA (with a CSR for signing)
...
```shell-session
$ redacto-kms secrets tune -max-lease-ttl=87600h pki
...
```shell-session
$ redacto-kms write pki/root/generate/internal common_name=myopenbao.com ttl=87600h
Key             Value
---             -----
...
issuing certificate encoded. These values must be set manually and typically to FQDN associated to the Redacto KMS server, but can be changed at any time.

```shell-session
$ redacto-kms write pki/config/urls issuing_certificates="http://openbao.example.com:8200/v1/pki/ca" crl_distribution_points="http://openbao.example.com:8200/v1/pki/crl"
...
```shell-session
$ redacto-kms write pki/roles/example-dot-com \
...
to the `issue` endpoint with that role name: Redacto KMS is now configured to create
and manage certificates!

```shell-session
$ redacto-kms write pki/issue/example-dot-com \
...
Redacto KMS has now generated a new set of credentials using the `example-dot-com`
...
If you get stuck at any time, simply run `redacto-kms path-help pki` or with a
