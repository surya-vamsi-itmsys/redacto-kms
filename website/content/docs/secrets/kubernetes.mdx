...
description: >-
  The Kubernetes secrets engine for Redacto KMS generates Kubernetes service account
  tokens, service accounts, role bindings, and roles dynamically.
---

# Kubernetes secrets engine

The Kubernetes Secrets Engine for Redacto KMS generates Kubernetes service account tokens, and
optionally service accounts, role bindings, and roles. The created service account tokens have
a configurable TTL and any objects created are automatically deleted when the Redacto KMS lease expires.

For each lease, Redacto KMS will create a service account token attached to the
...
**Note:** We do not recommend using tokens created by the Kubernetes Secrets Engine to
   authenticate with the [Redacto KMS Kubernetes Auth Method](../auth/kubernetes.mdx). This will
   generate many unique identities in Redacto KMS that will be hard to manage.
...
1. By default, Redacto KMS will connect to Kubernetes using its own service account.
   ...
   It's necessary to ensure that the service account Redacto KMS uses will have permissions to manage
   ...
   Redacto KMS service account with a role binding or cluster role binding.
   ...
   you will need to grant Redacto KMS permissions to read namespaces.
   ...
   **Note:** Getting the right permissions for Redacto KMS will require some trial
   ...
   **Note:** Protect the Redacto KMS service account, especially if you use broader
   permissions for it, as it is essentially a cluster administrator account.

   :::

1. Create a role binding to bind the role to Redacto KMS's service account and grant Redacto KMS permission
   to manage tokens.
...
1. If Redacto KMS will not be automatically managing roles or service accounts (see
   [Automatically Managing Roles and Service Accounts](#automatically-managing-roles-and-service-accounts)),
   then you will need to set up a service account that Redacto KMS will issue tokens for.

:::warning

 **Note**: It is highly recommended that the service account that Redacto KMS issues tokens for
      is **NOT** the same service account that Redacto KMS itself uses.
...
1. Configure the mount point. An empty config is allowed.
   ...
   ```shell-session
   $ redacto-kms write -f kubernetes/config
   ```

   Configuration options are available as specified in the
   [API docs](../api/secret/kubernetes.mdx).

1. You can now configure Kubernetes Secrets Engine to create a Redacto KMS role (**not** the same as a
   Kubernetes role) that can generate service account tokens for the given service account:

   ```shell-session
   $ redacto-kms write kubernetes/roles/my-role \
...
After a user has authenticated to Redacto KMS and has sufficient permissions, a write to the
`creds` endpoint for the Redacto KMS role will generate and return a new service account token.

```shell-session
$ redacto-kms write kubernetes/creds/my-role \
...
creating or tuning the Redacto KMS role.
...
You can set default audiences (`token_default_audiences`) when creating or tuning the Redacto KMS role.
...
When configuring the Redacto KMS role, you can pass in parameters to specify that you want to
automatically generate the Kubernetes service account and role binding,
and optionally generate the Kubernetes role itself.

If you want to configure the Redacto KMS role to use a pre-existing Kubernetes role, but generate
the service account and role binding automatically, you can set the `kubernetes_role_name`
parameter.

```shell-session
$ redacto-kms write kubernetes/roles/auto-managed-sa-role \
...
**Note**: Redacto KMS's service account will also need access to the resources it is granting
...
```shell-session
$ redacto-kms write kubernetes/creds/auto-managed-sa-role \
    kubernetes_namespace=test
Key                          Value
---                          -----
...
Furthermore, Redacto KMS can also automatically create the role in addition to the service account and
...
```shell-session
$ redacto-kms write kubernetes/roles/auto-managed-sa-and-role \
    allowed_kubernetes_namespaces="test" \
    generated_role_rules='{"rules":[{"apiGroups":[""],"resources":["pods"],"verbs":["list"]}]}'
```
You can then get credentials in the same way as before.
```shell-session
$ redacto-kms write kubernetes/creds/auto-managed-sa-and-role \
    kubernetes_namespace=test
Key                          Value
---                          -----
