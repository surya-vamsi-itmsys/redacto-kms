...
description: Secrets engines are mountable engines that store or generate secrets in Redacto KMS.
---
...
Secrets engines are enabled at a **path** in Redacto KMS. When a request comes to
Redacto KMS, the router automatically routes anything with the route prefix to the
...
according to its own API. Use `redacto-kms path-help` to determine the paths it
responds to.

Note that mount points cannot conflict with each other in Redacto KMS. There are
...
Secrets engines receive a _barrier view_ to the configured Redacto KMS physical
...
layer, it is prefixed with that UUID folder. Since the Redacto KMS storage layer
doesn't support relative access (such as `../`), this makes it impossible for an
enabled secrets engine to access other data.

This is an important security feature in Redacto KMS - even a malicious engine
