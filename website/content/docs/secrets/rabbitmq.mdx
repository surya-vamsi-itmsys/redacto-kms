...
description: >-
  The RabbitMQ secrets engine for Redacto KMS generates user credentials to access
  RabbitMQ.
---
...
Redacto KMS makes use both of its own internal revocation system as well as the
...
1.  Configure the credentials that Redacto KMS uses to communicate with RabbitMQ to
    generate credentials:

    ```text
    $ redacto-kms write rabbitmq/config/connection \
    ...
    It is important that the Redacto KMS user have the administrator privilege to
    manager users.

1.  Configure a role that maps a name in Redacto KMS to virtual host permissions:

    ```text
    $ redacto-kms write rabbitmq/roles/my-role \
        vhosts='{"/":{"write": ".*", "read": ".*"}}'
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```text
    $ redacto-kms read rabbitmq/creds/my-role
    Key                Value
    ---                -----
