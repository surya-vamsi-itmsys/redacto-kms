...
outside of Redacto KMS into the transit secrets engine.

This document describes the process for wrapping an externally-generated
key (the target key) for import into Redacto KMS. It describes the processes
...
Note that though this example uses SHA256, Redacto KMS also supports the use of
...
This is the ciphertext that should be provided to Redacto KMS when importing a
key into the transit secrets engine.

```shell-session
$ redacto-kms write transit/keys/test-key/import ciphertext=$CIPHERTEXT hash_function=SHA256 type=$KEY_TYPE
```
