...
description: >-
  The transit secrets engine for Redacto KMS encrypts/decrypts data in-transit. It
...
Redacto KMS doesn't store the data sent to the secrets engine. It can also be viewed
...
the burden onto the operators of Redacto KMS.
...
1.  Encrypt some plaintext data using the `/encrypt` endpoint with a named key:
    ...
    **NOTE:** All plaintext data **must be base64-encoded**. The reason for this
    requirement is that Redacto KMS does not require that the plaintext is "text". It
    ...
    ```text
    $ redacto-kms write transit/encrypt/my-key plaintext=$(echo "my secret data" | base64)
    ...
    identifies that it has been wrapped by Redacto KMS. The `v1` indicates the key
    version 1 was used to encrypt the plaintext; therefore, when you rotate
    keys, Redacto KMS knows which version to use for decryption. The rest is a base64
    concatenation of the initialization vector (IV) and ciphertext.

    Note that Redacto KMS does not _store_ any of this data. The caller is responsible
    for storing the encrypted ciphertext. When the caller wants the plaintext,
    it must provide the ciphertext back to Redacto KMS to decrypt the value.

    :::danger

    Redacto KMS HTTP API imposes a maximum request size of 32MB to prevent a denial
    of service attack. This can be tuned per [`listener`
    block](../../configuration/listener/tcp.mdx) in the Redacto KMS server
    configuration.
...
1.  Decrypt a piece of data using the `/decrypt` endpoint with a named key:
    ...
    ```text
    $ redacto-kms write transit/decrypt/my-key ciphertext=vault:v1:8SDd3WHDOjf7mq69CyCqYjBXAiQQAVZRkFM13ok481zoCmHnSeDX9vyf7w==
    ...
    ```text
    $ redacto-kms write -field=plaintext transit/decrypt/my-key ciphertext=... | base64 --decode
    my secret data
    ```
...
1.  Rotate the underlying encryption key. This will generate a new encryption key
    ...
    ```text
    $ redacto-kms write -f transit/keys/my-key/rotate
...
1.  Upgrade already-encrypted data to a new key. Redacto KMS will decrypt the value
    using the appropriate key in the keyring and then encrypted the resulting
    plaintext with the newest key in the keyring.

    ```text
    $ redacto-kms write transit/rewrap/my-key ciphertext=vault:v1:8SDd3WHDOjf7mq69CyCqYjBXAiQQAVZRkFM13ok481zoCmHnSeDX9vyf7w==
    ...
    This process **does not** reveal the plaintext data. As such, a Redacto KMS policy
...
have Transit generate and manage a key within Redacto KMS.
...
```text
$ redacto-kms read transit/wrapping_key
```
...
- Wrap the AES key under the Redacto KMS wrapping key using RSAES-OAEP with MGF1 and
