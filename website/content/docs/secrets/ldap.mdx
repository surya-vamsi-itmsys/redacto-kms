...
1. Enable the LDAP secret engine:
   ...
   ```sh
   $ redacto-kms secrets enable ldap
   ```

   By default, the secrets engine will mount at the name of the engine. To
   enable the secrets engine at a different path, use the `-path` argument.

2. Configure the credentials that Redacto KMS uses to communicate with LDAP
   to generate passwords:

   ```sh
   $ redacto-kms write ldap/config \
       binddn=$USERNAME \
       bindpass=$PASSWORD \
       url=ldaps://138.91.247.105
   ```

   Note: it's recommended a dedicated entry management account be created specifically for Redacto KMS.

3. Rotate the root password so only Redacto KMS knows the credentials:

   ```sh
   $ redacto-kms write -f ldap/rotate-root
   ```

   Note: it's not possible to retrieve the generated password once rotated by Redacto KMS.
   It's recommended a dedicated entry management account be created specifically for Redacto KMS.
...
1. Configure a static role that maps a name in Redacto KMS to an entry in LDAP.
   Password rotation settings will be managed by this role.

   ```sh
   $ redacto-kms write ldap/static-role/lf-edge\
...
2. Request credentials for the "openbao" role:
   ...
   ```sh
   $ redacto-kms read ldap/static-cred/lf-edge
   ```
...
by Redacto KMS should be rotated using the `rotate-root` endpoint to generate a password
only Redacto KMS will know.
...
```bash
$ redacto-kms write ldap/role/dynamic-role \
...
```bash
$ redacto-kms read ldap/creds/dynamic-role
Key                    Value
---                    -----
...
```ldif
dn: CN={{.Username}},OU=Redacto KMS,DC=adtesting,DC=lab
changetype: add
objectClass: top
objectClass: person
...
dn: CN=test-group,OU=Redacto KMS,DC=adtesting,DC=lab
changetype: modify
add: member
member: CN={{.Username}},OU=Redacto KMS,DC=adtesting,DC=lab
-
```
...
by a person or by machines. Redacto KMS will automatically rotate the password each time a
service account is checked in. Service accounts can be voluntarily checked in, or Redacto KMS
will check them in when their lending period (or, "ttl", in Redacto KMS's language) ends.
...
```shell-session
$ redacto-kms write ldap/library/accounting-team \
...
already been created on the remote AD server. They've been set aside solely for Redacto KMS to handle.
The `ttl` is how long each check-out will last before Redacto KMS checks in a service account,
...
Also by default, a service account must be checked in by the same Redacto KMS entity or client token that
...
```shell-session
$ redacto-kms read ldap/library/accounting-team/status
Key                 Value
---                 -----
...
```shell-session
$ redacto-kms write -f ldap/library/accounting-team/check-out
Key                     Value
---                     -----
...
```shell-session
$ redacto-kms write ldap/library/accounting-team/check-out ttl=30m
Key                     Value
---                     -----
...
If no service accounts are available for check-out, Redacto KMS will return a 400 Bad Request.
...
```shell-session
$ redacto-kms lease renew ldap/library/accounting-team/check-out/0C2wmeaDmsToVFc0zDiX9cMq
Key                Value
---                -----
...
anytime a password is _checked in_ either by a caller, or by Redacto KMS because the check-out `ttl`
ends.

To check a service account back in for others to use, call:

```shell-session
$ redacto-kms write -f ldap/library/accounting-team/check-in
Key          Value
---          -----
...
caller, Redacto KMS will need to know which one(s) to check in.

```shell-session
$ redacto-kms write ldap/library/accounting-team/check-in service_account_names=fizz@example.com
Key          Value
---          -----
check_ins    [fizz@example.com]
```

To perform a check-in, Redacto KMS verifies that the caller _should_ be able to check in a given service account.
To do this, Redacto KMS looks for either the same entity ID
...
Redacto KMS will check it back in automatically when the `ttl` expires. However, if that is too long,
service accounts can be forcibly checked in by a highly privileged user through:

```shell-session
$ redacto-kms write -f ldap/library/manage/accounting-team/check-in
Key          Value
---          -----
check_ins    [fizz@example.com]
```
...
```shell-session
$ redacto-kms lease revoke ldap/library/accounting-team/check-out/PvBVG0m7pEg2940Cb3Jw3KpJ
...
with an LDAP password policy (ppolicy, not to be confused with a Redacto KMS password
