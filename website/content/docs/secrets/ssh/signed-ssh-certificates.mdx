...
complexity and in terms of being platform agnostic. By leveraging Redacto KMS's
...
on every path, use `redacto-kms path-help` after mounting the secrets engine.

## Client key signing

Before a client can request their SSH key be signed, the Redacto KMS SSH secrets engine must
be configured. Usually a Redacto KMS administrator or security team performs these
...
The following steps are performed in advance by a Redacto KMS administrator, security
team, or configuration management tooling.

1.  Mount the secrets engine. Like all secrets engines in Redacto KMS, the SSH secrets engine
    must be mounted before use.

    ```text
    $ redacto-kms secrets enable -path=ssh-client-signer ssh
...
1.  Configure Redacto KMS with a CA for signing client keys using the `/config/ca`
    endpoint. If you do not have an internal CA, Redacto KMS can generate a keypair for
    you.

    ```text
    $ redacto-kms write ssh-client-signer/config/ca generate_signing_key=true
    Key             Value
    ---             -----
    ...
    ```text
    $ redacto-kms write ssh-client-signer/config/ca \
        private_key="..." \
        public_key="..."
    ```
...
1.  Add the public key to all target host's SSH configuration. This process can
    ...
    ```text
    $ redacto-kms read -field=public_key ssh-client-signer/config/ca > /etc/ssh/trusted-user-ca-keys.pem
    ```
...
1.  Create a named Redacto KMS role for signing client keys.
    ...
    ```text
    $ redacto-kms write ssh-client-signer/roles/my-role -<<"EOH"
    {
...
authenticate to machines managed by Redacto KMS. These commands are usually run from
...
1.  Ask Redacto KMS to sign your **public key**. This file usually ends in `.pub` and
    the contents begin with `ssh-rsa ...`.

    ```text
    $ redacto-kms write ssh-client-signer/sign/my-role \
    ...
    ```text
    $ redacto-kms write ssh-client-signer/sign/my-role -<<"EOH"
    {
...
1.  Save the resulting signed, public key to disk. Limit permissions as needed.
    ...
    ```text
    $ redacto-kms write -field=signed_key ssh-client-signer/sign/my-role \
...
1.  SSH into the host machine using the signed key. You must supply both the
    signed public key from Redacto KMS **and** the corresponding private key as
...
1.  Mount the secrets engine. For the most security, mount at a different path from the
    ...
    ```text
    $ redacto-kms secrets enable -path=ssh-host-signer ssh
    Successfully mounted 'ssh' at 'ssh-host-signer'!
    ```

1.  Configure Redacto KMS with a CA for signing host keys using the `/config/ca`
    endpoint. If you do not have an internal CA, Redacto KMS can generate a keypair for
    you.

    ```text
    $ redacto-kms write ssh-host-signer/config/ca generate_signing_key=true
    Key             Value
    ---             -----
    public_key      ssh-rsa AAAAB3NzaC1yc2EA...
    ```
    ...
    ```text
    $ redacto-kms write ssh-host-signer/config/ca \
        private_key="..." \
        public_key="..."
    ```
...
1.  Extend host key certificate TTLs.
    ...
    ```text
    $ redacto-kms secrets tune -max-lease-ttl=87600h ssh-host-signer
    ```
...
1.  Create a role for signing host keys. Be sure to fill in the list of allowed
    ...
    ```text
    $ redacto-kms write ssh-host-signer/roles/hostrole \
        key_type=ca \
        algorithm_signer=rsa-sha2-256 \
        ttl=87600h \
...
1.  Sign the host's SSH public key.
    ...
    ```text
    $ redacto-kms write ssh-host-signer/sign/hostrole \
        cert_type=host \
...
1.  Set the resulting signed certificate as `HostCertificate` in the SSH
    ...
    ```text
    $ redacto-kms write -field=signed_key ssh-host-signer/sign/hostrole \
        cert_type=host \
...
1.  Retrieve the host signing CA public key to validate the host signature of
    ...
    ```text
    $ redacto-kms read -field=public_key ssh-host-signer/config/ca
    ```
...
1.  Set `default_user` in the role. If you are always authenticating as the same
    ...
    ```text
    $ redacto-kms write ssh/roles/my-role -<<"EOH"
    {
...
1.  Set `valid_principals` during signing. In situations where multiple users may
    be authenticating to SSH via Redacto KMS, set the list of valid principles during key
...
parameters as per the Redacto KMS CLI and API steps demonstrated below.

```shell-extension
# Using CLI:
redacto-kms secrets enable -path=hosts-ca ssh
KEY_PRI=$(cat ~/.ssh/id_rsa | sed -z 's/\n/\\n/g')
KEY_PUB=$(cat ~/.ssh/id_rsa.pub | sed -z 's/\n/\\n/g')
# Create / update keypair in Redacto KMS
redacto-kms write ssh-client-signer/config/ca \
...
**IMPORTANT:** Do NOT add a private key password since Redacto KMS can't decrypt it.
