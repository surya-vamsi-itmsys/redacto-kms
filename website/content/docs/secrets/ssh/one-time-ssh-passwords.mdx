...
description: |-
  The One-Time SSH Password (OTP) SSH secrets engine type allows a Redacto KMS server
...
The One-Time SSH Password (OTP) SSH secrets engine type allows a Redacto KMS server to
...
An authenticated client requests credentials from the Redacto KMS server and, if
...
the Redacto KMS helper, which then validates the OTP with the Redacto KMS server. The Redacto KMS
server then deletes this OTP, ensuring that it is only used once.

Since the Redacto KMS server is contacted during SSH connection establishment, every
login attempt and the correlating Redacto KMS lease information is logged to the audit
secrets engine.
...
documentation on every path, use `redacto-kms path-help` after mounting the secrets
engine.

### Drawbacks

The main concern with the OTP secrets engine type is the remote host's
connection to Redacto KMS; if compromised, an attacker could spoof the Redacto KMS server
returning a successful request. This risk can be mitigated by using TLS for the
connection to Redacto KMS and checking certificate validity; future enhancements to
...
```shell-session
$ redacto-kms write ssh/roles/otp_key_role \
    key_type=otp \
...
```shell-session
$ redacto-kms write ssh/creds/otp_key_role ip=x.x.x.x
Key            	Value
...
```shell-session
$ redacto-kms ssh -role otp_key_role -mode otp username@x.x.x.x
...
```shell-session
$ redacto-kms ssh -role otp_key_role -mode otp -strict-host-key-checking=no username@x.x.x.x
