...
```shell-session
$ redacto-kms secrets enable -version=1 kv
```
...
1. Write arbitrary data:
   ...
   ```shell-session
   $ redacto-kms kv put kv/my-secret my-value=s3cr3t
...
1. Read arbitrary data:
   ...
   ```shell-session
   $ redacto-kms kv get kv/my-secret
   Key                 Value
   ---                 -----
   my-value            s3cr3t
   ```
...
1. Delete a key:
   ...
   ```shell-session
   $ redacto-kms kv delete kv/my-secret
...
1. Write data using the `example` policy:
   ...
   ```shell-session
   $ redacto-kms kv put kv/my-generated-secret \
...
1. Read the generated data:
   ...
   ```shell-session
   $ redacto-kms kv get kv/my-generated-secret
   ====== Data ======
   Key         Value
   ---         -----
...
```shell-session
$ redacto-kms kv put kv/my-secret ttl=30m my-value=s3cr3t
Success! Data written to: kv/my-secret
```
