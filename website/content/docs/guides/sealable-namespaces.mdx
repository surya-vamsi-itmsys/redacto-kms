...
description: >-
  This page contains a guide to get started with sealable namespaces in Redacto KMS
---
...
```shell-session
$ redacto-kms namespace create -ns=tenants -key-shares=5 -key-threshold=3 my-tenant
...
Just like `redacto-kms operator init`, you can specify a list of PGP or Keybase keys to
...
A newly created sealable namespace is sealed. To unseal a Shamir-sealed namespace, use the command `redacto-kms namespace unseal`:

```shell-session
$ redacto-kms namespace unseal -ns=tenants my-tenant
...
Unlike the global Redacto KMS seal, namespaces do not have to be unsealed per node,
but just once per cluster.

## Sealing a Namespace

To seal a sealable namespace, use the command `redacto-kms namespace seal`:

```shell-session
$ redacto-kms namespace seal -ns=tenants my-tenant
...
To retrieve the seal status of a namespace, you can use the command `redacto-kms namespace seal-status`:

```shell-session
$ redacto-kms namespace seal-status -ns=tenants my-tenant
Key                Value
---                -----
Seal Type          shamir
Initialized        true
...
If a sealable namespace is unsealed and empty, it can be deleted with the normal `redacto-kms namespace delete` command:

```shell-session
$ redacto-kms namespace delete -ns=tenants my-tenant
Success! Namespace deleted at: tenants/my-tenant/
```

If a namespace is sealed and does not contain child namespaces, it can be
deleted with `redacto-kms namespace delete-sealed`:

```shell-session
$ redacto-kms namespace delete-sealed -ns=tenants my-tenant
...
```shell-session
$ redacto-kms namespace delete-sealed -ns=tenants -force my-tenant
Success! Namespace deletion scheduled: tenants/my-tenant/
```

:::info

Note that `redacto-kms namespace delete-sealed` requires the sudo capability, and will
not clean up external resources via lease deletion like standard namespace
deletion does.

Prefer the standard `redacto-kms namespace delete` command unless the namespace is
...
`redacto-kms operator rotate-keys` command with the `-ns` flag.

```shell-session
$ redacto-kms operator rotate-keys -ns=tenants/my-tenant -init
...
**Note**: Unlike the `redacto-kms namespace` commands, the `redacto-kms operator` commands require
...
```shell-session
$ redacto-kms operator rotate-keys -ns=tenants/my-tenant
...
Redacto KMS unseal keys rotated to 5 key shares and a key threshold of 3. Please
securely distribute the key shares printed above. When Redacto KMS is re-sealed,
...
To rotate the data encryption keys of a namespace, you can use the existing `redacto-kms
operator rotate` command, while specifying the namespace with the `-ns` flag.
This requires the namespace to be unsealed.

```shell-session
$ redacto-kms operator rotate -ns=tenants/my-tenant
Success! Rotated key
...
You can always check the current status of the namespace keyring by using the `redacto-kms operator key-status` command:

```shell-session
$ redacto-kms operator key-status -ns=tenants/my-tenant
Key Term            2
Install Time        10 Jun 26 08:37 UTC
Encryption Count    0
```
...
You can use the `redacto-kms operator generate-root` command to generate a namespace-scoped root token for a sealable namespace.

```shell-session
$ redacto-kms operator generate-root -ns=tenants/my-tenant -init
...
```shell-session
$ redacto-kms operator generate-root -ns=tenants/my-tenant
...
Check out the generated token using `redacto-kms token lookup`:
