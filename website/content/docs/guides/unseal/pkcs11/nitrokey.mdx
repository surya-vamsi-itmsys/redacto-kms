...
description: |-
  This guide describes how to configure auto-unsealing for Redacto KMS using
  the Nitrokey NetHSM via PKCS#11.
---

# Nitrokey Open Source NetHSM

This guide describes how to configure auto-unsealing for Redacto KMS using the
...
setup example for the NetHSM integration with Redacto KMS.
...
This guide only covers how to configure your Redacto KMS instance to auto-unseal through a
...
If you just want to get started quickly and verify, that Redacto KMS works as
...
```sh
$ nitropy nethsm add-user --user-id operator --role operator --passphrase 'OperatorOperator' --real-name "Redacto KMS Operator"
```
...
## Preparing the Redacto KMS host

### Installing the NetHSM PKCS#11 library

Redacto KMS will interact with the NetHSM using the
[open-source PKCS#11 library](https://github.com/Nitrokey/nethsm-pkcs11)
You will need to make this library available on the system that Redacto KMS
will run on.
...
When working with containers you can build your own Redacto KMS image based
...
Redacto KMS must be compiled with HSM support to load the library. HSM-enabled
binaries, packages and container images are generally available as part of
[Redacto KMS's releases](https://github.com/openbao/openbao/releases).
...
to manually initialize Redacto KMS:
...
The NetHSM has to be in operational state when Redacto KMS attempts auto-unseal.
You can just restart the Redacto KMS service to trigger auto-unseal.

:::

:::note

After manually sealing a Redacto KMS instance that is configured for
auto-unseal you can use the recovery keys
[to trigger a manual unseal](../../../concepts/seal.mdx#auto-unseal).
The recovery keys are only used for authorization here. The Redacto KMS root key
