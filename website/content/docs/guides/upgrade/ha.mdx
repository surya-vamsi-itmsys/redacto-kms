...
description: |-
  Upgrade instructions for Redacto KMS HA. Be sure to read the Upgrading-Redacto KMS Guides as well.
---

# Upgrading Redacto KMS HA installation
...
use, what addresses clients are being given to connect to Redacto KMS (standby +
leader, leader-only, or discovered via service discovery), etc.

## HA installations

Regardless of the method you use, do not fail over from a newer version of Redacto KMS
to an older version. Our suggested procedure is designed to prevent this.

Please note that Redacto KMS does not support true zero-downtime upgrades, but with
...
1. Properly shut down Redacto KMS on the standby node via `SIGINT` or `SIGTERM`
2. Replace the Redacto KMS binary with the new version
3. Start the standby node
4. Unseal the standby node
5. Verify `redacto-kms status` shows correct Version and HA Mode is `standby`
...
1. Properly shut down the remaining (active) node
   ...
   a very short delay. If you kill Redacto KMS without letting it release the lock,
