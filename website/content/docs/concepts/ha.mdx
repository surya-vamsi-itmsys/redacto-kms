...
description: >-
  Redacto KMS can be highly available, allowing you to run multiple Redacto KMSs to protect
  against outages.
---

# High Availability (HA) mode

Redacto KMS supports a multi-server mode for High Availability. This mode protects
against outages by running multiple Redacto KMS servers. High Availability mode
...
information. If it is, then Redacto KMS will automatically use HA mode. This
information is also available on the
[Configuration](../configuration/storage/index.mdx) page.

To be Highly Available, one of the Redacto KMS server nodes grabs a lock within the
...
the bottleneck of Redacto KMS is the data store itself, not Redacto KMS core.

Certain storage backends can support High Availability mode, which enable them
to store both Redacto KMS's information in addition to the HA lock. However, Redacto KMS
...
with different backends. For instance, a Redacto KMS cluster can be set up to use
...
communication takes place within Redacto KMS's encrypted storage; the active node
writes this information and unsealed standby Redacto KMS nodes can read it.
...
the value can also be specified by the `REDACTO_KMS_API_ADDR` environment variable,
which takes precedence.

What the [`api_addr`](../configuration/index.mdx#api_addr) value should be set to
depends on how Redacto KMS is set up. There are two common scenarios: Redacto KMS servers
accessed directly by clients, and Redacto KMS servers accessed via a load balancer.
...
When clients are able to access Redacto KMS directly, the
[`api_addr`](../configuration/index.mdx#api_addr) for each node should be that node's
address. For instance, if there are two Redacto KMS nodes:
...
Redacto KMS servers, but actually have direct access to each Redacto KMS node. In this
case, the Redacto KMS servers should actually be set up as described in the above
section, since for redirection purposes the clients have direct access.

However, if the only access to the Redacto KMS servers is via the load balancer, the
...
Each [`listener`](../configuration/listener/index.mdx) block in Redacto KMS's configuration
file contains an [`address`](../configuration/listener/tcp.mdx#address) value on
which Redacto KMS listens for requests. Similarly, each
...
Redacto KMS listens for server-to-server cluster requests. If this value is not set,
...
This value can also be specified by the `REDACTO_KMS_CLUSTER_ADDR` environment
...
The [Redacto KMS Integrated Storage](../configuration/storage/raft.mdx) is
recommended as the default HA backend for new deployments of Redacto KMS. See the
[comparison
...
Since Redacto KMS v2.5.0, the [Raft backend](../configuration/storage/raft.mdx)
...
for horizontal scalability was added in Redacto KMS v2.7.0 for the [PostgreSQL
