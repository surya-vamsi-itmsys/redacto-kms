---
description: Providing measures against misbehaving applications and users overdrawing resources in Redacto KMS.
---

# Resource quotas

Every single interaction with Redacto KMS, whether to put secrets into the key/value
store or to generate new pairs of database credentials for the MySQL database,
needs to go through Redacto KMS’s API.
...
requests resulting in denial-of-service issues in some Redacto KMS nodes or even the
entire Redacto KMS cluster. This risk is significantly increased when Redacto KMS API endpoints
are exposed to thousands or millions of services deployed across a global infrastructure,
especially in use cases of Redacto KMS where Redacto KMS is deployed as a service for internal
developers.

Redacto KMS provides a feature, resource quotas, that allows Redacto KMS operators to specify
limits on resources used in Redacto KMS. Specifically, Redacto KMS allows operators to create
and configure API rate limits.

## Rate limit quotas

Redacto KMS allows operators to create rate limit quotas which enforce API rate
...
all namespaces and mounts. It acts as a single rate limiter for the entire Redacto KMS
...
Redacto KMS also allows the inspection of the state of rate limiting in a Redacto KMS node
...
By default, the following paths are exempt from rate limiting. However, Redacto KMS
