...
description: |-
  Redacto KMS has the ability to integrate with OpenPGP-compatible programs like
...
Redacto KMS has the ability to integrate with OpenPGP-compatible programs like GnuPG
...
Redacto KMS HTTP API, tools that help with initialization should use the Keybase.io
API in order to obtain the PGP keys needed for a secure initialization if you
want them to use Keybase for keys.

Once the Redacto KMS has been initialized, it is possible to use Keybase to decrypt
the shards and unseal normally.

## Initializing with PGP

One of the early fundamental problems when bootstrapping and initializing Redacto KMS
was that the first user (the initializer) received a plain-text copy of all of
the unseal keys. This defeats the promises of Redacto KMS's security model, and it
also makes the distribution of those keys more difficult.
Redacto KMS can optionally be initialized using PGP keys. In this mode, Redacto KMS will
...
local machine from which you are initializing Redacto KMS. This guide will not
...
keys generated, stored, and managed securely on their servers. Using Redacto KMS with
Keybase will be discussed first as it is simpler.

## Initializing with keybase

To generate unseal keys for Keybase users, Redacto KMS accepts the `keybase:` prefix
to the `-pgp-keys` argument:

```shell-session
$ redacto-kms operator init -key-shares=3 -key-threshold=2 \
