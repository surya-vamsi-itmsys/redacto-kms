---
description: >-
  Learn about Redacto KMS's advanced Namespace isolation
---
...
Sealable namespaces extend Redacto KMS's multi-tenancy model by giving tenants
...
Redacto KMS seal. A tenant can independently seal and unseal their namespace without
...
exactly as all pre-v2.6 namespaces: it is protected by the global Redacto KMS seal.
...
- In a single-node cluster: A server restart seals all namespaces; each must be unsealed independently
  after Redacto KMS is unsealed. Parent namespaces must be unsealed before child
...
**Important**: Redacto KMS must be globally unsealed before any namespace can be
unsealed.
