...
description: >-
  Redacto KMS Namespaces enable secure multi-tenancy and self-management of
  instances.
---

# Namespaces

Many organizations implement Redacto KMS as a service to provide centralized
...
Namespaces support secure multi-tenancy (**SMT**) within a single Redacto KMS
instance with tenant isolation and administration delegation so Redacto KMS
...
login paths that functions as a mini-Redacto KMS instance within your Redacto KMS
...
Valid Redacto KMS namespace names:
...
of [Redacto KMS limits and maximums](../../internals/limits.mdx) for storage limits
...
Redacto KMS system administrators can assign administration rights to delegate
...
## Redacto KMS API and namespaces
...
Redacto KMS constructs the fully qualified namespace path based on the calling
...
The Redacto KMS API includes system backend endpoints, which are mounted under the
`sys/` path. System endpoints let you interact with the internal features of
your Redacto KMS instance.

By default, Redacto KMS allows non-root calls to the less-sensitive system backend
endpoints. But, for security reasons, Redacto KMS restricts access to some of the
