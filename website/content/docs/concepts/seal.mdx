---
description: >-
  A Redacto KMS must be unsealed before it can access its data. Likewise, it can be
  sealed to lock it down.
---

# Seal/Unseal

When a Redacto KMS server is started, it starts in a _sealed_ state. In this
state, Redacto KMS is configured to know where and how to access the physical
...
read the decryption key to decrypt the data, allowing access to the Redacto KMS.

Prior to unsealing, almost no operations are possible with Redacto KMS. For
example authentication, managing the mount tables, etc. are all not possible.
The only possible operations are to unseal the Redacto KMS and check the status
of the seal.

## Why?

The data stored by Redacto KMS is encrypted. Redacto KMS needs the _encryption key_ in order
...
Therefore, to decrypt the data, Redacto KMS must decrypt the encryption key
which requires the root key. Unsealing is the process of getting access to
this root key. The root key is stored alongside all other Redacto KMS data,
but is encrypted by yet another mechanism: the unseal key.

To recap: most Redacto KMS data is encrypted using the encryption key in the keyring;
...
The default Redacto KMS config uses a Shamir seal. Instead of distributing the unseal
key as a single key to an operator, Redacto KMS uses an algorithm known as
...
The unseal process is done by running `redacto-kms operator unseal` or via the API.
...
Once a Redacto KMS node is unsealed, it remains unsealed until one of these things happens:

1. It is resealed via the API (see below).

2. The server is restarted.

3. Redacto KMS's storage layer encounters an unrecoverable error.

:::info

**Note:** Unsealing makes the process of automating a Redacto KMS install
difficult. Automated tools can easily install, configure, and start Redacto KMS,
...
There is also an API to seal the Redacto KMS. This will throw away the root
...
This way, if there is a detected intrusion, the Redacto KMS data can be locked
...
Redacto KMS will connect to the device or service implementing the seal and ask it
to decrypt the root key Redacto KMS read from storage.

![Auto Unseal](/img/openbao-auto-unseal.png)

There are certain operations in Redacto KMS besides unsealing that
...
It is still possible to seal a Redacto KMS node using the API. In this case Redacto KMS
...
sufficient to unseal Redacto KMS if the Auto Unseal mechanism isn't working. They
are purely an authorization mechanism.  Using Auto Unseal
creates a strict Redacto KMS lifecycle dependency on the underlying seal mechanism.
...
access to the Redacto KMS cluster until the mechanism is available again. **If the seal
mechanism or its keys are permanently deleted, then the Redacto KMS cluster cannot be recovered, even
from backups.**
...
When Redacto KMS is initialized while using an HSM or KMS, rather than unseal keys
...
to Redacto KMS's treatment of unseal keys when running without an HSM or KMS.
...
Additionally, Redacto KMS will refuse to initialize if the option has not been set to
...
Redacto KMS's unseal key can be rotated using a normal `redacto-kms operator rotate-keys`
...
target different key holders via different PGP keys. When using the Redacto KMS CLI,
this is performed by using the `-target=recovery` flag to `redacto-kms operator rotate-keys`.
...
supported in Redacto KMS. However, there is a current limitation that
