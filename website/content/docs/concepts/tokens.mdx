---
description: Tokens are a core auth method in Redacto KMS. Concepts and important features.
---
...
Tokens are the core method for _authentication_ within Redacto KMS. Tokens
...
`redacto-kms server -dev` (or `redacto-kms operator init` for a non-dev server) outputs an
initial "root token." This is the first method of authentication for Redacto KMS.
...
Within Redacto KMS, tokens map to information. The most important information mapped
to a token is a set of one or more attached
[policies](policies.mdx). These policies control what the token
holder is allowed to do within Redacto KMS. Other mapped information includes
...
tokens can do anything in Redacto KMS. _Anything_. In addition, they are the only
type of token within Redacto KMS that can be set to never expire without any renewal
needed. As a result, it is purposefully hard to create root tokens; in fact
there are only three ways to create root tokens:

1. The initial root token generated at `redacto-kms operator init` time -- this token has no
   expiration
1. By using another root token; a root token with an expiration cannot create a
   root token that never expires
1. By using `redacto-kms operator generate-root`
...
in production. In fact, the Redacto KMS team recommends that root tokens are only
...
If the token is renewable, Redacto KMS can be asked to extend the token validity
period using `redacto-kms token renew` or the appropriate renewal endpoint. At this
...
1. The system max TTL, which is 32 days but can be changed in Redacto KMS's
   configuration file.
...
token. However, outside of direct operator interaction, Redacto KMS will never revoke
...
currently the only way for a token in Redacto KMS to have an unlimited lifetime.
...
Service tokens are what users will generally think of as "normal" Redacto KMS tokens.
...
be used for Redacto KMS actions, but they require no storage on disk to track them.
