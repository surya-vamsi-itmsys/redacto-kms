---
description: >-
  Before performing any operation with Redacto KMS, the connecting client must be
  authenticated.
---
...
Authentication in Redacto KMS is the process by which user or machine supplied
information is verified against an internal or external system. Redacto KMS supports
multiple [auth methods](../auth/index.mdx) including GitHub,
LDAP, AppRole, and more. Each auth method has a specific use case.

Before a client can interact with Redacto KMS, it must _authenticate_ against an
...
Redacto KMS supports a number of auth methods. Some backends are targeted
...
```shell-session
$ redacto-kms write sys/auth/my-auth type=userpass
```
...
```shell-session
$ redacto-kms path-help auth/my-auth
# ...
```

Redacto KMS supports multiple auth methods simultaneously, and you can even
mount the same type of auth method at different paths. Only one
authentication is required to gain access to Redacto KMS, and it is not currently
...
Redacto KMS generates a unique access token for you to use for future requests.
...
To authenticate with the CLI, `redacto-kms login` is used. This supports many
of the built-in auth methods. For example, with GitHub:

```shell-session
$ redacto-kms login -method=github token=<token>
...
```
...
auth method implements its own login endpoint. Use the `redacto-kms path-help`
mechanism to find the proper endpoint.

For example, the GitHub login endpoint is located at `auth/github/login`.
And to determine the arguments needed, `redacto-kms path-help auth/github/login` can
be used.
...
completely reauthenticate. Just use `redacto-kms token renew <token>` with the
...
// Once you've set the token for your Redacto KMS client, you will need to
...
func renewToken(client *openbao.Client) {
	for {
		...
		if err != nil {
			log.Fatalf("unable to authenticate to Redacto KMS: %v", err)
		}
...
func login(client *openbao.Client) (*openbao.Secret, error) {
	...
	// log in to Redacto KMS using various auth methods. This function is just
