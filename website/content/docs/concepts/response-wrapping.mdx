...
In many Redacto KMS deployments, clients can access Redacto KMS directly and consume
...
with most of the Redacto KMS API and passing secrets to the end consumer.
...
To help address this problem, Redacto KMS includes a feature called _response
wrapping_. When requested, Redacto KMS can take the response it would have sent to an
...
an encryption key from Redacto KMS's keyring to decrypt the data.
...
- It provides _malfeasance detection_ by ensuring that only a single party can
  ...
  origin is from the expected location in Redacto KMS.
...
When a response is wrapped, the normal API response from Redacto KMS does not contain
...
- Wrapped Accessor: If the wrapped response is an authentication response
  containing a Redacto KMS token, this is the value of the wrapped token's accessor.
...
Redacto KMS currently does not provide signed response-wrapping tokens, as it
provides little extra protection. If you are being pointed to the correct Redacto KMS
...
and pointed to the wrong Redacto KMS server, the same attacker could trivially give
you the wrong signing public key that corresponds to the wrong Redacto KMS server.
You could cache a previously valid key, but could also cache a previously valid
address (and in most cases the Redacto KMS address will not change or will be set via
...
- **Wrap** (`sys/wrapping/wrap`)**:** A helper endpoint that echoes back the data sent
  ...
  can be done elsewhere in Redacto KMS.

## Response-Wrapping token creation

Response wrapping is per-request and is triggered by providing to Redacto KMS the
...
(`25h`). When using the Redacto KMS CLI, you can set this via the `-wrap-ttl`
...
2. Perform a lookup on the response-wrapping token. This immediately tells you
   ...
   an alert for immediate investigation, likely with the assistance of Redacto KMS's
