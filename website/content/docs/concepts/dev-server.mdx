...
description: >-
  The dev server in Redacto KMS can be used for development or to experiment with
  Redacto KMS.
---

# "Dev" server mode

You can start Redacto KMS as a server in "dev" mode like so: `redacto-kms server -dev`.
This dev-mode server requires no further setup, and your local `redacto-kms` CLI will
be authenticated to talk to it. This makes it easy to experiment with Redacto KMS or
start a Redacto KMS instance for development. Every feature of Redacto KMS is available in
...
- **Initialized and unsealed** - The server will be automatically initialized
  and unsealed. You don't need to use `redacto-kms operator unseal`. It is ready
  for use immediately.

- **In-memory storage** - All data is stored (encrypted) in-memory. Redacto KMS
...
- **Automatically Authenticated** - The server stores your root access
  token so `redacto-kms` CLI access is ready to go. If you are accessing Redacto KMS
...
- **Single unseal key** - The server is initialized with a single unseal
  key. The Redacto KMS is already unsealed, but if you want to experiment with
...
The dev server should be used for experimentation with Redacto KMS features, such
