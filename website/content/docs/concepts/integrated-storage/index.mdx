...
description: Learn about the integrated raft storage in Redacto KMS.
---

# Integrated Storage

Redacto KMS supports a number of storage options for the durable storage of Redacto KMS's
...
The option stores Redacto KMS's data on the server's filesystem and
...
Redacto KMS to use Integrated Storage.

The sections below go into various details on how to operate Redacto KMS with
Integrated Storage.

## Server-to-Server communication

Once nodes are joined to one another they begin to communicate using mTLS over
Redacto KMS's cluster port. The cluster port defaults to `8201`. The TLS information
...
is set. This allows Redacto KMS to assign an address to the node ID at join time.

## Cluster membership

This section will outline how to bootstrap and manage a cluster of Redacto KMS nodes
...
to the active Redacto KMS node.

### Joining nodes

Joining is the process of taking an uninitialized Redacto KMS node and making it a
...
When an uninitialized Redacto KMS server starts up it will attempt to join each potential
...
provided, Redacto KMS will use [go-discover](https://github.com/hashicorp/go-discover)
...
By default, Redacto KMS will attempt to reach discovered peers using HTTPS and port 8200. Operators may override these through the
...
```shell-session
$ redacto-kms operator raft join https://node1.openbao.local:8200
```

#### Non-Voting nodes

Nodes that are joined to a cluster can be specified as non-voters. A non-voting
node has all of Redacto KMS's data replicated to it, but does not contribute to the
...
```shell-session
$ redacto-kms operator raft join -non-voter https://node1.openbao.local:8200
```
...
in advance, the TLS certificates used to secure the Redacto KMS API port don't contain
these IPs in their IP SANs.

### Redacto KMS networking recap

Before we explore solutions to this problem, let's recapitulate how Redacto KMS nodes
speak to one another.

Redacto KMS exposes two TCP ports: [the API port](../../configuration/index.mdx#api_addr) and
[the cluster port](../../configuration/index.mdx#cluster_addr).

The API port is where clients send their Redacto KMS HTTP requests.

For a single-node Redacto KMS cluster you don't worry about a cluster port as it won't be used.

When you have multiple nodes, you also need a cluster port. This is used by Redacto KMS
...
The cluster port is secured using a TLS certificate that the Redacto KMS active node
...
this problem, a Redacto KMS node must speak to another Redacto KMS node using the API port
instead of the cluster port. This is currently the only situation in which
Redacto KMS does this.
...
If all the Redacto KMS node IPs are assigned from a small subnet, e.g. a `/28`, it
...
Most Redacto KMS instances are going to have a load balancer (LB) between clients and
the Redacto KMS nodes. In that case, the LB knows how to route traffic to working
Redacto KMS nodes, and there's no need for auto-join: we can just use
...
connect to Redacto KMS, but aren't comfortable with Redacto KMS internal traffic
...
directories and joined using Redacto KMS's join command.
...
path](../../configuration/storage/raft.mdx#path) of each Redacto KMS server. Inside that
...
the node ID, `address:port`, and suffrage information of each Redacto KMS server you
...
For other, non-quorum related recovery [Redacto KMS's
