---
description: >-
  Redacto KMS provides a lease with every secret. When this lease is expired, Redacto KMS
  will revoke that secret.
---

# Lease, renew, and revoke

With every dynamic secret and `service` type authentication token, Redacto KMS
creates a _lease_: metadata containing information such as a time duration,
renewability, and more. Redacto KMS promises that the data will be valid for the
given duration, or Time To Live (TTL). Once the lease is expired, Redacto KMS can
...
Redacto KMS routinely to either renew the lease (if allowed) or request a
replacement secret. This makes the Redacto KMS audit logs more valuable and
also makes key rolling a lot easier.

All dynamic secrets in Redacto KMS are required to have a lease. Even if the data is
...
Revocation can happen manually via the API, via the `redacto-kms lease revoke` cli command,
the user interface (UI) under the Access tab, or automatically by Redacto KMS. When a lease
is expired, Redacto KMS will automatically revoke that lease. When a token is revoked,
Redacto KMS will revoke all leases that were created using that token.
...
When reading a dynamic secret, such as via `redacto-kms read`, Redacto KMS always returns a
`lease_id`. This is the ID used with commands such as `redacto-kms lease renew` and `redacto-kms lease revoke` to manage the lease of the secret.
...
example, `redacto-kms lease renew -increment=3600 my-lease-id` would request that the TTL of the lease
...
example, to revoke all Userpass logins, you can do `redacto-kms lease revoke -prefix auth/userpass/`.
