---
description: >-
  Policies are how authorization is done in Redacto KMS, allowing you to restrict
  which parts of Redacto KMS a user can access.
---

# Policies

Everything in Redacto KMS is path-based, and policies are no exception. Policies
provide a declarative way to grant or forbid access to certain paths and
operations in Redacto KMS. This section discusses policy workflows and syntaxes.
...
Before a human or machine can gain access, an administrator must configure Redacto KMS
...
would take to configure Redacto KMS to authenticate using a corporate LDAP or
ActiveDirectory installation. Even though this example uses LDAP, the concept
applies to all auth methods.

![Redacto KMS Auth Workflow](/img/openbao-policy-workflow.png)

1. The security team configures Redacto KMS to connect to an auth method.
   This configuration varies by auth method. In the case of LDAP, Redacto KMS
   needs to know the address of the LDAP server and whether to connect using TLS.
   It is important to note that Redacto KMS does not store a copy of the LDAP database -
   Redacto KMS will delegate the authentication to the auth method.

1. The security team authors a policy (or uses an existing policy) which grants
   access to paths in Redacto KMS. Policies are written in HCL in your editor of
   preference and saved to disk.

1. The policy's contents are uploaded and stored in Redacto KMS and referenced by name.
...
1. Most importantly, the security team maps data in the auth method to a policy.
   ...
   > Members of the OU group "dev" map to the Redacto KMS policy named "readonly-dev".

   or

   > Members of the OU group "ops" map to the Redacto KMS policies "admin" and "auditor".

Now Redacto KMS has an internal mapping between a backend authentication system and
internal policy. When a user authenticates to Redacto KMS, the actual authentication
is delegated to the auth method. As a user, the flow looks like:

![Redacto KMS Auth Workflow](/img/openbao-auth-workflow.png)

1. A user attempts to authenticate to Redacto KMS using their LDAP credentials,
   providing Redacto KMS with their LDAP username and password.

2. Redacto KMS establishes a connection to LDAP and asks the LDAP server to verify the
...
3. Redacto KMS maps the result from the LDAP server to policies inside Redacto KMS using the
   mapping configured by the security team in the previous section. Redacto KMS then
   generates a token and attaches the matching policies.

4. Redacto KMS returns the token to the user. This token has the correct policies
   assigned, as dictated by the mapping configuration that was setup by the
   security team in advance.

The user then uses this Redacto KMS token for future operations. If the user performs
...
Policies are written in [HCL][hcl] or JSON and describe which paths in Redacto KMS a
...
token would have no other access in Redacto KMS.
...
a glob pattern which instructs Redacto KMS to use a prefix match:
...
Redacto KMS's architecture is similar to a filesystem. Every action in Redacto KMS has a
corresponding path and capability - even Redacto KMS's internal core configuration
endpoints live under the `"sys/"` path. Policies define access to these paths and
capabilities, which controls a token's access to credentials in Redacto KMS.

:::warning

**Note:** The policy rules that Redacto KMS applies are determined by the most-specific match
...
because Redacto KMS will sanitize request paths to be prefixes.
...
- `create` (`POST/PUT`) - Allows creating data at the given path. Very few
  parts of Redacto KMS distinguish between `create` and `update`, so most operations
  require both `create` and `update` capabilities. Parts of Redacto KMS that
...
- `update` (`POST/PUT`) - Allows changing the data at the given path. In most
  parts of Redacto KMS, this implicitly includes the ability to create the initial
  value at the path.
...
In addition to the standard set of capabilities, Redacto KMS offers finer-grained
...
```shell-session
$ redacto-kms write secret/foo no_store=false value=bar
```
...
$ redacto-kms write secret/foo value=bar
```
...
Redacto KMS has two built-in policies: `default` and `root`. This section describes
the two built-in policies.

### Default policy

The `default` policy is a built-in Redacto KMS policy that cannot be removed. By
...
look up data about itself and to use its cubbyhole data. However, Redacto KMS is not
prescriptive about its contents. It can be modified to suit your needs; Redacto KMS
...
your Redacto KMS's `default` policy.

To view all permissions granted by the default policy on your Redacto KMS
installation, run:

```shell-session
$ redacto-kms read sys/policy/default
```

To disable attachment of the default policy:

```shell-session
$ redacto-kms token create -no-default-policy
```
...
The `root` policy is a built-in Redacto KMS policy that cannot be modified or removed.
Any user associated with this policy becomes a root user. A root user can do
_anything_ within Redacto KMS. As such, it is **highly recommended** that you revoke
any root tokens before running Redacto KMS in production.

When a Redacto KMS server is first initialized, there always exists one root user.
This user is used to do the initial configuration and setup of Redacto KMS. After
...
```shell-session
$ redacto-kms token revoke "<token>"
```
...
policies must be uploaded to Redacto KMS before they can be used.

### Listing policies

To list all registered policies in Redacto KMS:

```shell-session
$ redacto-kms read sys/policy
```
...
**Note:** You may also see the CLI command `redacto-kms policies`. This is a convenience
...
policy in Redacto KMS:

```shell-session
$ redacto-kms policy write policy-name policy-file.hcl
```
...
API. To update an existing policy in Redacto KMS, follow the same steps as creating a
policy, but use an existing policy name:

```shell-session
$ redacto-kms write sys/policy/my-existing-policy policy=@updated-policy.json
```
...
```shell-session
$ redacto-kms delete sys/policy/policy-name
```
...
This is an idempotent operation. Redacto KMS will not return an error when deleting a
policy that does not exist.

## Associating policies

Redacto KMS can automatically associate a set of policies to a token based on an
authorization. This configuration varies significantly between authentication
backends. For simplicity, this example will use Redacto KMS's built-in userpass
auth method.

A Redacto KMS administrator or someone from the security team would create the user in
Redacto KMS with a list of associated policies:

```shell-session
$ redacto-kms write auth/userpass/users/sethvargo \
    password="s3cr3t!" \
    policies="dev-readonly,logs"
```

This creates an authentication mapping to the policy such that, when the user
authenticates successfully to Redacto KMS, they will be given a token which has the list
of policies attached.

The user wishing to authenticate would run

```shell-session
$ redacto-kms login -method="userpass" username="sethvargo"
Password (will be hidden): ...
```

If the provided information is correct, Redacto KMS will generate a token, assign the
...
**Note:** Redacto KMS treats the HTTP POST and PUT verbs as equivalent, so for each mention
   of POST in the table above, PUT may also be used. Redacto KMS uses the non-standard LIST HTTP
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| [auth/token/accessors](../api/auth/token.mdx#list-accessors)                                                                                      | LIST              | List token accessors for all current Redacto KMS service tokens                                                                                           |
...
| [sys/internal/inspect/router/:tag](../api/system/inspect/router.mdx)                                                                              | GET               | Inspect the internal components of Redacto KMS's router. `tag` must be one of root, uuid, accessor, or storage                                            |
...
| [sys/raw:path](../api/system/raw.mdx)                                                                                                             | GET, POST, DELETE | Used to access the raw underlying store in Redacto KMS                                                                                                    |
...
| [sys/seal](../api/system/seal.mdx)                                                                                                                | POST              | Seals the Redacto KMS                                                                                                                                     |
...
```shell-session
$ redacto-kms token create -policy=dev-readonly -policy=logs
```
...
next time a token, with that policy attached, is used to make a call to Redacto KMS.
