...
Redacto KMS supports moving secrets and auth mounts, both within and across namespaces. The operation
is supported by API endpoints and a CLI helper around them.

## Why

There can be several reasons for wanting to migrate mounts. In Redacto KMS, the use cases could be for
...
There are two primary cleanup operations that Redacto KMS operators will need to keep in mind after performing a remount.
...
the Redacto KMS operator should find and remove those aliases from the source namespace, and create equivalent aliases
...
Redacto KMS will dynamically generate them upon login operations.
...
which does not have a secrets engine called `my-secret/`, the Redacto KMS operator would either create an equivalent `my-secret/`
