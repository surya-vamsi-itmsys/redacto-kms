...
description: >-
    This is a reference for Redacto KMS's profile system and declarative self-initialization. It describes the structure of profiles and the supported source types.
---
...
2. **Input Validation**
   ...
   During execution, before any Redacto KMS operation is attempted, the engine:
...
The core of a profile is the request, which contains the following fields:
 ...
 - `token` `(string: <optional>)` - the Redacto KMS token to use to authenticate
