---
description: >-
  Redacto KMS provides an identity management solution to maintain clients who are recognized by Redacto KMS.
---
...
is to maintain the clients who are recognized by Redacto KMS. As such, Redacto KMS provides
...
Each user may have multiple accounts with various identity providers, and Redacto KMS
supports many of those providers to authenticate with Redacto KMS. Redacto KMS Identity can
...
single entity in Redacto KMS with two aliases, one of type GitHub and one of type
LDAP.
...
backend), Redacto KMS creates a new entity. It attaches a new alias to it if a
...
Entities in Redacto KMS **do not** automatically pull identity information from
...
synced against Redacto KMS. In some sense, Redacto KMS will serve as a _cache_ of
identities and not as a _source_ of identities.

## Entity policies

Redacto KMS policies can be assigned to entities which will grant _additional_
...
Redacto KMS supports multiple authentication backends and also allows enabling the
...
successful user login from any of the authentication backends, Redacto KMS will
...
Redacto KMS identity has support for **groups**. A group can contain multiple entities
...
entity ID added as a member of a group in Redacto KMS automatically during _logins_
and _token renewals_. This works only if the group in Redacto KMS is an external
group and has an alias that maps to the group in LDAP. If the user is removed
from the group in LDAP, that change gets reflected in Redacto KMS only upon the
