...
This quick start explains how operators (administrators) can deploy and initialize Redacto KMS.
At the end of this guide, you will have a simple Redacto KMS environment that you can use as a playground for learning.
You will also have connected your first application that can use Redacto KMS as a central secret storage.

## Step 1: Install Redacto KMS

Follow the [installation guide](../install.mdx) to install Redacto KMS on a machine of your choice.

## Step 2: Locate config file

Make sure you know where your Redacto KMS [config file](../configuration/index.mdx) is
located, so that you can edit it later when necessary.
For example: the official container images expect to find the configuration at `/openbao/config/redacto-kms.hcl`,
and the native Linux packages (DEB/RPM) place it at `/etc/openbao/openbao.hcl`.

If you are running Redacto KMS using a container, you typically provide a full configuration at `/openbao/config/redacto-kms.hcl`
(for example, using a Docker volume mount or a Kubernetes ConfigMap).

:::info

If you edit the config file while Redacto KMS is already running, you need to restart
Redacto KMS or send a SIGHUP signal to cause Redacto KMS to reload the config file.

:::

## Step 3: Configure listener

Configure where Redacto KMS should listen (Unix socket, TCP HTTP/HTTPS) using the
[`listener` stanza](../configuration/listener/index.mdx).
Edit your `redacto-kms.hcl` accordingly.

When using HTTPS, use your preferred CA/PKI to obtain a private key and certificate to use with Redacto KMS.
Redacto KMS [supports ACME](../configuration/listener/tcp.mdx#acme-parameters)
to automatically request certificates from services such as Let's Encrypt.

## Step 4: Enable audit device

[Audit devices](../audit/index.mdx) keep a log of all requests to Redacto KMS.
The Redacto KMS project strongly recommends that you enable at least one audit device.
...
Start Redacto KMS (or restart it if it was already running).
Then check Redacto KMS's status, to verify that the instance is up and that you can connect to the API.

From the command line, you can use the [`redacto-kms` tool](../commands/index.mdx) to
...
## Step 6: Initialize Redacto KMS

The [`operator` command](../commands/operator/index.mdx) of the [Redacto KMS CLI](../commands/index.mdx)
is the main interaction point for operators with their Redacto KMS instance.

Most importantly, you need to initialize the Redacto KMS instance before it can be used:
...
Redacto KMS's storage is encrypted at rest; therefore, Redacto KMS requires cryptographic
material each time that it starts: the [_unseal keys_](../concepts/seal.mdx).
When you initialize the instance, Redacto KMS generates a set of unseal keys.
Securely note down the unseal keys, as without them, you won't be able to decrypt the storage.

From now on, you will need to unseal Redacto KMS after every restart.
...
It is needed to authenticate to the Redacto KMS API, through which you will perform further configuration.

:::warning

These values are highly sensitive and grant access to your Redacto KMS instance!
...
```shell-session
$ redacto-kms write cubbyhole/my-secret my-value=s3cr3t
$ redacto-kms read cubbyhole/my-secret
```
...
Now that your Redacto KMS instance is running, connect your applications to it.
This involves the following steps:

1. [**Enable the secrets engine**](../secrets/index.mdx) that your application plans to use.
   Different types of engines are available, depending on the use case that you are deploying Redacto KMS for.

1. [**Define a policy**](../concepts/policies.mdx) stating
   which paths your application is allowed to access.

1. [**Choose an auth method**](../auth/index.mdx) to let client applications authenticate against Redacto KMS.
...
Your client applications should now be able to access Redacto KMS.
...
You now have a basic Redacto KMS instance running and applications connected.
...
Before deploying a production instance, the Redacto KMS projects recommends that you
...
- [**Concepts:**](../concepts/index.mdx)
  Learn how Redacto KMS works and how its concepts impact your day-to-day usage and operation.

- [**High availability:**](../commands/operator/raft.mdx)
  Redacto KMS supports high availability via the integrated storage backend.
  You can add additional Redacto KMS instances to the cluster using `operator raft join`.
  Note: Only the first instance must be initialized with a seal.

- [**Plugins:**](../plugins/index.mdx)
  Plugins extend the core Redacto KMS functionality. The main plugin categories are
  auth methods, secret engines, database providers, and KMS providers.

- [**Sealing:**](../configuration/seal/index.mdx)
  Redacto KMS can be unsealed with different methods, for example, with keys stored in a KMS or HSM.
