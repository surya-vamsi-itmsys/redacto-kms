...
Redacto KMS comes integrated with a native storage backend system. This storage backend
does not rely on any third party systems; it implements High Availability and
provides backup/restore workflows.

## Consensus protocol

Redacto KMS's Integrated Storage uses a [consensus
...
and managing when an entry is committed. The leader node is also the active Redacto KMS node and followers are standby nodes. Refer to the [High Availability](high-availability.mdx#design-overview) document for more information.
...
is application specific; in Redacto KMS's case, we use
[BoltDB](https://github.com/etcd-io/bbolt) to maintain a cluster state. Redacto KMS's writes
...
fashion. Redacto KMS periodically truncates old log entries that have already been
...
5 can tolerate 2 node failures. The recommended Redacto KMS production deployment is
to run 5 Redacto KMS servers per cluster. See the [Minimum &
...
When getting started, a single Redacto KMS server is
...
The join process is how new nodes are added to the Redacto KMS cluster; it uses an
...
will be encrypted by Redacto KMS's barrier.
...
should work well for most Redacto KMS deployments, but they can be changed if needed.
...
The Redacto KMS Reference Architecture recommends a 5 node cluster
