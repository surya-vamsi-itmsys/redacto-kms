---
description: Learn about the security model of Redacto KMS.
---

# Security model

Due to the nature of Redacto KMS and the confidentiality of data it manages,
the Redacto KMS security model is very critical. The overall goal of Redacto KMS's security
...
The following are the various parts of the Redacto KMS threat model:

- **Eavesdropping on any Redacto KMS communication.** Client communication with Redacto KMS
  should be secure from eavesdropping as well as communication from Redacto KMS to
  its storage backend or between Redacto KMS cluster nodes.

- **Tampering with data at rest or in transit.** Any tampering should be detectable
  and cause Redacto KMS to abort processing of the transaction.
...
- **Confidentiality of stored secrets.** Any data that leaves Redacto KMS to rest in the
  storage backend must be safe from eavesdropping. In practice, this means all
  data at rest must be encrypted.

- **Availability of secret material in the face of failure.** Redacto KMS supports
  running in a highly available configuration to avoid loss of availability.

### Out of Scope

The following are not considered part of the Redacto KMS threat model:
...
- **Protecting against arbitrary control of the storage backend.** An attacker
  ...
  of the storage backend causing total data loss for Redacto KMS. The ability to control
...
- **Protecting against memory analysis of a running Redacto KMS.** If an attacker is able
  to inspect the memory state of a running Redacto KMS instance, then the confidentiality
  of data may be compromised.

- **Protecting against flaws in external systems or services used by Redacto KMS.**
  Some authentication methods or secrets engines delegate sensitive operations to
  systems external to Redacto KMS. If an attacker can compromise credentials or otherwise
  ...
  is considered out of scope for the Redacto KMS threat model.
...
- **Protecting against flaws in clients or systems that access Redacto KMS.** If an attacker
  can compromise a Redacto KMS client (e.g., system, browser) and obtain this client’s Redacto KMS
  credentials, they can access Redacto KMS with the level of privilege associated with this
  client.

- **Protecting against Redacto KMS administrators supplying vulnerable or malicious configuration
  data.** Any data provided as configuration values to Redacto KMS's administrative endpoints
  (e.g. [secret engines](../secrets/index.mdx) configurations), or Redacto KMS's
  configuration files should be validated. If an attacker can write to Redacto KMS's
...
- **Protecting against fingerprinting**. Traffic patterns between clients,
  ...
  - **Namespace existence**. The existence of namespaces can often be inferred
    ...
    fingerprinting the service as being a Redacto KMS instance.

## External threat overview

Redacto KMS architecture compromises of three distinct systems:

- Client: Speaks to Redacto KMS over an API.
- Server: Provides an API and serves requests.
- Storage backend: Utilized by the server to read and write data.


There is no mutual trust between the Redacto KMS client and server. Clients use
...
All server-to-server traffic between Redacto KMS instances within a cluster uses
...
[unseal challenge](../concepts/integrated-storage/index.mdx#redacto-kms-networking-recap).

The storage backends used by Redacto KMS are also untrusted by design. Redacto KMS uses a
security barrier for all requests made to the backend. The security barrier
automatically encrypts all data leaving Redacto KMS using a 256-bit [Advanced
Encryption Standard
...
Depending on the backend used, Redacto KMS may communicate with the backend over TLS
...
Within the Redacto KMS system, a critical security concern is an attacker attempting
to gain access to secret material they are not authorized to. This is an internal
threat if the attacker is already permitted to some level of access to Redacto KMS, and is
able to authenticate.

When a client first authenticates with Redacto KMS, an auth method is used to verify
the identity of the client and to return a list of associated ACL policies.
This association is configured by operators of Redacto KMS ahead of time. For
example, GitHub users in the "engineering" team may be mapped to the
"engineering" and "ops" Redacto KMS policies. Redacto KMS then generates a client token
...
On each request, a client provides this token. Redacto KMS then uses it to check that
the token is valid and has not been revoked or expired, and generates an ACL
based on the associated policies. Redacto KMS uses a strict default deny
...
Redacto KMS. When the policies are merged (if multiple policies are associated with a
...
policy built into Redacto KMS. This is similar to the concept of a root user on a
Unix system or an administrator on Windows. In cases where clients are provided
with root tokens or associated with the root policy, Redacto KMS supports the
...
operations without being granted global root access to Redacto KMS.

Lastly, Redacto KMS supports using a [Two-person
rule](https://en.wikipedia.org/wiki/Two-person_rule) for unsealing using [Shamir's
Secret Sharing
technique](https://en.wikipedia.org/wiki/Shamir's_Secret_Sharing). When Redacto KMS
...
malicious attacker with access to it can decrypt the entire Redacto KMS. Instead,
...
default Redacto KMS generates 5 shares, any 3 of which must be provided to
...
useful for making any requests to Redacto KMS, and can only be used for unsealing.
...
practical entrance. The analogy to Redacto KMS is that the cryptosystem is the
...
combination. Similarly, Redacto KMS requires multiple shares be provided to
reconstruct the root key. Once unsealed, each security deposit boxes still
requires that the owner provide a key, and similarly the Redacto KMS ACL system protects
