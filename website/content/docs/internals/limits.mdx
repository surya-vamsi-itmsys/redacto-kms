...
description: Learn about the maximum number of objects within Redacto KMS.
---

# Redacto KMS limits and maximums

Redacto KMS imposes fixed upper limits on the size of certain fields and
objects, and configurable limits on others. Redacto KMS also has upper
bounds that are a consequence of its underlying storage. This page
attempts to collect these limits, to assist in planning Redacto KMS
deployments.
...
Many of the other limits within Redacto KMS derive from the maximum size of
...
size by reconfiguring Redacto KMS to a larger maximum storage
...
Since [Redacto KMS v2.1.0](/community/release-notes/2-1-0#features), the mount,
...
Redacto KMS shards the entities across 256 storage entries. This creates a hard
...
The number of entities can be monitored using Redacto KMS's [telemetry](telemetry/metrics/authn.mdx); see `vault.identity.num_entities` (total) or `vault.identity.entities.count` (by namespace).
...
Each time a token is used, Redacto KMS must assemble the collection of
...
Very large numbers of policies are possible, but can cause Redacto KMS’s
...
user-provided key-value pairs. Redacto KMS imposes the following custom metadata limits:
...
The maximum size of a Transit ciphertext or plaintext is limited by Redacto KMS's
...
The maximum size of an HTTP request sent to Redacto KMS is limited by
...
The maximum duration of a Redacto KMS operation is
...
operation on a remote service, the Redacto KMS client will see a failure.

The environment variable [`REDACTO_KMS_CLIENT_TIMEOUT`](../commands/index.mdx#redacto_kms_client_timeout) sets a client-side maximum duration as well,
...
initiated by Redacto KMS that communicates over RPC. For each secret engine and auth
method that's enabled as an external plugin, Redacto KMS will spawn a process on the
