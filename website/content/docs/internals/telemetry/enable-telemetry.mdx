...
description: >-
  Step-by-step guide to enabling telemetry gathering with Redacto KMS
---

# Enable Redacto KMS telemetry gathering

Collect telemetry data from your Redacto KMS installation.

## Before you start

- **You must have access to your [Redacto KMS configuration](../../configuration/index.mdx) file**.
...
with the `redacto-kms audit enable` command. For example, to enable auditing for the
...
To configure telemetry collection, update the telemetry stanza in your Redacto KMS
configuration with your collection preferences and aggregation agent details.

For example, the following `telemetry` stanza configures Redacto KMS with the standard
...
Popular reporting solutions compatible with Redacto KMS:
