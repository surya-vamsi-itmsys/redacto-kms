...
description: |-
  Learn about the telemetry data available in Redacto KMS
---

# Telemetry

The Redacto KMS server process collects various runtime metrics about the performance
...
Telemetry from Redacto KMS must be streamed and stored in metrics aggregation
software to monitor Redacto KMS and collect durable metrics.

@include 'telemetry/supported-aggregation-agents.mdx'

## Working with raw telemetry data

You can view raw telemetry data for debugging purposes by interrupting the Redacto KMS
process with `USR1` (on *nix) or `BREAK` (on Windows). When the Redacto KMS process
