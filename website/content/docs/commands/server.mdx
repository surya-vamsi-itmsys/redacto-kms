---
description: |-
  The "server" command starts a Redacto KMS server that responds to API requests. By
  default, Redacto KMS will start in a "sealed" state. The Redacto KMS cluster must be
...
The `server` command starts a Redacto KMS server that responds to API requests. By
default, Redacto KMS will start in a "sealed" state. The Redacto KMS cluster must be
initialized before use, usually by the `redacto-kms operator init` command. Each Redacto KMS
server must also be unsealed using the `redacto-kms operator unseal` command or the
API before the server can respond to requests.

For more information, please see:

- [`operator init` command](operator/init.mdx) for information
  on initializing a Redacto KMS server.

- [`operator unseal` command](operator/unseal.mdx) for
  information on providing unseal keys.

- [Redacto KMS configuration](../configuration/index.mdx) for the syntax and
  various configuration options for a Redacto KMS server.
...
```shell-session
$ redacto-kms server -config=/etc/openbao/config.hcl
```
...
- `-config` `(string: "")` - Path to a configuration file or directory of
  ...
  The environment variable `REDACTO_KMS_CONFIG_PATH` may also be used to specify multiple
...
- `-log-file` <a name="_log_file"></a> - the absolute path where Redacto KMS should save log
  ...
  log file rotates, Redacto KMS appends the current timestamp to the file name
...
- `REDACTO_KMS_ALLOW_PENDING_REMOVAL_MOUNTS` `(bool: false)` <a name="bao_allow_pending_removal_mounts"></a> - (environment variable)
  Allow Redacto KMS to be started with builtin engines which have the "Pending Removal"
  ...
  the next major release of Redacto KMS), the environment variable will no longer work
...
- `-dev` `(bool: false)` - Enable development mode. In this mode, Redacto KMS runs
  in-memory and starts unsealed. As the name implies, do not run "dev" mode in
  production.

- `-dev-tls` `(bool: false)` - Enable TLS development mode. In this mode, Redacto KMS runs
...
- `-dev-listen-address` `(string: "127.0.0.1:8200")` - Address to bind to in
  "dev" mode. This can also be specified via the `REDACTO_KMS_DEV_LISTEN_ADDRESS`
...
- `-dev-root-token-id` `(string: "")` - Initial root token. This only applies
  ...
  `REDACTO_KMS_DEV_ROOT_TOKEN_ID` environment variable.
