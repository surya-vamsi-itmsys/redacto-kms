---
description: |-
  The "path-help" command retrieves API help for paths. All endpoints in Redacto KMS
...
in Redacto KMS, and also allows you to discover new paths.

Before using `path-help`, it is important to understand "paths" within Redacto KMS.
Paths are the parameters used for `redacto-kms read`, `redacto-kms write`, etc. An example
...
To discover what paths are supported, use `redacto-kms path-help PATH`. For example,
if you enabled the LDAP secrets engine, you can use `redacto-kms path-help ldap` to find
...
Once you've found a path you like, you can learn more about it by using `redacto-kms path-help <path>` where "path" is a path that matches one of the regular
expressions from the backend help.

```shell-session
$ redacto-kms path-help secret/password
Request:        password
Matching Route: ^.*$
...
when the secret is read, Redacto KMS will report a lease with that duration. It
