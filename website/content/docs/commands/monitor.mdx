---
description: |-
  The "monitor" command displays the server logs of a Redacto KMS server in real
  time.
---

# monitor

The `monitor` command shows a real time display of the server logs of a Redacto KMS
server. This command accepts a log level as an argument, which can be different
from the log level that the Redacto KMS server was started with.

The `monitor` command honors the `REDACTO_KMS_ADDR` environment variable. The address
...
If Redacto KMS is emitting log messages faster than a receiver can process them, the
...
```shell-session
$ redacto-kms monitor -log-level=debug
```
...
- `-log-level` `(string: "info")` - Monitor the Redacto KMS server at this log level.
