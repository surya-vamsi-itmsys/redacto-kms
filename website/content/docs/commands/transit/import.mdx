...
```
$ redacto-kms transit import-version transit/keys/test-key @test-key-updated
Retrieving transit wrapping key.
Wrapping source key with ephemeral key.
Encrypting ephemeral key with transit wrapping key.
Submitting wrapped key to Redacto KMS transit.
...
This command does not have any unique flags and respects core Redacto KMS CLI
commands. See `redacto-kms transit import -help` for more information.
