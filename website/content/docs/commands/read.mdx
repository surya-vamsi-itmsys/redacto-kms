---
description: |-
  The "read" command reads data from Redacto KMS at the given path. This can be used
...
The `read` command reads data from Redacto KMS at the given path (wrapper command for
...
```shell-session
$ redacto-kms read identity/entity/id/2f09126d-d161-abb8-2241-555886491d97
```
...
```shell-session
$ redacto-kms read secret/data/customers
```

This is equivalent to:

```shell-session
$ curl --request GET --header "X-Vault-Token: $REDACTO_KMS_TOKEN" \
    $REDACTO_KMS_ADDR/v1/secret/data/customers
```

Since K/V secrets engine is a commonly used feature, Redacto KMS CLI provides the
[`kv`](kv/index.mdx) command. Read secrets from the `secret/data/customers`
path using the `kv` CLI command:

```shell-session
$ redacto-kms kv get -mount=secret customers
```

:::info

**Comparison:** All three commands retrieve the same data, but display the
output in a different format. By default, `redacto-kms read` prints output in
...
- `-format` `(string: "table")` - Print the output in the given format. Valid
  ...
  via the `REDACTO_KMS_FORMAT` environment variable.
