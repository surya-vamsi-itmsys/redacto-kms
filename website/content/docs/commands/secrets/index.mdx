...
description: |-
  The "secrets" command groups subcommands for interacting with Redacto KMS's secrets
  engines.
---
...
```shell-session
$ redacto-kms secrets move database/ db-prod/
...
```shell-session
$ redacto-kms secrets tune -max-lease-ttl=30m db-prod/
...
```shell-session
$ redacto-kms secrets disable db-prod/
