---
description: |-
  The "write" command writes data to Redacto KMS at the given path. The data can be
...
The `write` command writes data to Redacto KMS at the given path (wrapper command for
...
with an "**@**", then it is loaded from a file. If the value for a key is "**-**", Redacto KMS
...
This causes `redacto-kms write` to read a JSON blob containing all request parameters
...
```shell-session
$ redacto-kms write cubbyhole/git-credentials username="student01" password="p@$$w0rd"
```

Create a new encryption key in the transit secrets engine:

```shell-session
$ redacto-kms write -force transit/keys/my-key
```
...
```shell-session
$ redacto-kms write auth/token/create policies="admin" policies="secops" ttl=8h num_uses=3
```
...
$ curl --header "X-Vault-Token: $REDACTO_KMS_TOKEN" \
    --request POST \
    --data @request_payload.json \
    $REDACTO_KMS_ADDR/v1/auth/token/create
```

The `redacto-kms write` command simplifies the API call.

Since token management is a common task, Redacto KMS CLI provides a
[`token`](token/index.mdx) command with
[`create`](token/create.mdx) subcommand. The CLI command simplifies
the token creation. Use the `redacto-kms create` command with options to set the token
TTL, policies, and use limit.

```shell-session
$ redacto-kms token create -policy=admin -policy=secops -ttl=8h -use-limit=3
```
