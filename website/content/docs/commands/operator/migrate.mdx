...
description: >-
  ...
  migrating Redacto KMS between configurations. It operates directly at the storage
...
This is intended to be an offline operation to ensure data consistency, and Redacto KMS
...
```shell-session
$ redacto-kms operator migrate -config migrate.hcl -start "data/logical/fd"
```

Alternatively, you can specify the configuration path using the `REDACTO_KMS_MIGRATE_CONFIG_PATH` environment variable. If the `-config` flag is provided, the environment variable is ignored.

**Example using environment variable:**

```shell-session
$ export REDACTO_KMS_MIGRATE_CONFIG_PATH=/etc/openbao/migrate.hcl
$ redacto-kms operator migrate -start "data/logical/fd"
```
...
to that used to [configure Redacto KMS](../../configuration/storage/index.mdx),
...
Redacto KMS will need to be offline during the migration process. First, stop Redacto KMS.
Then, run the migration on the server you wish to become a the new Redacto KMS node.
...
use the new storage backend with Redacto KMS, update Redacto KMS's configuration file as
...
- `-reset` - Reset the migration lock. A lock file is added during migration to prevent
  starting the Redacto KMS server or another migration. The `-reset` option can be used to
