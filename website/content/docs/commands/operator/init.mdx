...
sidebar_label: init
description: |-
  The "operator init" command initializes a Redacto KMS server. Initialization is the
  process by which Redacto KMS's storage backend is prepared to receive data. Since
  Redacto KMS servers share the same storage backend in HA mode, you only need to
  initialize one Redacto KMS to initialize the storage backend.
---
...
Redacto KMS servers share the same storage backend in HA mode, you only need to
initialize one Redacto KMS to initialize the storage backend.
This command cannot be run against already-initialized Redacto KMS cluster.

During initialization, Redacto KMS generates a root key, which is stored in the storage backend alongside all other Redacto KMS data. The root key itself is encrypted and requires an _unseal key_ to decrypt it.

The default Redacto KMS configuration uses [Shamir's Secret Sharing](https://en.wikipedia.org/wiki/Shamir%27s_Secret_Sharing) to split the root key into a configured number of shards (referred as key shares, or unseal keys). A certain threshold of shards is required to reconstruct the root key, which is then used to decrypt the Redacto KMS's encryption key.
...
- `-format` `(string: "")` - Print the output in the given format. Valid formats
  ...
  specified via the `REDACTO_KMS_FORMAT` environment variable.
...
- `-status` `(bool: false)` - Print the current initialization status. An exit
  code of 0 means Redacto KMS is already initialized. An exit code of 1 means an
  error occurred. An exit code of 2 means Redacto KMS is not initialized.

### Consul options

- `-consul-auto` `(bool: false)` - Perform automatic service discovery using
  Consul in HA mode. When all nodes in a Redacto KMS HA cluster are registered with
  ...
  variables are set (CONSUL_HTTP_ADDR, etc). When only one Redacto KMS server is
  discovered, it will be initialized automatically. When more than one Redacto KMS
...
- `-consul-service` `(string: "openbao")` - Name of the service in Consul under
  which Redacto KMS servers are registered.
