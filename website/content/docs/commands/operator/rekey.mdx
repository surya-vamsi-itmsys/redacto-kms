...
description: |-
  ...
  downtime, but it requires the Redacto KMS is unsealed and a quorum of existing
...
but it requires the Redacto KMS is unsealed and a quorum of existing unseal keys are
provided.
...
Initialize a rekey when Auto Unseal is used for the Redacto KMS cluster:
...
Rekey an Auto Unseal Redacto KMS and encrypt the resulting recovery keys with PGP:
...
```shell-session
$ redacto-kms operator rekey -verify -nonce="..."
```
...
- `-backup` `(bool: false)` - Store a backup of the current PGP encrypted unseal
  keys in Redacto KMS's core. The encrypted values can be recovered in the event of
