...
description: |-
  ...
  downtime, but it requires the Redacto KMS instance to be unsealed and a quorum
...
Initialize a rotation when Auto Unseal is used for the Redacto KMS cluster:
...
Rotate an Auto Unseal Redacto KMS and encrypt the resulting recovery keys with PGP:
...
```shell-session
$ redacto-kms operator rotate-keys -verify -nonce="..."
```
