...
description: |-
  "redacto-kms operator diagnose" is a new operator-centric command, focused on providing a clear description
  of what is working in Redacto KMS, and what is not working. The command focuses on why Redacto KMS cannot serve requests,
...
The `operator diagnose` command should be used primarily when Redacto KMS is down or
partially inoperational. The command can be used safely regardless of the state
Redacto KMS is in, but may return meaningless results for some of the test cases if the
Redacto KMS server is already running.
...
- `-format` `(string: "table")` - Print the output in the given format. Valid
  formats are "table" or "json". This can also be specified via the `REDACTO_KMS_FORMAT`
...
issues to look into that may relate to the issues Redacto KMS is experiencing. Diagnose warns
...
- `-config` `(string: "")` - The paths to the Redacto KMS configuration files used by
the Redacto KMS server on startup. See the [server `-config` option](../../server#command-options) for more details.
...
#### Redacto KMS diagnose

`Redacto KMS Diagnose` is the top level check that contains the rest of the checks. It will report the status
of the check

#### Check operating system / check open file limit

`Check Open File Limit` verifies that the open file limit value is set high enough for Redacto KMS
...
`Parse Configuration` will check the Redacto KMS server config file for syntax errors. It will check
...
`Create Storage Backend` ensures that the storage stanza configured in the Redacto KMS server config
...
`Check Raft Folder Ownership` ensures that Redacto KMS does not need to run as root to access the boltDB folder.
...
`Check Consul Direct Service Discovery` is a consul-specific check that ensures Redacto KMS
is not accessing the consul server directly, but rather through a local agent.

#### Create Redacto KMS server configuration seals

`Create Redacto KMS Server Configuration Seals` creates seals from the Redacto KMS configuration
...
`Initialize Randomness for Core` ensures that Redacto KMS has access to the randReader that
the Redacto KMS core uses.
...
`ha_storage` section of the Redacto KMS configuration, as opposed to the `storage` section.

#### Determine redirect address

Ensures that one of the `REDACTO_KMS_API_ADDR`, `REDACTO_KMS_REDIRECT_ADDR`, or `REDACTO_KMS_ADVERTISE_ADDR`
environment variables are set, or that the redirect address is specified in the Redacto KMS
configuration.

#### Check cluster address

Parses the cluster address from the `REDACTO_KMS_CLUSTER_ADDR` environment variable, or from the
redirect address or cluster address specified in the Redacto KMS configuration, and checks that
the address is of the form `host:port`.

#### Check core creation

`Check Core Creation` verifies the logical configuration checks that Redacto KMS does when it
creates a core object. These are runtime checks, meaning any errors thrown by this diagnose
test will also be thrown by the Redacto KMS server itself when it is run.
...
that the minimum and maximum TLS versions are within the bounds of what Redacto KMS supports.
