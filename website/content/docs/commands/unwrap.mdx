---
description: |-
  The "unwrap" command unwraps a wrapped secret from Redacto KMS by the given token.
  The result is the same as the "redacto-kms read" operation on the non-wrapped
...
The `unwrap` command unwraps a wrapped secret from Redacto KMS by the given token. The
result is the same as the "redacto-kms read" operation on the non-wrapped secret. If
