...
```shell-session
$ redacto-kms read sys/auth/github/tune
Key                  Value
---                  -----
...
```shell-session
$ redacto-kms auth tune -default-lease-ttl=-1 github/
Success! Tuned the auth method at: github/
```
...
```shell-session
$ redacto-kms auth tune -audit-non-hmac-request-keys=value1 -audit-non-hmac-request-keys=value2 github/
Success! Tuned the auth method at: github/
```
...
```shell-session
$ redacto-kms auth tune -user-lockout-threshold=10  -user-lockout-duration=10m userpass/
...
$ redacto-kms read sys/auth/userpass/tune
...
- `-default-lease-ttl` `(duration: "")` - The default lease TTL for this auth
  method. If unspecified, this defaults to the Redacto KMS server's globally
