...
the mount and `/sys` against the given Redacto KMS server
...
```shell-session
$ redacto-kms pki health-check -list pki-root/
```
...
1. Delete any expired CAs with one of the following options:
  - Run [tidy](../../api/secret/pki.mdx#tidy) manually with `redacto-kms write <mount>/tidy tidy_expired_issuers=true`.
  - Use the Redacto KMS API to call [delete issuer](../../api/secret/pki.mdx#delete-issuer).
...
Use `redacto-kms write` to enable CRL auto-rebuild:

```shell-session
$ redacto-kms write <mount>/config/crl auto_rebuild=true
```
...
1. Add the roles that allow glob domains and wildcards to `allowed_roles` so
   Redacto KMS ignores them in future checks.
...
Redacto KMS will provide warnings and performance will suffer if you have a large
...
Use `redacto-kms secrets tune` to set the desired audit parameters:

```shell-session
$ redacto-kms secrets tune \
...
1. Update `allowed_response_headers` and `passthrough_request_headers` for all
   policies with `redacto-kms secrets tune`:
...
1. Update ACME-specific headers with `redacto-kms secrets tune` (if you are using ACME):
...
Use `redacto-kms write` to enable auto-tidy with the recommended defaults:

```shell-session
$ redacto-kms write <mount>/config/auto-tidy \
    enabled=true \
...
This health check verifies that tidy has run within the last run window. This can be critical/warning alerts as this can start to seriously impact Redacto KMS's performance.

**Remediation steps**:

1. Schedule a manual run of tidy with `redacto-kms write`:

  ```shell-session
  $ redacto-kms write <mount>/tidy \
      tidy_cert_store=true \
      tidy_revoked_certs=true \
      tidy_acme=true \
      tidy_revocation_queue=true \
...
1. Review the tidy status endpoint, `redacto-kms read <mount>/tidy-status` for
...
1. Verify that tidy ran recently with `redacto-kms read`:
    ```shell-session
    $ redacto-kms read <mount>/tidy-status
    ````
1. Schedule a manual run of tidy with `redacto-kms write`:
    ```shell-session
    $ redacto-kms write <mount>/tidy \
...
API documentation to learn about enabling ACME support in Redacto KMS.
...
Use `redacto-kms secrets tune` to add the missing headers to `allowed_response_headers`:
```shell-session
$ redacto-kms secrets tune \
  -allowed-response-headers="Last-Modified" \
  -allowed-response-headers="Replay-Nonce" \
