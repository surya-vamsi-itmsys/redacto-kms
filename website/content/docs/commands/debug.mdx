---
description: |-
  The "debug" command monitors a Redacto KMS server, probing information about
  it for a certain duration.
---

# debug

The `debug` command starts a process that monitors a Redacto KMS server, probing
information about it for a certain duration.

Gathering information about the state of the Redacto KMS cluster often requires the
...
variables `REDACTO_KMS_TOKEN` and `REDACTO_KMS_ADDR`. The token used determines the
...
```shell-session
$ redacto-kms debug -duration=1m -interval=10s -metrics-interval=5s -compress=false
```
