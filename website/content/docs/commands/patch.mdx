---
description: |-
  The "patch" command updates data in Redacto KMS at the given path. The data can be
...
The `patch` command updates data in Redacto KMS at the given path (wrapper command for
...
This causes `redacto-kms patch` to read a JSON blob containing all request parameters
...
$ curl --header "X-Vault-Token: $REDACTO_KMS_TOKEN" \
    ...
    $REDACTO_KMS_ADDR/v1/pki/roles/example
```

The `redacto-kms patch` command simplifies the API call.
