---
description: |-
  The "login" command authenticates users or machines to Redacto KMS using the
  provided arguments. A successful authentication results in a Redacto KMS token -
  conceptually similar to a session token on a website.
---

# login

The `login` command authenticates users or machines to Redacto KMS using the provided
arguments. A successful authentication results in a Redacto KMS token - conceptually
...
auth method, use the "redacto-kms auth help TYPE" command. You can also use "redacto-kms
...
is already stored in the token helper. You do NOT need to run "redacto-kms login"
again. Future Redacto KMS requests will automatically use this token.
...
```shell-session
$ redacto-kms login s.3jnbMAKl1i4YS3QoKdbHzGXq
Success! You are now authenticated. The token information displayed below
is already stored in the token helper. You do NOT need to run "redacto-kms login"
again. Future Redacto KMS requests will automatically use this token.
...
stored in the token helper. You do NOT need to run "redacto-kms login" again. Future
...
- `-path` `(string: "")` - Remote path in Redacto KMS where the auth method
