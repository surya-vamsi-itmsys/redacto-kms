...
description: |-
  The "kv" command groups subcommands for interacting with Redacto KMS's key/value
  secret engine.
---
...
The path to where the secrets engine is mounted can be indicated with the `-mount` flag, such as `redacto-kms kv get -mount=secret creds`.

The deprecated path-like syntax can also be used (e.g. `redacto-kms kv get secret/creds`), but this should be avoided
...
```shell-session
$ redacto-kms kv put -mount=secret creds passcode=my-long-passcode
== Secret Path ==
secret/data/creds
...
```shell-session
$ redacto-kms kv get -mount=secret creds
== Secret Path ==
secret/data/creds
...
```shell-session
$ redacto-kms kv metadata get -mount=secret creds
=== Metadata Path ===
...
```shell-session
$ redacto-kms kv get -mount=secret -version=1 creds
== Secret Path ==
secret/data/creds
...
Subcommands:
    ...
    metadata             Interact with Redacto KMS's Key-Value storage
