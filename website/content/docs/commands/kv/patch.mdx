...
```shell-session
$ redacto-kms kv patch -mount=secret creds ttl=48h
== Secret Path ==
secret/data/creds
...
```shell-session
$ redacto-kms kv put -mount=secret creds ttl=48h passcode=my-long-passcode
```
...
```shell-session
$ redacto-kms kv patch -mount=secret creds @data.json
```
