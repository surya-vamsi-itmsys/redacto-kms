...
description: |-
  ...
  that it can be used before it is revoked by Redacto KMS.
---

# lease renew

The `lease renew` command renews the lease on a secret, extending the time that
it can be used before it is revoked by Redacto KMS.

Every secret in Redacto KMS has a lease associated with it. If the owner of the secret
...
- `-increment` `(duration: "")` - Request a specific increment in seconds. Redacto KMS
