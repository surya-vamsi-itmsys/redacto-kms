...
```shell-session
$ redacto-kms audit enable socket
```

Supply configuration parameters via K=V pairs:

```shell-session
$ redacto-kms audit enable socket address=127.0.0.1:9090 socket_type=tcp
```
...
- `socket_type` `(string: "tcp")` - The socket type to use, any type compatible
  ...
  Redacto KMS may become unresponsive per [Blocked Audit Devices](index.mdx#blocked-audit-devices).
