...
and should not be enabled if any standby Redacto KMS instances do not support it.
...
directly to `syslog` will not result in Redacto KMS being blocked.
...
```shell-session
$ redacto-kms audit enable syslog
```

Supply configuration parameters via K=V pairs:

```shell-session
$ redacto-kms audit enable syslog tag="openbao" facility="AUTH"
```
