...
Sending a `SIGHUP` to the Redacto KMS process will cause `file` audit devices to close
...
```shell-session
$ redacto-kms audit enable -path="openbao_audit_1" file file_path=/home/user/openbao_audit.log
```

Enable logs on stdout. This is useful when running in a container:

```shell-session
$ redacto-kms audit enable file file_path=stdout
```

## Configuration

Note the difference between `audit enable` command options and the `file` backend
configuration options. Use `redacto-kms audit enable -help` to see the command options.
...
- `mode` `(string: "0600")` - A string containing an octal number representing
  ...
  prevent Redacto KMS from modifying the file mode.

  :::warning

  Note: Starting with Redacto KMS v2.4.0, any executable bits set in `mode` are
...
To properly rotate Redacto KMS File Audit Device log files on BSD, Darwin, or Linux-based Redacto KMS servers, it is important that you configure your log rotation software to send the `redacto-kms` process a signal hang up / `SIGHUP` after each rotation of the log file.
