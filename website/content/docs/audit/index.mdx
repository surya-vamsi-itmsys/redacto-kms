...
description: Audit devices are mountable devices that log requests and responses in Redacto KMS.
---

# Audit devices

Audit devices are the components in Redacto KMS that collectively keep a detailed log of all
requests to Redacto KMS, and their responses. Because every operation with Redacto KMS is an API
request/response, when using a single audit device, the audit log contains _every_ interaction with
the Redacto KMS API, including errors - except for a few paths which do not go via the audit system.
...
When multiple audit devices are enabled, Redacto KMS will attempt to send the audit
...
Redacto KMS considers a request to be successful if it can log to *at least* one
...
Note: It is **highly recommended** that you configure Redacto KMS to use multiple audit
devices. Audit failures can prevent Redacto KMS from servicing requests, so it is
...
interaction with Redacto KMS. The request and response can be matched utilizing a
...
inside all JSON sent to Redacto KMS (i.e., that integer values are provided in quotes).

:::

While most strings are hashed, Redacto KMS does make some exceptions, such as auth and secrets, and users can enable additional exceptions using the [secrets enable](../commands/secrets/enable.mdx) command, and then tune it afterward.
...
When a Redacto KMS server is first initialized, no auditing is enabled. Audit
devices must be enabled by a root user using [`redacto-kms audit enable`](../commands/audit/index.mdx)
...
```shell-session
$ redacto-kms audit enable file file_path=/var/log/openbao_audit.log
```
...
[server configuration](../configuration/index.mdx) starting with Redacto KMS v2.3.2.
...
cluster(s) will be able to successfully log to the audit device to avoid Redacto KMS
...
Audit device logs are critically important and ignoring auditing failures opens an avenue for attack. Redacto KMS will not respond to requests when no enabled audit devices can record them.

Redacto KMS can distinguish between two types of audit device failures.
...
- `skip_test` `(string: "")` When set to `"true"`, Redacto KMS will not send an
  initial test probe request to the audit log destination when enabling the
  audit device. By default, Redacto KMS tests the connection to the destination
  ...
  audit log. Starting with Redacto KMS v2.3.2, enabling new audit devices with this
...
Some Redacto KMS responses can be very large. Primarily, this affects list operations -
as Redacto KMS lacks pagination in its APIs, listing a very large collection can result
...
contains data outside of the usual conventions that apply to Redacto KMS list responses,
