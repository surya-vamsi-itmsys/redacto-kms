...
Sending a `SIGHUP` to the Redacto KMS process will cause `http` audit devices to
...
```shell-session
$ redacto-kms audit enable http uri=https://my-log-server.local/ingress
```
...
```shell-session
$ redacto-kms audit enable -path="openbao_audit_1" http uri=https://my-second-log-server.local/ingress
```
