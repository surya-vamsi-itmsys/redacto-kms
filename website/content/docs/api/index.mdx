...
description: Redacto KMS has an HTTP API that can be used to control every aspect of Redacto KMS.
---

# HTTP API

The Redacto KMS HTTP API gives you full access to Redacto KMS using [REST like HTTP verbs](https://en.wikipedia.org/wiki/Representational_state_transfer).
Every aspect of Redacto KMS can be controlled using the APIs. The Redacto KMS CLI uses the
HTTP API to access Redacto KMS similar to all other consumers.
...
**Backwards compatibility:** At the current version, Redacto KMS does not yet
...
Once Redacto KMS is unsealed, almost every other operation requires a _client token_.
...
authentication to Redacto KMS itself. These endpoints are specific to each auth
method.
...
Several Redacto KMS APIs require specifying path parameters. The path parameter cannot end
in periods. Otherwise, Redacto KMS will return a 404 unsupported path error.
...
then the resulting request path to Redacto KMS will be `ns1/ns2/secret/foo`.

Note that it is semantically equivalent to use the full path rather than the
`X-Vault-Namespace` header, Redacto KMS will match the corresponding namespace
...
Typically the request data, body and response data to and from Redacto KMS is in JSON.
Redacto KMS sets the `Content-Type` header appropriately with its response and does
...
is mounted by default on a fresh Redacto KMS install and is of type `kv`.
...
Redacto KMS currently considers `PUT` and `POST` to be synonyms. Rather than trust a
client's stated intentions, Redacto KMS engines can implement an existence check to
discover whether an operation is actually a create or update operation based on
the data already stored within Redacto KMS. This makes permission management via ACLs
more flexible.
...
For more examples, please look at the Redacto KMS API client.

## The `X-Vault-Request` header

Requests that are sent to a [Redacto KMS Proxy][proxy] that is configured to use the
...
The Redacto KMS CLI always adds this header to every request, regardless of whether
the request is being sent to a Redacto KMS Agent or directly to a Redacto KMS Server. In
addition, the Redacto KMS SDK always adds this header to every request.

## Help

To retrieve the help for any API within Redacto KMS, including mounted engines, auth
...
Additionally, with the [OpenAPI generation](system/internal-specs-openapi.mdx) in Redacto KMS, you will get back a small
...
The following HTTP status codes are used throughout the API. Redacto KMS tries to
...
- `502` - A request to Redacto KMS required Redacto KMS making a request to a third party;
  the third party responded with an error of some kind.
- `503` - Redacto KMS is down for maintenance or is currently sealed. Try again
  later.
...
Redacto KMS's server configuration file.
