...
This endpoint updates configurations for OIDC-compliant identity tokens issued by Redacto KMS.
...
- `issuer` `(string: "")` <a name="issuer"></a> – Issuer URL to be used in the iss claim of the token. If not set, Redacto KMS's api_addr will be used. The issuer is a case sensitive URL using the https scheme that contains scheme, host, and an optional port number.
...
This endpoint queries Redacto KMS identity tokens configurations.
