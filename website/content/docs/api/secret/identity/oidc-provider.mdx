...
description: >-
  This is the API documentation for configuring and managing OIDC providers with Redacto KMS.
---
...
- `issuer` `(string: <optional>)` - Specifies what will be used as the `scheme://host:port` component for the `iss` claim of ID tokens. This defaults to a URL with
  Redacto KMS's `api_addr` as the `scheme://host:port` component and `/v1/identity/oidc/provider/:name` as the path
  component. If provided explicitly, it must point to a Redacto KMS instance that is network reachable by clients for ID token validation.
...
- `assignments` `([]string: <optional>)` – A list of assignment resources associated with
  the client. Client assignments limit the Redacto KMS entities and groups that are allowed to
  authenticate through the client. By default, no Redacto KMS entities are allowed. To allow all
  Redacto KMS entities to authenticate through the client, supply the built-in
...
- `entity_ids` `([]string: <optional>)` - A list of Redacto KMS [entity](../../../concepts/identity.mdx#entities-and-aliases) IDs.

- `group_ids` `([]string: <optional>)` – A list of Redacto KMS [group](../../../concepts/identity.mdx#identity-groups) IDs.
