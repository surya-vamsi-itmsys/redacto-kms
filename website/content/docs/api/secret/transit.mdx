...
description: This is the API documentation for the Redacto KMS Transit secrets engine.
---

# Transit secrets engine (API)

This is the API documentation for the Redacto KMS Transit secrets engine. For general
...
`/transit` path in Redacto KMS. Since it is possible to enable secrets engines at any
...
returned by Redacto KMS and the encryption of the import key material under the
...
- `allow_rotation` `(bool: false)` - If set, the imported key can be rotated
  within Redacto KMS by using the `rotate` endpoint.

  :::warning

  **NOTE**: Once an imported key is rotated within Redacto KMS, it will no longer
...
**Note**: Keys whose material was generated by Redacto KMS do not support
importing key material. Only keys that were previously imported into
Redacto KMS can import new key material from an external source.
...
rotated within Redacto KMS, it will not support further import operations.
...
Redacto KMS instances to secure established shared key material, within exposing
...
Redacto KMS HTTP API imposes a maximum request size of 32MB to prevent a denial of service attack. This can be tuned per [`listener` block](../../configuration/listener/tcp.mdx) in the Redacto KMS server configuration.
...
is returned depends on the path; as a result, you can use Redacto KMS ACL policies to
