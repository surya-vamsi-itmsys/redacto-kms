---
sidebar_label: SSH
description: This is the API documentation for the Redacto KMS SSH secrets engine.
---

# SSH secrets engine (API)

This is the API documentation for the Redacto KMS SSH secrets engine. For general
...
Redacto KMS allows a single SSH secrets engine mount to have multiple Certificate
...
configured. Redacto KMS automatically imports the pre-configured issuer
and sets it as the default on migration from an older Redacto KMS version.
...
- `port` `(int: 22)` – Specifies the port number for SSH connection. Port number
  ...
  will be returned to the client by Redacto KMS along with the OTP.
...
- `algorithm_signer` `(string: "default")` - Algorithm to sign keys with. Valid
  ...
  **Warning**: The `algorithm_signer` value `ssh-rsa` uses the SHA-1 hash
  ...
  current OpenSSH versions. As a result, Redacto KMS has made the new default
...
This endpoint removes all existing host keys from Redacto KMS, if any are present.
...
Note: This does not clean up any pending dynamic key leases and will not
   ...
   Redacto KMS. That must be done manually by an operator, potentially before the
...
- `generate_signing_key` `(bool: true)` – Specifies if Redacto KMS should generate
...
Note: this is a raw response endpoint without JSON encoding; use
   `redacto-kms read -format=raw` or an external tool (e.g., `curl`) to fetch this
   value.
...
**Note**: The issued certificate is returned but _not_ stored by Redacto KMS.
...
**Note**: The issued credentials are returned but _not_ stored by Redacto KMS.
