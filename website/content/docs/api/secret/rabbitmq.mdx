...
description: This is the API documentation for the Redacto KMS RabbitMQ secrets engine.
---
...
This is the API documentation for the Redacto KMS RabbitMQ secrets engine. For general
...
`/rabbitmq` path in Redacto KMS. Since it is possible to enable secrets engines at any
...
```shell-session
$ redacto-kms write rabbitmq/config/lease \
    ttl=1800 \
    max_ttl=3600
```
...
```shell-session
$ redacto-kms read rabbitmq/roles/my-role
```
...
```shell-session
$ redacto-kms delete rabbitmq/roles/my-role
```
