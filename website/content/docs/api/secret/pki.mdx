---
sidebar_label: PKI
description: This is the API documentation for the Redacto KMS PKI secrets engine.
---

# PKI secrets engine (API)

This is the API documentation for the Redacto KMS PKI secrets engine. For general
...
in Redacto KMS. Since it is possible to enable secrets engines at any location, please
...
- [Redacto KMS CLI with DER/PEM responses](#redacto-kms-cli-with-derpem-responses)

## Notice about new Multi-Issuer functionality

Redacto KMS allows a single PKI mount to have multiple Certificate
...
configured. Redacto KMS automatically selects the default issuer from the
current issuing certificate on migration from an older Redacto KMS version.

:::

## ACME certificate issuance

Redacto KMS supports the [ACME certificate lifecycle management
...
Redacto KMS PKI supports the following ACME directories, providing different
...
These endpoints are unauthenticated from a Redacto KMS authentication model, but
...
Redacto KMS supports the following ACME challenge types presently:
...
have a valid external account binding to Redacto KMS. Before registering a new account,
an authenticated Redacto KMS client will need to [fetch a new EAB
...
```shell-session
$ redacto-kms write -f /pki/acme/new-eab
$ certbot certonly --server https://localhost:8200/v1/pki/acme/directory \
                   --eab-kid <id> --eab-hmac-key <hmac-key>
```

With or without EAB, requests from the ACME client are not authenticated using
traditional Redacto KMS authentication, but are instead authenticated through the
ACME protocol. With EAB however, a Redacto KMS authenticated client will have to
...
Redacto KMS endpoint, but not further to the client's entity or other information.

:::warning

Note: Enabling EAB is strongly recommended for public-facing Redacto KMS
deployments. Use of the `REDACTO_KMS_DISABLE_PUBLIC_ACME` environment variable
...
```shell-session
$ redacto-kms secrets tune -allowed-response-headers=Location -allowed-response-headers=Replay-Nonce \
...
Redacto KMS PKI secrets engine presently only allows revocation by serial number;
...
- `issuer_ref` `(string: <required>)` - Reference to an existing issuer,
  either by Redacto KMS-generated identifier, the literal string `default` to
...
- `format` `(string: "pem")` - Specifies the format for returned data. Can be
  ...
  Redacto KMS-derived self-signed root, this will be included as well.
...
- `format` `(string: "pem")` - Specifies the format for returned data. Can be
  ...
  issuing CA is not a Redacto KMS-derived self-signed root, it will be concatenated
...
This endpoint can be used both when signing a Redacto KMS-backed intermediate or
...
- `serial_number` `(string: "")` -  - Specifies the requested Subject's named
  ...
  Certificate's serial number field, which Redacto KMS randomly generates.
...
- `skid` `(string: "")` - Value for the Subject Key Identifier field
  ...
  is empty, allowing Redacto KMS to automatically calculate the SKID according
...
don't want/can't get access to a CSR (such as if it's a root stored in Redacto KMS
...
alternative option to the standard method of revoking using Redacto KMS lease IDs. A
...
This is an alternative option to the standard method of revoking using Redacto KMS
...
has a private key corresponding to a certificate signed by Redacto KMS. However,
to avoid third parties performing a denial-of-service (DOS) against Redacto KMS,
...
- `private_key` `(string: <required>)` - Specifies the private key (in PEM
  format) corresponding to the certificate issued by Redacto KMS that is attempted
  ...
  certificates as Redacto KMS does not maintain such a mapping.
...
| `GET`  | `/pki/ca`                      | `default` | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") |
| `GET`  | `/pki/ca/pem`                  | `default` | [PEM](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") |
| `GET`  | `/pki/issuer/:issuer_ref/json` | Selected  | JSON                                                                           |
| `GET`  | `/pki/issuer/:issuer_ref/der`  | Selected  | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") |
| `GET`  | `/pki/issuer/:issuer_ref/pem`  | Selected  | [PEM](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") |
...
| Method | Path                 | Issuer    | Format                                                                         |
| :----- | :------------------- | :-------- |:-------------------------------------------------------------------------------|
| `GET`  | `/pki/ca_chain`      | `default` | [PEM](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") |
| `GET`  | `/pki/cert/ca_chain` | `default` | JSON                                                                           |

:::warning

**Note**: These endpoints return the full chain (including the default
issuer's certificate and all parent issuers known to Redacto KMS) in these
responses.
...
| `GET`  | `/pki/crl`                                      | `default` | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Complete | Local  |
| `GET`  | `/pki/crl/pem`                                  | `default` | [PEM](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Complete | Local  |
| `GET`  | `/pki/cert/delta-crl`                           | `default` | JSON                                                                           | Delta    | Local  |
| `GET`  | `/pki/crl/delta`                                | `default` | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Delta    | Local  |
| `GET`  | `/pki/crl/delta/pem`                            | `default` | [PEM](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Delta    | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl`                   | Selected  | JSON                                                                           | Complete | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/der`               | Selected  | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Complete | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/pem`               | Selected  | [PEM](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Complete | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/delta`             | Selected  | JSON                                                                           | Delta    | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/delta/der`         | Selected  | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Delta    | Local  |
| `GET`  | `/pki/issuer/:issuer_ref/crl/delta/pem`         | Selected  | [PEM](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Delta    | Local  |
...
At this time there are certain limitations of the OCSP implementation at this path:
 ...
 1. Note that this API will not work with the Redacto KMS client as both request and responses are DER encoded, and
...
| :----- | :--------------------------------------------------------- | :----------------------------------------------------------------------------- | :------ |
| `GET`  | `/pki/ocsp/<base 64+URL encoded ocsp DER request>`         | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Local   |
| `POST` | `/pki/ocsp`                                                | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") | Local   |
...
```shell-session
openssl ocsp -no_nonce -issuer issuer.pem -CAfile ca_chain.pem -cert cert-to-revoke.pem -text -url $REDACTO_KMS_ADDR/v1/pki/ocsp
```
...
| `GET`  | `/pki/cert/:serial/raw`     | [DER](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") |
| `GET`  | `/pki/cert/:serial/raw/pem` | [PEM](#redacto-kms-cli-with-derpem-responses "Redacto KMS CLI With DER/PEM Responses") |
...
(including this certificate and all parent issuers known to Redacto KMS) in
...
key. If using Redacto KMS as a root (and, like many other CAs), the various parameters
...
Note: Redacto KMS will deduplicate differently-encoded but same-valued keys and
...
**Note** `POST`ing to this endpoint causes Redacto KMS to overwrite the previous
...
Redacto KMS supports the PATCH operation to this endpoint, using the [JSON patch
format](https://datatracker.ietf.org/doc/html/rfc6902) supported by KVv2,
allowing update of specific fields. Note that `redacto-kms write` uses `POST`.
...
- `manual_chain` `([]string: nil)` - Chain of issuer references to build this
  ...
  Redacto KMS. Because each cross-signed intermediate will only know of the
...
- `crl_distribution_points` `(array<string>: nil)` - Specifies the URL values
  ...
  aggregated into a single CRL (externally; as Redacto KMS does not support this
...
**Note**: Redacto KMS does not allow reading the value of the private key after
it has been created.
...
**Note**: Because Redacto KMS does not allow exporting the private key
...
As a result, Redacto KMS prohibits deletion of keys until **all** issuers
...
Redacto KMS to overwrite the contents of the role, using the provided request
...
- `issuer_ref`: `(string: "default")` - Specifies the default issuer of this
  ...
  **Note**: existing roles from previous Redacto KMS versions are migrated to use
...
- `allowed_serial_numbers` `(string: "")` - If set, an array of allowed serial
  ...
  forbidden. It is strongly recommended to allow Redacto KMS to generate random
...
- `generate_lease` `(bool: false)` - Specifies if certificates issued/signed
  against this role will have Redacto KMS leases attached to them. Certificates can be
  added to the CRL by `redacto-kms revoke <lease_id>` when certificates are associated
  ...
  disabled, as large amount of leases adversely affect the startup time of Redacto KMS.
...
- `enable_templating` `(bool: false)` - Specifies that the above AIA
  ...
  use an external, non-Redacto KMS CDN.
...
It also has `aia_path`, which allows using a non-Redacto KMS, external responder,
...
- `aia_path` `(string: "")` - Specifies the path to this  cluster's AIA
  distribution point; may refer to an external, non-Redacto KMS responder.  This is
...
- `ocsp_disable` `(bool: false)` - Disables or enables the OCSP responder in Redacto KMS.
...
**Note**: Mirroring the behavior of earlier Redacto KMS versions, we add
...
though distribution must still occur outside of Redacto KMS (either manually or
...
CRL of revocations across distinct Redacto KMS clusters.
...
- `maintain_stored_certificate_counts` `(bool: false)` - When enabled,
  ...
  **Note**: It is strongly recommend to not enable this value if 50k or more
  ...
  cluster. Instead, use audit logs and aggregate this data externally to Redacto KMS
  so as not to impact Redacto KMS performance.
...
Redacto KMS above and beyond what the CEL program contains. It is up to the
...
## Redacto KMS CLI with DER/PEM responses

The Redacto KMS CLI can only display JSON responses. For APIs that return non-JSON formatted
data such as DER and PEM formats, `redacto-kms read` will fail without the `-format=raw`
