...
description: >-
  The PostgreSQL plugin for Redacto KMS's database secrets engine generates database
...
- `password_authentication` `(string: "password")` - When set to "scram-sha-256", passwords will be hashed by Redacto KMS and stored as-is by PostgreSQL.
