...
description: >-
  The Cassandra plugin for Redacto KMS's database secrets engine generates database
...
If using the Redacto KMS CLI, it's probably easiest to write the JSON to a file and then reference the file:

```shell-session
$ redacto-kms write database/config/cassandra-example <...other fields> pem_json=@/path/to/file.json
```

</details>

- `skip_verification` `(bool: false)` - Skip permissions checks when a connection to Cassandra
  is first created. These checks ensure that Redacto KMS is able to create roles, but can be resource
