...
This is the API documentation for the Redacto KMS Database secrets engine. For
general information about the usage and operation of the database secrets engine,
please see the
[Redacto KMS database secrets engine documentation](../../../secrets/databases/index.mdx).

This documentation assumes the database secrets engine is enabled at the
`/database` path in Redacto KMS. Since it is possible to enable secrets engines at any
...
We highly recommended that you use a Redacto KMS-specific user rather than the admin user
...
- `password` `(string)` - Specifies the password to use when connecting with the
  `username`. This value will not be returned by Redacto KMS when performing a read upon the
...
```shell-session
$ redacto-kms write database/config/mysql \
    plugin_name="mysql-database-plugin" \
    allowed_roles="readonly" \
    connection_url="{{username}}:{{password}}@tcp(127.0.0.1:3306)/" \
    username="openbaouser" \
...
recommended that you create a user for Redacto KMS to utilize rather than using the actual root user.
...
1-to-1 mapping of a Redacto KMS Role to a user in a database which are automatically
...
Redacto KMS will rotate the password when creating a static role. Redacto KMS must do
...
- `username` `(string: <required>)` – Specifies the database username that this
  Redacto KMS role corresponds to.

- `rotation_period` `(string/int: <required>)` – Specifies the amount of time
  Redacto KMS should wait before rotating the password. The minimum is 5 seconds.
...
role name. While Static Roles are rotated automatically by Redacto KMS at configured
