...
description: This is the API documentation for the Redacto KMS Kubernetes secrets engine.
---

# Kubernetes secrets engine (API)

This is the API documentation for the Redacto KMS Kubernetes secrets engine. To
...
`/kubernetes` path in Redacto KMS. Since it is possible to enable secrets engines at
...
for Redacto KMS's own Kubernetes service account.
...
- `kubernetes_namespace` `(string)` - The name of the Kubernetes
  namespace in which to generate the credentials. Optional if the Redacto KMS role is configured
...
- `cluster_role_binding` `(bool: false)` - If true, generate a ClusterRoleBinding
  ...
  Requires the Redacto KMS role to have `kubernetes_role_type` set to ClusterRole.
- `ttl` `(string: "")` - The TTL of the generated Kubernetes service account token,
  ...
  Kubernetes, Redacto KMS system-wide controls, or role-specific controls.
