...
description: This is the API documentation for the Redacto KMS AppRole auth method.
---

# AppRole auth method (API)

This is the API documentation for the Redacto KMS AppRole auth method. For
general information about the usage and operation of the AppRole method, please
see the [Redacto KMS AppRole method documentation](../../auth/approle.mdx).

This documentation assumes the AppRole method is mounted at the `/auth/approle`
path in Redacto KMS. Since it is possible to enable auth methods at any location,
...
Issues a Redacto KMS token based on the presented credentials. `role_id` is always
...
to be able to delegate specific endpoints using Redacto KMS's ACL system.
