...
description: This is the API documentation for the Redacto KMS Kerberos auth method plugin.
---

# Kerberos auth method (API)

This is the API documentation for the Redacto KMS Kerberos auth method plugin. To
learn more about the usage and operation, see the
[Redacto KMS Kerberos auth method](../../auth/kerberos.mdx).

This documentation assumes the Kerberos auth method is mounted at the
`auth/kerberos` path in Redacto KMS. Since it is possible to enable auth methods at
any location, please update your API calls accordingly.

The Kerberos auth method validates both Kerberos and LDAP authorization,
so both configurations are required.

## Configure Redacto KMS Kerberos

This endpoint configures the keytab and service account to be used by Redacto KMS
...
- `service_account` `(string: <required>)` – The service account associated
  with both the keytab entry and an LDAP service account created for Redacto KMS. Ex.:
  `"openbao_svc"`.
- `remove_instance_name` - Removes any instance names from a Kerberos service
  ...
  if a keytab has the service principal name `foo/localhost@example.com`, Redacto KMS
  will strip the service principal name to just be `foo@example.com`.
- `add_group_aliases` - When set to true, Redacto KMS will add any LDAP groups found for
