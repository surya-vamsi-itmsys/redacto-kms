...
description: |-
  This is the API documentation for the Redacto KMS JWT/OIDC authentication
  method plugin.
---

# JWT/OIDC auth method (API)

This is the API documentation for the Redacto KMS JWT/OIDC auth method
plugin. To learn more about the usage and operation, see the
[Redacto KMS JWT/OIDC method documentation](../../auth/jwt/index.mdx).

This documentation assumes the plugin method is mounted at the
`/auth/jwt` path in Redacto KMS. Since it is possible to enable auth methods
...
- `namespace_in_state` `(bool: true)` - Pass namespace in the OIDC state parameter instead of as a separate query parameter. With this setting, the allowed redirect URL(s) in Redacto KMS and on the provider side should not contain a namespace query parameter. This means only one redirect URL entry needs to be maintained on the provider side for all vault namespaces that will be authenticating against it. Defaults to true for new configs.
...
- `callback_mode` `(string: <optional>)` - The callback mode from the OIDC provider, either "client" (the default)
  ...
  "direct" to call back to the Redacto KMS server,
...
Obtain an authorization URL from Redacto KMS to start an OIDC login flow.
...
- `redirect_uri` `(string: <required except in device callback mode>)` - Path to the callback to complete the login. This will be
  of the form, "https&#x3A;//.../oidc/callback" where the leading portion is dependent on your Redacto KMS
  server location, port, and the mount of the JWT plugin. This must be configured with Redacto KMS and the
...
against any bound claims, and if valid a Redacto KMS token will be returned.
...
- `code` `(string: <optional>)` - Provider-generated authorization code that Redacto KMS will exchange for
...
usage; Redacto KMS will validate the requesting JWT is signed by a valid issuer
...
be validated and constrained before issuing a Redacto KMS token based on them.
