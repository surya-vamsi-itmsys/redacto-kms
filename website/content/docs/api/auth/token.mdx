...
description: This is the API documentation for the Redacto KMS token auth method.
---

# Token auth method (API)

This is the API documentation for the Redacto KMS token auth method. For
general information about the usage and operation of the token method, please
see the [Redacto KMS Token method documentation](../../auth/token.mdx).
...
- `increment` `(string: "")` - An optional requested increment duration can be
  ...
  If not supplied, Redacto KMS will use the default TTL. This is specified as a numeric string
...
Second, this will cause Redacto KMS's memory usage to balloon up, because the default
Redacto KMS internal cache is unlimited in size and every value read from storage will
...
for your cluster, it would be wise to give Redacto KMS more resources before attempting
...
```json
{
  ...
  "wrap_info": null,
  "warnings": [
    "Tidy operation successfully started. Any information from the operation will be printed to Redacto KMS's server logs."
  ],
  "auth": null
}
```
