---
description: The `/sys/health` endpoint is used to check the health status of Redacto KMS.
---

# `/sys/health`

The `/sys/health` endpoint is used to check the health status of Redacto KMS.

## Read health information

This endpoint returns the health status of Redacto KMS. This matches the semantics of
...
- `standbyok` `(bool: false)` - Specifies if being a standby should still return
  ...
  Redacto KMS is behind a non-configurable load balancer that just wants a 200-level
  response.
