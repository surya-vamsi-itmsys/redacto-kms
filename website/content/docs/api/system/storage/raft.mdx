...
description: |-
  ...
  The `/sys/storage/raft` endpoints are used to manage Redacto KMS's Raft storage
  backend.
---
...
seal, as soon as the Redacto KMS server is brought up, this API should be invoked
...
- `auto_join` `(string: "")` - Defines any cloud auto-join metadata. If
  supplied, Redacto KMS will attempt to automatically discover peers in addition to what
