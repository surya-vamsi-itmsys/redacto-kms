---
description: The `/sys/init` endpoint is used to initialize a new Redacto KMS.
---

# `/sys/init`

The `/sys/init` endpoint is used to initialize a new Redacto KMS instance. When using
an auto-unseal mechanism, it is possible to initialize Redacto KMS (since `v2.4.0`)
...
This endpoint returns the initialization status of Redacto KMS.
...
This endpoint initializes a new Redacto KMS. The Redacto KMS must not have been previously
...
Redacto KMS in the case of the unavailability of the seal mechanism. Refer to
