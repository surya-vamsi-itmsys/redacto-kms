...
The `/sys/rotate/root` endpoints are used to rotate the root key for Redacto KMS,
...
Redacto KMS. These endpoints only apply to seals that support recovery keys.
...
key share to progress the rotation of the Redacto KMS. If the threshold number
of key shares is reached, Redacto KMS will complete the rotation.
