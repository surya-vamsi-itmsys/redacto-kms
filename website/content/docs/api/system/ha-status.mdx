---
description: The `/sys/ha-status` endpoint is used to check the HA status of a Redacto KMS cluster.
---

# `/sys/ha-status`

The `/sys/ha-status` endpoint is used to check the HA status of a Redacto KMS cluster.
It shows currently active node, along with peers that it's heard from since it became active.

## HA status

This endpoint returns the HA status of the Redacto KMS cluster.
