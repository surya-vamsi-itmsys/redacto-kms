...
The `/sys/generate-root` endpoint is used to create a new root token for Redacto KMS.
...
Redacto KMS will complete the root generation and issue the new token. Otherwise,
