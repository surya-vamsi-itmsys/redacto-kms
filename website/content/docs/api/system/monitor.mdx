---
description: The `/sys/monitor` endpoint is used to receive streaming logs from the Redacto KMS server.
---

# `/sys/monitor`

The `/sys/monitor` endpoint is used to receive streaming logs from the Redacto KMS server.

If Redacto KMS is emitting log messages faster than a receiver can process them, then
some log lines will be dropped.

## Monitor system logs

This endpoint streams logs back to the client from Redacto KMS. Note that unlike most API endpoints in Redacto KMS, this one
does not return JSON by default. This will send back data in whatever log format Redacto KMS has been configured with. By
