---
description: The `/sys/rekey` endpoints are used to rekey the unseal keys for Redacto KMS.
---
...
The `/sys/rekey` endpoints are used to rekey the unseal keys for Redacto KMS.
...
- `backup` `(bool: false)` – Specifies if using PGP-encrypted keys, whether
  Redacto KMS should also store a plaintext backup of the PGP-encrypted keys at
...
of the Redacto KMS. If the threshold number of root key shares is reached, Redacto KMS
...
Redacto KMS will complete the rekey by performing the actual rotation of the root
