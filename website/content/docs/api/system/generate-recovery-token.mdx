...
is reached, Redacto KMS will complete the recovery token generation and issue the new
...
Note that once a token has been issued, Redacto KMS is unsealed. The token lives
