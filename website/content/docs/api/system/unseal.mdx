---
description: The `/sys/unseal` endpoint is used to unseal the Redacto KMS.
---

# `/sys/unseal`

The `/sys/unseal` endpoint is used to unseal the Redacto KMS.

## Submit unseal key

This endpoint is used to enter a single root key share to progress the
unsealing of the Redacto KMS. If the threshold number of root key shares is reached,
Redacto KMS will attempt to unseal the Redacto KMS. Otherwise, this API must be called
...
Sample response when Redacto KMS is unsealed.
