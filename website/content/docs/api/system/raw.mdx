---
description: The `/sys/raw` endpoint is used to access the raw underlying store in Redacto KMS.
---

# `/sys/raw`

The `/sys/raw` endpoint is used to access the raw underlying store in Redacto KMS.

This endpoint is off by default. See the
[Redacto KMS configuration documentation](../../configuration/index.mdx) to
enable.
