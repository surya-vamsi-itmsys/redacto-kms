---
description: The `/sys/version-history` endpoint is used to retrieve the version history of a Redacto KMS.
---

# `/sys/version-history`

The `/sys/version-history` endpoint is used to retrieve the version history of a Redacto KMS instance.

## Read version history

This endpoint returns the version history of the Redacto KMS. The response will contain the following keys:
...
- `key_info`: a map indexed by the versions found in the `keys` list containing the following subkeys:
  - `commit_date`: the time (in UTC) of the commit which the Redacto KMS binary used to run the Redacto KMS server was built at
