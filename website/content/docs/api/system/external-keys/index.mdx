...
description: The `/sys/external-keys` endpoint is used to configure external keys in Redacto KMS.
---

# `/sys/external-keys`

The `/sys/external-keys` endpoint is used to configure external keys in Redacto KMS.
...
- `plugin` `(string: <required>)` - Specifies the name of the KMS plugin to use,
  ...
  more information on this parameter's intersection with Redacto KMS's plugin system
...
delete the underlying KMS keys, only the mappings in Redacto KMS.
...
only the mapping in Redacto KMS.
