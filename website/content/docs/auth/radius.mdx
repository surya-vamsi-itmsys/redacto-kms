...
description: |-
  The "radius" auth method allows users to authenticate with Redacto KMS using an
...
```shell-session
$ redacto-kms login -method=radius username=sethvargo
```
...
1. Enable the radius auth method:
   ...
   ```text
   $ redacto-kms auth enable radius
   ```

1. Configure connection details for your RADIUS server.

   ```text
   $ redacto-kms write auth/radius/users/mitchellh policies=admins
   ```
   ...
   Alternatively, Redacto KMS can assign a configurable set of policies to any user
