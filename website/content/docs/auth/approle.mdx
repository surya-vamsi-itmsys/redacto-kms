...
Redacto KMS-defined _roles_. The open design of `AppRole` enables a varied set of
...
An "AppRole" represents a set of Redacto KMS policies and login constraints that must
...
```shell-session
$ redacto-kms write auth/approle/login \
...
for a code snippet demonstrating the authentication with Redacto KMS using the
AppRole auth method.
...
1. Create a named role:
   ...
   ```shell-session
   $ redacto-kms write auth/approle/role/my-role \
       secret_id_ttl=10m \
       token_num_uses=10 \
       token_ttl=20m \
       token_max_ttl=30m \
...
1. Fetch the RoleID of the AppRole:
   ...
   ```shell-session
   $ redacto-kms read auth/approle/role/my-role/role-id
   role_id     db02de05-fa39-4855-059b-67221c5c2f63
   ```
...
1. Get a SecretID issued against the AppRole:
   ...
   ```shell-session
   $ redacto-kms write -f auth/approle/role/my-role/secret-id
   secret_id               6a174c20-f6de-a53c-74d2-6018fcceff64
...
func getSecretWithAppRole() (string, error) {
	...
	// A combination of a Role ID and Secret ID is required to log in to Redacto KMS
	// with an AppRole.
	// First, let's get the role ID given to us by our Redacto KMS administrator.
