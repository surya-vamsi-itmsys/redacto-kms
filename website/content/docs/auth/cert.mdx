...
description: >-
  The "cert" auth method allows users to authenticate with Redacto KMS using TLS
  client certificates.
---
...
Please note that to use this auth method, `tls_disable` and `tls_disable_client_certs` must be false in the Redacto KMS
...
trusted CRL distribution point, and have Redacto KMS fetch the CRL as needed.
...
including Redacto KMS's `pki` method, multiple CRLs can successfully be used as
...
a configured certificate, in which case Redacto KMS will query the OCSP server either
...
**NOTE** The `-ca-cert` value used here is for the Redacto KMS TLS Listener CA
certificate, not the CA that issued the client authentication certificate. This
can be omitted if the CA used to issue the Redacto KMS server certificate is trusted
...
**NOTE** The `--cacert` value used here is for the Redacto KMS TLS Listener CA
certificate, not the CA that issued the client authentication certificate. This
can be omitted if the CA used to issue the Redacto KMS server certificate is trusted
by the local system executing this command.
...
1. Configure it with trusted certificates that are allowed to authenticate:
   ...
   ```text
   $ redacto-kms write auth/cert/certs/web \
       display_name=web \
       policies=web,prod \
