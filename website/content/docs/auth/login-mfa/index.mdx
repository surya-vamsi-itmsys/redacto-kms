...
Redacto KMS supports Multi-factor Authentication (MFA) for authenticating to an auth
method using different authentication types.  Login MFA is built on top of the
Identity system of Redacto KMS.

## MFA types

MFA in Redacto KMS includes the following login types:

- `Time-based One-time Password (TOTP)` - If configured and enabled on a login path,
  this would require a TOTP passcode along with a Redacto KMS token to be presented
  while invoking the API login request. The passcode will be validated against the
  TOTP key present in the caller's identify in Redacto KMS.
...
**NOTE:** Redacto KMS's built-in Login MFA feature does not protect against brute forcing of
...
```shell-session
$ redacto-kms write sys/mfa/validate -format=json @payload.json
```

#### Interactive CLI for login MFA

Redacto KMS supports an interactive way of authenticating to an auth method using CLI only if the
...
```shell-session
$ redacto-kms write -non-interactive sys/mfa/validate -format=json @payload.json
```

### TOTP passcode validation rate limit

Rate limiting of Login MFA paths are enforced by default.  Redacto KMS allows for 5
