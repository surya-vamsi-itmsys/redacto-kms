...
The `kubernetes` auth method can be used to authenticate with Redacto KMS using a
Kubernetes Service Account Token. This method of authentication makes it easy to
introduce a Redacto KMS token into a Kubernetes Pod.
...
can check with `redacto-kms read -field disable_iss_validation auth/kubernetes/config`.
...
```shell-session
$ redacto-kms write auth/kubernetes/login role=demo jwt=...
```
...
1. Use the `/config` endpoint to configure Redacto KMS to talk to Kubernetes. Use
   ...
   ```bash
   redacto-kms write auth/kubernetes/config \
   ...
   **Note:** The pattern Redacto KMS uses to authenticate Pods depends on sharing
   the JWT token over the network. Given the [security model of
   Redacto KMS](../internals/security.mdx), this is allowable because Redacto KMS is part
...
1. Create a named role:
   ...
   ```bash
   redacto-kms write auth/kubernetes/role/demo \
...
if the expiry time passes, and Redacto KMS will no longer be able to use the
...
reviewing tokens, so enabling issuer validation on the Redacto KMS side is
duplicated work. Without disabling Redacto KMS's issuer validation, it is not
...
you wish to enable issuer validation in Redacto KMS.
...
|--------------------------------------|----------------------------|-------------------------|-----------------------------------------------------------|
| Use local token as reviewer JWT      | Yes                        | Yes                     | Requires Redacto KMS to be deployed on the Kubernetes cluster |
...
When running Redacto KMS in a Kubernetes pod the recommended option is to use the pod's local
service account token. Redacto KMS will periodically re-read the file to support
...
Redacto KMS will attempt to load them from `token` and `ca.crt` respectively inside
...
#### Use the Redacto KMS client's JWT as the reviewer JWT

When configuring Kubernetes auth, you can omit the `token_reviewer_jwt`, and Redacto KMS
will use the Redacto KMS client's JWT as its own auth token when communicating with
the Kubernetes `TokenReview` API. If Redacto KMS is running in Kubernetes, you also need
to set `disable_local_ca_jwt=true`.

This means Redacto KMS does not store any JWTs and allows you to use short-lived tokens
...
Redacto KMS. Each client of Redacto KMS would need the `system:auth-delegator` ClusterRole:
...
recommended value for all versions of Redacto KMS.
...
// Fetches a key-value secret (kv-v2) after authenticating to Redacto KMS with a Kubernetes service account.
...
func getSecretWithKubernetesAuth() (string, error) {
	// If set, the REDACTO_KMS_ADDR environment variable will be the address that
	// your pod uses to communicate with Redacto KMS.

	config := openbao.DefaultConfig() // modify for more granular configuration

	client, err := openbao.NewClient(config)
	if err != nil {
		return "", fmt.Errorf("unable to initialize Redacto KMS client: %w", err)
	}
	...
	// get secret from Redacto KMS, from the default mount path for KV v2 in dev mode, "secret"
