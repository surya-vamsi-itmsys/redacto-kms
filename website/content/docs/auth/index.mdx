...
description: Auth methods are mountable methods that perform authentication for Redacto KMS.
---

# Auth methods

Auth methods are the components in Redacto KMS that perform authentication and are
responsible for assigning identity and a set of policies to a user. In all cases,
Redacto KMS will enforce authentication as part of the request processing. In most cases,
Redacto KMS will delegate the authentication administration and decision to the relevant configured
external auth method (e.g., Kubernetes).

Having multiple auth methods enables you to use an auth method that makes the
most sense for your use case of Redacto KMS and your organization.
...
they are mounted within the Redacto KMS mount table and can be accessed
...
```shell-session
$ redacto-kms auth enable -path=my-login userpass
```
...
When using an external auth method (e.g., Kubernetes), Redacto KMS will call the external service
...
disabled), Redacto KMS denies requests to **renew** tokens associated with the entity.
However, any existing token remain valid for the original grant period unless
they are explicitly revoked within Redacto KMS. Operators should set appropriate
[token TTLs](../concepts/tokens.mdx#the-general-case) when using external
authN methods.

## Authentication Flows

There are two primary flows client applications use to talk to Redacto KMS.
...
result in a more performant Redacto KMS instance than standard authentication.
