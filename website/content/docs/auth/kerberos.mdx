...
a Redacto KMS token for Kerberos entities.
...
Redacto KMS's Kerberos auth method was originally written by the folks at
...
and confirmed working, you will be ready to use Kerberos with Redacto KMS.

## Configuration

- Enable Kerberos authentication in Redacto KMS:

```shell-session
$ redacto-kms auth enable \
    -passthrough-request-headers=Authorization \
    -allowed-response-headers=www-authenticate \
    kerberos
```

- Create a `keytab` for the Kerberos plugin (this keytab is used by the Redacto KMS server itself, another keytab should be generated for login purposes):
...
The KVNO (`-k 1`) should match the KVNO of the service account. An error will show in the Redacto KMS logs if this is incorrect.
...
```shell-session
$ redacto-kms write auth/kerberos/config \
...
- Configure the Kerberos auth method to communicate with
  ...
  first tested your configuration from the Redacto KMS server using
  a tool like `ldapsearch`.

```shell-session
$ redacto-kms write auth/kerberos/config/ldap \
    binddn=openbao_svc@MATRIX.LAN \
    bindpass=$REDACTO_KMS_SVC_PASSWORD \
...
- Configure the Redacto KMS policies that should be granted to those
...
```shell-session
$ redacto-kms write auth/kerberos/groups/engineering-team \
    policies=engineers
```
...
```shell-session
$ redacto-kms login -method=kerberos \
    username=grace \
...
- `keytab_path` is the path to the `keytab` in which the entry lives for the
  entity authenticating to Redacto KMS. Keytab files should be protected from other
...
1. Use `ldapsearch` while logged into your machine hosting Redacto KMS to ensure
   your LDAP configuration is functional.
2. Authenticate to your domain server using `kinit`, your `keytab`, and your
   `krb5.conf`. Do this with both Redacto KMS's `keytab`, and any client `keytab` being
   used for logging in. This ensures your Kerberos network is working.
3. While logged into your client machine, verify you can reach Redacto KMS
   through the following command: `$ curl $REDACTO_KMS_ADDR/v1/sys/health`.
...
The Redacto KMS Kerberos library has a working integration test environment that
