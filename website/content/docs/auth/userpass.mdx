...
description: >-
  The "userpass" auth method allows users to authenticate with Redacto KMS using a
  username and password.
---

# Userpass auth method

The `userpass` auth method allows users to authenticate with Redacto KMS using
...
```shell-session
$ redacto-kms login -method=userpass \
    username=mitchellh \
    password=foo
```
...
1. Enable the userpass auth method:
   ...
   ```shell-session
   $ redacto-kms auth enable -path=<path> userpass
   ```

1. Configure it with users that are allowed to authenticate:

   ```shell-session
   $ redacto-kms write auth/<userpass:path>/users/mitchellh \
       password=foo \
       policies=admins
   ```
