...
Kubernetes can function as an OIDC provider such that Redacto KMS can validate its
...
still be considered valid by Redacto KMS until their expiry time. To mitigate this
...
* kube-apiserver's `--service-account-issuer` flag is set to a URL that is
  reachable from Redacto KMS. Public by default for most managed Kubernetes solutions.
* Must use short-lived service account tokens when logging in.
  * Tokens mounted into pods default to short-lived from 1.21.

#### Configuration steps when Redacto KMS is running in Kubernetes

Enable and configure JWT auth in Redacto KMS.

```bash
kubectl exec openbao-0 -- redacto-kms auth enable jwt
kubectl exec openbao-0 -- redacto-kms write auth/jwt/config -<<EOF
{
   "provider_config": {
       "provider": "kubernetes"
   }
...
Redacto KMS automatically derives the OIDC discovery URL from the `KUBERNETES_SERVICE_HOST` and
...
#### Configuration steps when Redacto KMS is _not_ running in Kubernetes:

:::info

**Note:** When Redacto KMS is outside the cluster, the `$ISSUER` endpoint below
...
1. Enable and configure JWT auth in Redacto KMS.

   ```bash
   redacto-kms auth enable jwt
   redacto-kms write auth/jwt/config oidc_discovery_url="${ISSUER}"
   ```
...
This method can be useful if Kubernetes' API is not reachable from Redacto KMS or if
...
1. Create a role for JWT auth that the `default` service account from the
   ...
   ```bash
   redacto-kms write auth/jwt/role/my-role \
      role_type="jwt" \
...
1. Pods or other clients with access to a service account JWT can then log in.
   ...
   ```bash
   redacto-kms write auth/jwt/login \
      role=my-role \
   ...
   curl \
      ...
      "${REDACTO_KMS_ADDR}/v1/auth/jwt/login"
   ```
...
when creating roles in Redacto KMS's JWT auth mount.
