...
1. Add Redirect URIs with the "Web" type. You may include two redirect URIs,
   one for CLI access another one for Redacto KMS UI access.
...
1. Under **Certificates & secrets**,
   ...
   Record the secret's value as you will need it as the `oidc_client_secret` for Redacto KMS.

### Connect AD group with Redacto KMS external group

To connect the AD group with a [Redacto KMS external groups](../../../concepts/identity.mdx#external-vs-internal-groups),
you will need
[Azure AD v2.0 endpoints](https://docs.microsoft.com/en-gb/azure/active-directory/develop/azure-ad-endpoint-comparison).
You should set up a [Redacto KMS policy](../../../concepts/policies.mdx) for the Azure AD group to use.

1. Go to **Azure Active Directory** and choose your Redacto KMS application.
...
1. In Redacto KMS, enable the OIDC auth method.
...
1. Configure the OIDC auth method with the `oidc_client_id` (application ID), `oidc_client_secret`
   ...
   ```shell
   redacto-kms write auth/oidc/config \
...
2. Configure the [OIDC Role](../../../api/auth/jwt.mdx#createupdate-role) with the following:
  ...
  from Azure.
   - `allowed_redirect_uris` should be the two redirect URIs for Redacto KMS CLI and UI access.
   - `groups_claim` should be set to `"groups"`.
   - `oidc_scopes` should be set to `"https://graph.microsoft.com/.default"`.

   ```shell
   redacto-kms write auth/oidc/role/your_default_role \
      user_claim="sub" \
...
1. In Redacto KMS, create the [external group](../../../api/secret/identity/group.mdx).
   Record the group ID as you will need it for the group alias.

1. From Redacto KMS, retrieve the [OIDC accessor ID](../../../api/system/auth.mdx#list-auth-methods)
   from the OIDC auth method as you will need it for the group alias's `mount_accessor`.

1. Go to the Azure AD Group you want to attach to Redacto KMS's external group. Record the `objectId`
   as you will need it as the group alias name in Redacto KMS.

1. In Redacto KMS, create a [group alias](../../../api/secret/identity/group-alias.mdx)
   for the external group and set the `objectId` as the group alias name.
   ```shell
   redacto-kms write identity/group-alias \
...
which will be used by Redacto KMS to retrieve the groups for the user. Additional API permissions
...
1. Configure the OIDC auth method in Redacto KMS by setting `"provider_config"` to Azure.
   ```shell
   redacto-kms write auth/oidc/config -<<"EOH"
   {
     "oidc_client_id": "your_client_id",
     "oidc_client_secret": "your_client_secret",
     "default_role": "your_default_role",
