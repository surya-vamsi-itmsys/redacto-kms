...
The `jwt` auth method can be used to authenticate with Redacto KMS using
...
user's web browser. This method may be initiated from the Redacto KMS UI or the
...
Redacto KMS includes two built-in OIDC login flows: the Redacto KMS UI, and the CLI
using a `redacto-kms login`.
...
done both in Redacto KMS and with the OIDC provider, and these configurations must align. The
redirect URIs are specified for a role with the `allowed_redirect_uris` parameter. There are
different redirect URIs to configure the Redacto KMS UI and CLI flows, so one or both will need to
be set up depending on the installation.

**CLI**

If you plan to support authentication via `redacto-kms login -method=oidc` and
...
where "host:port" is the Redacto KMS server name and port, and "path" is the path the JWT
...
When using `callbackmode=direct`, Redacto KMS shows an intermediate confirmation page to the user after the OIDC
...
If your OIDC provider always presents its own confirmation prompt to the user, the Redacto KMS confirmation page
...
Logging in via the Redacto KMS UI requires a redirect URI of the form:

`https://{host:port}/ui/vault/auth/{path}/oidc/callback`

The "host:port" must be correct for the Redacto KMS server, and "path" must match the path the JWT
backend is mounted at (e.g. "oidc" or "jwt").

If the [oidc_response_mode](../../api/auth/jwt.mdx#oidc_response_mode) is set to `form_post`, then
logging in via the Redacto KMS UI requires a redirect URI of the same form as the direct callback mode:

`https://{host:port}/v1/auth/{path}/oidc/callback`

### OIDC login (Redacto KMS UI)
...
```shell-session
$ redacto-kms login -method=oidc port=8400 role=test
...
- `callbackmode` (default: "client").  Mode of callback:
   ...
   `direct` for direct connection to the Redacto KMS server,
...
- `callbackhost` (default: the Redacto KMS's server and port in direct callback mode, else "localhost")
- `callbackmethod` (default: the method used for the Redacto KMS server in direct callback mode, else "http").
   The method to use in an OIDC `redirect_uri`.
- `callbackport` (default: value set for `port` in client callback mode, otherwise the port of the Redacto KMS
...
guide to configuring OAuth/OIDC applications is beyond the scope of Redacto KMS documentation, but a
...
- If a role parameter (e.g. `bound_claims`) requires a map value, it can't be set individually using
  the Redacto KMS CLI. In these cases the best approach is to write the entire configuration as a single
  JSON object:

```text
redacto-kms write auth/oidc/role/demo -<<EOF
{
  "user_claim": "sub",
...
- Monitor Redacto KMS's log output. Important information about OIDC validation failures will be emitted.

- Ensure Redirect URIs are correct in Redacto KMS and on the provider. They need to match exactly. Check:
...
The authentication flow for roles of type "jwt" is simpler than OIDC since Redacto KMS
...
```shell-session
$ redacto-kms write auth/jwt/login role=demo jwt=...
```
...
1. Enable the JWT auth method. Either the "jwt" or "oidc" name may be used. The
   ...
   $ redacto-kms auth enable jwt
     or
   $ redacto-kms auth enable oidc
   ```

1. Use the `/config` endpoint to configure Redacto KMS. To support JWT roles, either local keys, a JWKS URL, or an OIDC
   ...
   ```text
   $ redacto-kms write auth/jwt/config \
...
1. Create a named role:
   ...
   ```text
   redacto-kms write auth/jwt/role/demo \
