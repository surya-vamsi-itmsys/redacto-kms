...
configured audit devices failed to log a request (or response). If Redacto KMS cannot
properly audit a request, or the response to a request, the original request
will fail.

Refer to the Redacto KMS logs and any device-specific metrics to troubleshoot the
