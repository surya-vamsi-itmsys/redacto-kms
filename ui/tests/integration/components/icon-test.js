...
module('Integration | Component | icon', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(hbs`<Icon @name="redacto-mark" />`);
    assert.dom('.redacto-logo').exists('inlines the SVG');
    ...
    await render(hbs`<Icon @name="redacto-mark" @size="24"/>`);
    assert.dom('.hs-icon').hasClass('hs-icon-xl', 'adds the larger size class');

    const promise = waitForError();
    render(hbs`<Icon @name="redacto-mark" @size="12"/>`);
