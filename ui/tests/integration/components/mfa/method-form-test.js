...
module('Integration | Component | mfa-method-form', function (hooks) {
  ...
  test('it should create new mfa method', async function (assert) {
    ...
    await fillIn('[data-test-input="issuer"]', 'Redacto KMS');
    ...
    assert.strictEqual(this.model.issuer, 'Redacto KMS', 'Issuer property set on model');
  });

  test('it should populate form fields with model data', async function (assert) {
    assert.expect(3);

    this.model.issuer = 'Redacto KMS';
    ...
    await render(hbs`
      ...
      <div id="modal-wormhole"></div>
    `);
    assert.dom('[data-test-input="issuer"]').hasValue('Redacto KMS', 'Issuer input is populated');
