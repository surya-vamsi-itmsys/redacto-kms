...
module('Integration | Component | auth form', function (hooks) {
  ...
  const CSP_ERR_TEXT = `Error This is a standby Redacto KMS node but can't communicate with the active node via request forwarding. Sign in at the active node to use the Redacto KMS UI.`;
  ...
  test('it renders with Redacto KMS style errors', async function (assert) {
    assert.expect(1);
