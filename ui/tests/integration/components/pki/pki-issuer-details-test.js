...
module('Integration | Component | page/pki-issuer-details', function (hooks) {
  ...
  test('it renders parsing error banner if issuer certificate contains unsupported OIDs', async function (assert) {
    ...
    assert
      ...
      .hasText(
        "There was an error parsing certificate metadata Redacto KMS cannot display unparsed values, but this will not interfere with the certificate's functionality. However, if you wish to cross-sign this issuer it must be done manually using the CLI. Parsing error(s): certificate contains stuff we cannot parse"
      );
  });
  ...
  test('it renders parsing error banner if can_parse=false but no parsing_errors', async function (assert) {
    ...
    assert
      .dom(SELECTORS.parsingAlertBanner)
      .hasText(
        "There was an error parsing certificate metadata Redacto KMS cannot display unparsed values, but this will not interfere with the certificate's functionality. However, if you wish to cross-sign this issuer it must be done manually using the CLI."
      );
  });
