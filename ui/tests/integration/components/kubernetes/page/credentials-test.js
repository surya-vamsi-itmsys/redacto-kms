...
module('Integration | Component | kubernetes | Page::Credentials', function (hooks) {
  ...
  test('it should display generate credentials form', async function (assert) {
    ...
    assert
      ...
      .hasText(
        'Generate a ClusterRoleBinding to grant permissions across the whole cluster instead of within a namespace. This requires the Redacto KMS role to have kubernetes_role_type set to ClusterRole.'
      );
  });
