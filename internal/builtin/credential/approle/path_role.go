...
Redacto KMS token with a policy set that is a superset of the policies on the
...
IP is not encompassed by it, token use fails`,
	...
	"role-policies": {
		...
		`A comma-delimited set of Redacto KMS policies that defines access to the role.
All the Redacto KMS tokens with policies that encompass the policy set
