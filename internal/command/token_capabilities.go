...
func (c *TokenCapabilitiesCommand) Help() string {
	helpText := `
Usage: redacto-kms token capabilities [options] [TOKEN] PATH
  ...
  List capabilities for the local token on the "secret/foo" path:
      ...
      $ redacto-kms token capabilities secret/foo

  List capabilities for a token on the "cubbyhole/foo" path:

      $ redacto-kms token capabilities 96ddf4bc-d217-f3ba-f9bd-017055595017 cubbyhole/foo
