...
func (c *KVMetadataCommand) Help() string {
	helpText := `
Usage: redacto-kms kv metadata <subcommand> [options] [args]

  This command has subcommands for interacting with the metadata endpoint in
  Redacto KMS's key-value store. Here are some simple examples; more detailed
  examples are available in the subcommands or the documentation.

  Create or update a metadata entry for a key:

      $ redacto-kms kv metadata put -mount=secret -max-versions=5 -delete-version-after=3h25m19s foo
  ...
  Delete a key and all existing versions:
      ...
      $ redacto-kms kv metadata delete -mount=secret foo
