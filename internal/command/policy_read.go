...
func (c *PolicyReadCommand) Help() string {
	helpText := `
Usage: redacto-kms policy read [options] [NAME]

  Prints the contents and metadata of the Redacto KMS policy named NAME. If the policy
  does not exist, an error is returned.

  Read the policy named "my-policy":

      $ redacto-kms policy read my-policy
