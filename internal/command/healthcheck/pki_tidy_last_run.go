...
func (h *TidyLastRun) Evaluate(e *Executor) (results []*Result, err error) {
	...
	baseMsg := "Tidy hasn't run in the last %v; this can point to problems with the mount's auto-tidy configuration or an external tidy executor; this can impact PKI's and Redacto KMS's performance if not run regularly."
	...
	if h.TidyStatus.Secret != nil && h.TidyStatus.Secret.Data != nil {
		...
		if when == nil {
			...
			ret.Message = "Tidy hasn't run since this mount was created; this can point to problems with the mount's auto-tidy configuration or an external tidy executor; this can impact PKI's and Redacto KMS's performance if not run regularly. It is suggested to enable auto-tidy on this mount."
		} else {
			now := time.Now()
