...
func TLSFileWarningChecks(leafCerts, interCerts, rootCerts []*x509.Certificate) ([]string, error) {
	...
	if len(leafCerts) > 1 {
		warnings = append(warnings, "More than one leaf certificate detected. Please ensure that there is one unique leaf certificate being supplied to Redacto KMS in the Redacto KMS server configuration file.")
	}
...
func TLSMutualExclusionCertCheck(l *configutil.Listener) (int, string) {
	if l.TLSDisableClientCerts {
		if l.TLSRequireAndVerifyClientCert {
			return 1, "The tls_disable_client_certs and tls_require_and_verify_client_cert fields in the listener stanza of the Redacto KMS server configuration are mutually exclusive fields. Please ensure they are not both set to true."
		}
	}
	return 0, ""
}
